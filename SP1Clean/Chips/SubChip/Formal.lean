...
/-- Operands are 64-bit values (true on real and zero-padded rows). `is_real`-binary is proved by
soundness from the in-circuit gate; only completeness needs it as a prover precondition. -/
...
theorem soundness : GeneralFormalCircuit.Soundness (ZMod p) main Assumptions Spec := by
  -- `RTypeChipSpec` passed so `circuit_proof_start` unfolds the shared Spec-builder, re-normalizes

...
mirror lemma `formalSpec_of_subcircuit_specs` (`Lemmas.lean`). All
sub-circuit composition logic — including the `Word.isU64 op_b/op_c`
extraction from `h_alu` (via the
`ALUTypeReader.Gated.Assertion.isU64_op_b_of_spec` /
`isU64_op_c_memory_of_spec` helpers) and the bridge to the
`RV64.addw`-semantic conjunct — lives inside the named lemma.

The lowercase `ALUTypeReader.Gated.assertion.Spec` produced by
`circuit_proof_start` needs a one-line `simpa` bridge to the uppercase
`Gated.Assertion.Spec` form (mirror of SubwChip); without the bridge,
the inline-constructed `op_a_write_value` (`#v[addw_value[0],
addw_value[1], addw_msb*65535, addw_msb*65535]`) goes through
`Vector.map (Expression.eval env)` and doesn't reduce automatically. -/
theorem soundness :

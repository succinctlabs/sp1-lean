...
theorem soundness :
  ...
  -- Convert the poly-form scalar gates to the disjunction/equality shapes the
  -- canonical (a) FormalSpec now uses (Form A: conj 4; Form C: conj 5-7;
  -- Form B: conj 8).
  have h_isa_disj : input_is_auipc = 0 ∨ input_is_auipc = 1 :=
    (mul_sub_one_eq_zero_iff _).mp h_isa_bin'
  have h_a0_eq : input_addend[0] = input_is_auipc * input_state_pc[0] :=
    sub_eq_zero.mp h_a0
  have h_a1_eq : input_addend[1] = input_is_auipc * input_state_pc[1] :=
    sub_eq_zero.mp h_a1
  have h_a2_eq : input_addend[2] = input_is_auipc * input_state_pc[2] :=
    sub_eq_zero.mp h_a2
  have h_isreal_disj : input_is_real = 1 ∨ input_adapter_op_a_0 = 0 := by
    rcases mul_eq_zero.mp h_isreal_op_a_0' with h | h
    · left; linear_combination h
    · right; exact h
  -- Assemble FormalSpec's 9 conjuncts.
  refine ⟨h_cpu, ?_, ?_, h_isa_disj, h_a0_eq, h_a1_eq, h_a2_eq, h_isreal_disj, ?_⟩

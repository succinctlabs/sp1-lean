...
Sibling of `StoreWordChip` for half-word (16-bit) stores (`sh`). 45
columns: adds a 2-selector byte-position machinery (upper/lower
half-word at the address's 2-bit offset) on top of StoreWord's shape.
...
def TraceSpec (cols : StoreHalfCols (ZMod p)) : Prop :=
...
def SpecForIff_of_is_real (cols : StoreHalfCols (ZMod p)) : Prop :=
  ...
  cols.store_value[1] = cols.store_prev_value[1] +
      (cols.adapter.op_a_memory.prev_value[0] - cols.store_prev_value[1]) *
  ...
  cols.store_value[3] = cols.store_prev_value[3] +
      (cols.adapter.op_a_memory.prev_value[0] - cols.store_prev_value[3]) *
...
/-- The chip-level bridge for the Store Half variant. -/
theorem iff_sp1_of_is_real (Main : Vector (ZMod p) 45) (h_is_real : Main[44] = 1) :
    (_root_.Store.StoreHalf.constraints Main).allHold ↔
      SpecForIff_of_is_real (fromMain Main) := by
  haveI : NeZero p := ⟨by have := Fact.out (p := 2 ^ 17 < p); omega⟩
  change List.Forall SP1Constraint.toProp (_root_.Store.StoreHalf.constraints Main) ↔ _
  rw [_root_.Store.StoreHalf.allHold_constraints_iff_of_is_real Main h_is_real]
...
def assertion : FormalAssertion (ZMod p) StoreHalfCols :=
...
def main (cols : Var StoreHalfCols (ZMod p)) : Circuit (ZMod p) Unit := do
  let ⟨⟨clk_high, clk_16_24, clk_0_16, pc⟩,
       ...
       offset_bit_1, offset_bit_0, store_value, is_real, _adapter_cols⟩ := cols
  let clk_low : Expression (ZMod p) := clk_0_16 + clk_16_24 * 65536
  let opcode : Expression (ZMod p) := is_real * 37  -- SH
  -- The store halfword comes from op_a_memory.prev_value[0] (low 16 bits).
  let store_halfword : Expression (ZMod p) := op_a_memory.prev_value[0]
  SP1Clean.CPUState.assertion
    (⟨clk_0_16, clk_16_24⟩ : Var SP1Clean.CPUState.Inputs (ZMod p))
  ...
  SP1Clean.AddressShape.assertion
    (⟨addr_value, addr_top_two_limb_inv, 0, offset_bit_1, offset_bit_0⟩ :
  ...
  SP1Clean.StoreHalfAssembler.assertion
    (⟨store_prev_value, store_value, store_halfword,
       0, offset_bit_1, offset_bit_0⟩ :
      Var SP1Clean.StoreHalfAssembler.Inputs (ZMod p))
  is_real * (is_real - 1) === 0

@[reducible]
instance elaborated : ElaboratedCircuit (ZMod p) StoreHalfCols unit where
  name := "SP1Clean.StoreHalf.Gated"
  main := main
...
promoted `StoreMemoryAccessGated` sub-circuit; StoreHalfAssembler stays a
trivial-Spec stub. -/
def Assumptions (_ : StoreHalfCols (ZMod p)) : Prop := True
...
def assertionGated : FormalAssertion (ZMod p) StoreHalfCols :=

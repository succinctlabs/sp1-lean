...
/-- Chip-level Assumptions: store-memory contract (StoreByteAssembler stays a
trivial-Spec stub, so no extra conjunct needed). -/
def Assumptions (cols : StoreByteCols (ZMod p)) : Prop :=
...
set_option maxHeartbeats 800000 in
-- Same destructure as soundness; circuit_proof_start unfolds 6 sub-circuit
-- calls + 2 inline gates and exceeds the default cap.
theorem completeness :
    FormalAssertion.Completeness (ZMod p) elaborated Assumptions FormalSpec := by
  circuit_proof_start
  obtain ⟨⟨e1, e2, e3, e4⟩, e5, e6, e7, e8, e9, e10, e11, e12, e13, e14, e15, e16,
...
Mirror of `SP1Clean.StoreDouble`'s Phase 3 helpers for StoreByte (SB, width 1).
Opcode 36. `_root_.Store.StoreByte.correct` lives in the SP1Chips file (the
`Store.StoreByte` namespace) and verifies width-1 store semantics. -/

@[reducible] def sp1_op_a_cols (cols : StoreByteCols (ZMod p)) : BitVec 5 :=
  BitVec.ofNat 5 cols.adapter.op_a.val

@[reducible] def sp1_op_b_cols (cols : StoreByteCols (ZMod p)) : BitVec 5 :=
  BitVec.ofNat 5 cols.adapter.op_b.val

@[reducible] def sp1_imm_c_cols (cols : StoreByteCols (ZMod p)) : BitVec 12 :=
  BitVec.ofNat 12 (Word.toNat cols.adapter.op_c_imm)

@[reducible] def sp1_sb_cols (cols : StoreByteCols (ZMod p)) :
...
def storeByteInitialState_cols (cols : StoreByteCols (ZMod p))
    (s : SailState) : Prop :=
  ∀ Main : Vector (ZMod p) 50, fromMain Main = cols →
    (_root_.Store.StoreByte.constraints Main).initialState s

omit [Fact (Nat.Prime p)] [Fact (2 ^ 17 < p)] in
@[simp] lemma sp1_op_a_cols_fromMain (Main : Vector (ZMod p) 50) :
    sp1_op_a_cols (fromMain Main) = _root_.Store.StoreByte.sp1_op_a Main := rfl

omit [Fact (Nat.Prime p)] [Fact (2 ^ 17 < p)] in
@[simp] lemma sp1_op_b_cols_fromMain (Main : Vector (ZMod p) 50) :
    sp1_op_b_cols (fromMain Main) = _root_.Store.StoreByte.sp1_ob_b Main := rfl

omit [Fact (2 ^ 17 < p)] in
@[simp] lemma sp1_imm_c_cols_fromMain (Main : Vector (ZMod p) 50) :
    sp1_imm_c_cols (fromMain Main) = _root_.Store.StoreByte.sp1_imm_c Main := rfl

omit [Fact (2 ^ 17 < p)] in
@[simp] lemma sp1_sb_cols_fromMain (Main : Vector (ZMod p) 50) :
    sp1_sb_cols (fromMain Main) = _root_.Store.StoreByte.sp1_sb Main := rfl

omit [Fact (Nat.Prime p)] [Fact (2 ^ 17 < p)] in
/-- Cols round-trip, conditional on `is_trusted = is_real`. -/
lemma fromMain_toMain (cols : StoreByteCols (ZMod p))

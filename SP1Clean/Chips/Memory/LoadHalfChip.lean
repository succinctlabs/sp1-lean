...
/-- Chip-level Assumptions: the half-word selector contract. (The load-memory
contract is now derived from the promoted `LoadMemoryAccessGated` sub-circuit.) -/

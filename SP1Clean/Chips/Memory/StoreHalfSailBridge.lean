...
theorem sail_correct_of_allHold
    (cols : StoreHalfCols (ZMod p))
    (h_trusted : cols.adapter_cols.is_trusted = cols.is_real)
    (h_is_real : cols.is_real = 1)
    (h_cstrs : (_root_.Store.StoreHalf.constraints (toMain cols)).allHold)
    (s : SailState) (hs : SailState.isInitialized s)
    (hs_config : SailState.isValidMemConfig s hs)
    (h_init : storeHalfInitialState_cols cols s)
    (h_fits_in_mem :
      let reg_val :=
      ...
      is_aligned_vaddr (virtaddr.Virtaddr
        (Word.toBitVec64 cols.adapter.op_b_memory.prev_value +
          BitVec.signExtend 64
            (BitVec.ofNat 12 (Word.toNat cols.adapter.op_c_imm)))) 2 = true) :
    (_root_.Store.StoreHalf.spec_sb
        (sp1_imm_c_cols cols)
          ...
          BitVec.signExtend 64
            (BitVec.ofNat 12 (Word.toNat
              #v[(toMain cols)[21], (toMain cols)[22],
                 (toMain cols)[23], (toMain cols)[24]])))) 2 = true := by
    simpa using h_is_aligned
  have h_main := _root_.Store.StoreHalf.correct (toMain cols) s hs hs_config
    h_cstrs h_state h_is_real_main h_fits_main h_align_main
  simp only at h_main
  rw [show _root_.Store.StoreHalf.sp1_sb (toMain cols) = sp1_sb_cols cols from by
    rw [← sp1_sb_cols_fromMain, h_round_trip],
   show _root_.Store.StoreHalf.sp1_imm_c (toMain cols) = sp1_imm_c_cols cols from by
    rw [← sp1_imm_c_cols_fromMain, h_round_trip],
   show _root_.Store.StoreHalf.sp1_op_a (toMain cols) = sp1_op_a_cols cols from by
    rw [← sp1_op_a_cols_fromMain, h_round_trip],
   show _root_.Store.StoreHalf.sp1_ob_b (toMain cols) = sp1_op_b_cols cols from by

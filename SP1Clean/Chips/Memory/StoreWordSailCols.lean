...
/-! # Cols-level Sail helpers for `StoreWordChip` (Sail-isolation layer)

The Sail-monadic content for `StoreWord` lives here, quarantined out of the
pure-Clean `StoreWordChip.lean`. Mirrors `SP1Clean/Chips/Memory/LoadDoubleSailCols.lean`'s
cols-level Sail-side helpers + round-trip lemmas. Consumed only by
`StoreWordSailBridge.lean`, which lifts SP1's
`_root_.Store.StoreWord.correct_sw` to a cols-parameterized equivalence.

Splitting this off keeps `StoreWordChip.lean` free of `open Sail`/`SailM`, so the

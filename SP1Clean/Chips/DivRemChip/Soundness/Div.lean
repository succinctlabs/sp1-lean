...
/-! # `DivRemChip` — `div` conjunct soundness (split out for parallel compilation)

The signed-64-bit DIV conjunct. Proved as its own `GeneralFormalCircuit.Soundness` over a single-conjunct local `Spec`;

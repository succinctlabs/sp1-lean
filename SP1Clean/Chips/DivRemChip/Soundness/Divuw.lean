...
/-! # `DivRemChip` — `divuw` conjunct soundness (split out for parallel compilation)

The unsigned-32-bit word DIVUW conjunct. Proved as its own `GeneralFormalCircuit.Soundness` over a single-conjunct local `Spec`;

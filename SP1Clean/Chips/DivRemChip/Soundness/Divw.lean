...
/-! # `DivRemChip` — `divw` conjunct soundness (split out for parallel compilation)

The signed-32-bit word DIVW conjunct. Proved as its own `GeneralFormalCircuit.Soundness` over a single-conjunct local `Spec`;

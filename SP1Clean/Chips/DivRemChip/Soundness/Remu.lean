...
/-! # `DivRemChip` — `remu` conjunct soundness (split out for parallel compilation)

The unsigned-64-bit REMU conjunct. Proved as its own `GeneralFormalCircuit.Soundness` over a single-conjunct local `Spec`;

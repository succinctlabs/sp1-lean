...
/-! # `ShiftRightChip` — srlw conjunct soundness (split out for parallel compilation)

The logical word right-shift (SRLW) conjunct, proved as its own `GeneralFormalCircuit.Soundness` over a single-conjunct local
...
replaced by the single `intro hsrlw` for this conjunct. -/
...
theorem soundness : GeneralFormalCircuit.Soundness (ZMod p) main Assumptions Spec := by
  ...
  case spec =>
      ...
      have a2eq : env.get (i₀ + 2) = srwmsb * 65535 := by
        have hh := h_w4; rw [hsrlw, h_sraw0] at hh; simp only [id_eq] at hh ⊢; linear_combination hh
      have a3eq : env.get (i₀ + 3) = srwmsb * 65535 := by
        have hh := h_w5; rw [hsrlw, h_sraw0] at hh; simp only [id_eq] at hh ⊢; linear_combination hh
      rw [hcolsa]
      ...
      · -- byteShift = 0 (su0 = 1): a[0] = limb_result[0], a[1] = limb_result[1] = hl1.
        ...
        have ha0 : env.get i₀ = lr0 := by
          have hh := h_w0; rw [hsrlw, h_sraw0, su0o] at hh; simp only [id_eq] at hh ⊢

...
/-! # `ShiftRightChip` — srl conjunct soundness (split out for parallel compilation)

The logical right-shift (SRL) conjunct, proved as its own `GeneralFormalCircuit.Soundness` over a single-conjunct local
...
replaced by the single `intro hsrl` for this conjunct. -/

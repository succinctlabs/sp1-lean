...
/-! # `ShiftRightChip` — sraw conjunct soundness (split out for parallel compilation)

The arithmetic word right-shift (SRAW) conjunct, proved as its own `GeneralFormalCircuit.Soundness` over a single-conjunct local
...
replaced by the single `intro hsraw` for this conjunct. -/

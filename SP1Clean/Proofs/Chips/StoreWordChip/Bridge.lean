...
theorem correct_store_word_native
  ...
  have hwrite := run_vmem_write_of_width_4 rs1_idx reg_val (BitVec.signExtend 64 imm)
    (Sail.BitVec.extractLsb stored 31 0) sp hsp_init hsp_rs1 h_align' hsp_config h_in_range
...
theorem sw_chip_reaches_sail
  ...
  rw [h_imm] at hfacts
  have h_align4 :
      (Word.toBitVec64 input.op_b_val + BitVec.signExtend 64 imm).toNat % 4 = 0 := by
    rw [← Nat.mod_mod_of_dvd _ (by norm_num : (4 : ℕ) ∣ 8), ← hfacts.2.2]
    simp
  have hhi :
  ...
  refine correct_store_word_native rs1_idx rs2_idx imm (Word.toBitVec64 input.op_b_val)
    ...
    h_align4 (by omega) (by simpa using hfacts.2.1)
...
def ramAccessView (inp : Inputs (ZMod p)) (cols : StoreWordChip.Columns (ZMod p)) :
  ...
  { compareLow := inp.memory_access.access_timestamp.compare_low
    ...
    address := AddressOperation.alignedValue
      ⟨inp.op_b_val, inp.op_c_imm, 0, 0, inp.offset_bit, inp.is_real⟩
...
/-- **StoreWord's `advanceReady` bundle** (4-byte twin of StoreHalf's): the op_a read-back, the two
...
theorem advance (inp : Inputs (ZMod p)) (cols : StoreWordChip.Columns (ZMod p))
  ...
  have halignNat : (obv + sign_extend (m := 64) imm).toNat % 4 = 0 := by
    rw [← Nat.mod_mod_of_dvd _ (by norm_num : (4 : ℕ) ∣ 8), ← haddressFacts.2.2]
    simp [addressInput]
  ...
  exact execute_STORE_reaches_width4 imm rs1 rs2 obv opv t hinit hcfgt.toValidMemConfig
    ((hframe rs1).trans hrs1) ((hframe rs2).trans hrs2) haligned4 hinr

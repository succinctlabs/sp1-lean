...
/-! # `SP1Clean.AddwChip` — honest witness generation (`ComputableWitnesses`)

Addw on the witgen bridge (see `AddChip/Witgen.lean` for the programme note). The first chip here
with **two** witnessed payloads rather than one: the W-result's two low limbs (`valueIR`) and its
sign bit (`msbIR`). Both read the same two operand words, so both discharge from the same input
agreement. -/
...
theorem computableWitnesses : (circuit (p := p)).base.ComputableWitnesses := by
  ...
  refine ⟨FlatOperation.forAll_witnessCongr_of_subcircuit _ _ ?_,
    ...
    FlatOperation.forAll_witnessCongr_of_subcircuit _ _ ?_⟩
  · simp [circuit_norm]
  · refine AddwOperation.valueIR_congr env env' _ _ (fun i hi => ?_) (fun i hi => ?_)
    ...
    · have hv := congrArg (fun r : Inputs (ZMod p) => r.op_c_val) h_input
      ...
      simpa [Vector.getElem_map] using congrArg (fun v : Word (ZMod p) => v[i]) hv
  · refine AddwOperation.msbIR_congr env env' _ _ (fun i hi => ?_) (fun i hi => ?_)
    · have hv := congrArg (fun r : Inputs (ZMod p) => r.op_b_val) h_input
      simp only [eval_opBVal] at hv
      simpa [Vector.getElem_map] using congrArg (fun v : Word (ZMod p) => v[i]) hv
    · have hv := congrArg (fun r : Inputs (ZMod p) => r.op_c_val) h_input

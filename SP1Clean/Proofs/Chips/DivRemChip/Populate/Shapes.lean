...
lemma flags_cases {f : Vector (ZMod p) 8}
  ...
  · exact Or.inl ⟨eq_one_of_val hf0 h, eq_zero_of_val (by omega), eq_zero_of_val (by omega),
      eq_zero_of_val (by omega), eq_zero_of_val (by omega), eq_zero_of_val (by omega),
      eq_zero_of_val (by omega), eq_zero_of_val (by omega)⟩
  · exact Or.inr <| Or.inl ⟨eq_zero_of_val (by omega), eq_one_of_val hf1 h,
      eq_zero_of_val (by omega), eq_zero_of_val (by omega), eq_zero_of_val (by omega),
      eq_zero_of_val (by omega), eq_zero_of_val (by omega), eq_zero_of_val (by omega)⟩
  · exact Or.inr <| Or.inr <| Or.inl ⟨eq_zero_of_val (by omega), eq_zero_of_val (by omega),
      eq_one_of_val hf2 h, eq_zero_of_val (by omega), eq_zero_of_val (by omega),
      eq_zero_of_val (by omega), eq_zero_of_val (by omega), eq_zero_of_val (by omega)⟩
  · exact Or.inr <| Or.inr <| Or.inr <| Or.inl ⟨eq_zero_of_val (by omega),
      eq_zero_of_val (by omega), eq_zero_of_val (by omega), eq_one_of_val hf3 h,
      eq_zero_of_val (by omega), eq_zero_of_val (by omega), eq_zero_of_val (by omega),
      eq_zero_of_val (by omega)⟩
  · exact Or.inr <| Or.inr <| Or.inr <| Or.inr <| Or.inl ⟨eq_zero_of_val (by omega),
      eq_zero_of_val (by omega), eq_zero_of_val (by omega), eq_zero_of_val (by omega),
      eq_one_of_val hf4 h, eq_zero_of_val (by omega), eq_zero_of_val (by omega),
      eq_zero_of_val (by omega)⟩
  · exact Or.inr <| Or.inr <| Or.inr <| Or.inr <| Or.inr <| Or.inl ⟨eq_zero_of_val (by omega),
      eq_zero_of_val (by omega), eq_zero_of_val (by omega), eq_zero_of_val (by omega),
      eq_zero_of_val (by omega), eq_one_of_val hf5 h, eq_zero_of_val (by omega),
      eq_zero_of_val (by omega)⟩
  · exact Or.inr <| Or.inr <| Or.inr <| Or.inr <| Or.inr <| Or.inr <| Or.inl
      ⟨eq_zero_of_val (by omega), eq_zero_of_val (by omega), eq_zero_of_val (by omega),
       eq_zero_of_val (by omega), eq_zero_of_val (by omega), eq_zero_of_val (by omega),
       eq_one_of_val hf6 h, eq_zero_of_val (by omega)⟩
...
private lemma wordOfBits_setWidth32_hi (x : BitVec 64) :
  ...
  · simp only [wordOfBits, wordOfNat, Vector.getElem_mk, List.getElem_toArray,
    ...
    have hz : x.toNat % 2 ^ 32 / 2 ^ 32 % 2 ^ 16 = 0 := by omega
    rw [hz, Nat.cast_zero]
  · simp only [wordOfBits, wordOfNat, Vector.getElem_mk, List.getElem_toArray,
      List.getElem_cons_zero, List.getElem_cons_succ, ht]
    have hz : x.toNat % 2 ^ 32 / 2 ^ 48 % 2 ^ 16 = 0 := by omega
    rw [hz, Nat.cast_zero]
...
private lemma signExtend32_limbs (x : BitVec 32) :
  ...
  have hmsb := BitVec.msb_eq_decide x
  by_cases hb : x.msb = true
  · have hge : 2 ^ (32 - 1) ≤ x.toNat := by
      rw [hb] at hmsb
      exact of_decide_eq_true hmsb.symm
    have h2 : (x.signExtend 64).toNat / 2 ^ 32 % 2 ^ 16
        = (x.signExtend 64).toNat / 2 ^ 16 % 2 ^ 16 / 32768 * 65535 := by
      rw [hse, if_pos hb]
      omega
    have h3 : (x.signExtend 64).toNat / 2 ^ 48 % 2 ^ 16
        = (x.signExtend 64).toNat / 2 ^ 16 % 2 ^ 16 / 32768 * 65535 := by
      rw [hse, if_pos hb]
      omega
    exact wordOfNat_limbs23_eq_msb_fill h2 h3
  · have hlt : ¬2 ^ (32 - 1) ≤ x.toNat := by
      rw [Bool.not_eq_true] at hb
      rw [hb] at hmsb
      exact of_decide_eq_false hmsb.symm
    have h2 : (x.signExtend 64).toNat / 2 ^ 32 % 2 ^ 16
        = (x.signExtend 64).toNat / 2 ^ 16 % 2 ^ 16 / 32768 * 65535 := by
      rw [hse, if_neg hb]
      omega
...
lemma quotComp_limb0 (B C : Word (ZMod p)) (f : Vector (ZMod p) 8) :
  ...
  split_ifs
  · exact wordOfBits_setWidth32_limb0 _
  · rfl
...
lemma quotComp_limb1 (B C : Word (ZMod p)) (f : Vector (ZMod p) 8) :
  ...
  unfold populateQuotComp quotCompBits populateQuotient
  split_ifs
  · exact wordOfBits_setWidth32_limb1 _
  · rfl
...
lemma quotComp_hi_unsignedW (B C : Word (ZMod p)) {f : Vector (ZMod p) 8}
  ...
  unfold populateQuotComp quotCompBits
  rw [if_pos hW]
...
lemma remComp_hi_unsignedW (B C : Word (ZMod p)) {f : Vector (ZMod p) 8}
  ...
  unfold populateRemComp remCompBits
  rw [if_pos hW]
  exact wordOfBits_setWidth32_hi _
...
lemma quotComp_hi_64 (B C : Word (ZMod p)) {f : Vector (ZMod p) 8}
  ...
  have hcomp : quotCompBits B C f = quotBits B C f := by
    simp [quotCompBits, h6, h7]
  unfold populateQuotComp populateQuotient
  rw [hcomp]
  exact ⟨rfl, rfl⟩
...
lemma remComp_hi_64 (B C : Word (ZMod p)) {f : Vector (ZMod p) 8}
  ...
  have hcomp : remCompBits B C f = remBits B C f := by
    simp [remCompBits, h6, h7]
  unfold populateRemComp populateRemainder
  rw [hcomp]
  exact ⟨rfl, rfl⟩

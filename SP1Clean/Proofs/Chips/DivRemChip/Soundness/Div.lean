...
/-! # `DivRemChip` — `div` conjunct soundness (split for parallel compilation)

Signed-64-bit DIV conjunct, proved as a standalone `GeneralFormalCircuit.Soundness` over a
single-conjunct local `Spec`. The `?_tail` (`Operations.Requirements`) is proved alongside;
`Formal.lean` reuses it from here. -/

namespace SP1Clean.DivRemChip.SoundDiv
...
/-- The `div` conjunct of `DivRemChip.Spec`, as a standalone single-conjunct spec. -/
...
theorem soundness : GeneralFormalCircuit.Soundness (ZMod p) main Assumptions Spec := by
  ...
  spec_proof_start
  -- `op_b_val`/`op_c_val` are now the raw register reads (adapter projections); `hbU`/`hcU` are their `isU64`,
  -- used by the shared `?_tail` `msb` range-checks. The `?_spec` arithmetic aliases the witnessed **operand**
  -- columns `b`/`c` (i₀+16/i₀+20) *locally* under these names and shadows `hbU`/`hcU` with the operand `isU64`.
  ...
  · intro hr
    -- alias the witnessed operand columns `b`/`c` under the old names (local to `?_spec`, so the shared
    -- `?_tail` is unaffected); the body's arithmetic uses these and the operand `isU64` shadows below.
    ...
    intro hflag
    have hflag : env.get i₀ = 1 := hflag
    have hdd : (env.get i₀).val = 1 := by rw [hflag]; exact ZMod.val_one p
    ...
    simp only [circuit_norm] at e48 e49 e59 e69 e184 e194 e204 e214
    have hgateD : env.get (i₀ + 1) + env.get (i₀ + 3) + env.get i₀ + env.get (i₀ + 2) = 1 := by
      rw [hz_divu, hz_remu, hflag, hz_rem]; ring
    have hgateDR : env.get (i₀ + 1) + env.get i₀ + env.get (i₀ + 4) + env.get (i₀ + 6) = 1 := by
      rw [hz_divu, hflag, hz_divw, hz_divuw]; ring
    -- For DIV (`e2 = 0`, all word flags 0) the operand columns equal the raw reads (E20/E22/E29/E41), giving
    -- the operand `isU64` (shadowing `hbU`/`hcU` = read `isU64`) and the read-lift that closes the read-based
    -- `Spec`.
    ...
    have hdiv_id : Word.toBitVec64 (Vector.map (Expression.eval env)
      ...
      · -- DIVZERO (`c = 0`): `quotient = -1`, `remainder = b` ⟹ `div_rem_divzero`.
        ...
        have hrb : Word.toBitVec64 (#v[env.get (B + 7 + 8 + 8 + 11 + 11 + 11 + 4 + 4),
          ...
          rw [q0, q1, q2, q3]
        exact (div_rem_divzero hc0bv hqm1 hrb).1
      · by_cases hovf : env.get B = 1
        · -- OVERFLOW (`is_overflow = 1`): `b = i64::MIN`, `c = -1`, `quotient = b`, `remainder = 0`.
          ...
          have hpair := overflow_of_iseqword
            ...
            (by
              ...
              have hE10 : env.get i₀ + env.get (i₀ + 2) + env.get (i₀ + 4) + env.get (i₀ + 5) = 1 := by
                rw [hflag, hz_rem, hz_divw, hz_remw]; ring
          ...
          exact (div_rem_overflow hb_im hc_m1 hq_eq hr_eq).1
        · -- normal: signed Euclidean assembly (c≠0, no overflow). `b/c/quotient_comp/remainder_comp`
          ...
          have hE10 : env.get i₀ + env.get (i₀ + 2) + env.get (i₀ + 4) + env.get (i₀ + 5) = 1 := by
            rw [hflag, hz_rem, hz_divw, hz_remw]; ring
          have hihm1 : env.get i₀ + env.get (i₀ + 2) = 1 := by rw [hflag, hz_rem]; ring
          ...
          -- de-gate the four upper glue equalities (the 64-bit flag sum is `1` on a DIV row), then
          ...
          rw [hflag, hz_divu, hz_rem, hz_remu] at h_ctq4' h_ctq5' h_ctq6' h_ctq7'
          ...
          have hsgn := sign_conditions (quotient := Word.toBitVec64 (Vector.map (Expression.eval env)
            ...
            (by rw [hid]; ring) hlt
          exact (assemble_signed_normal hc0bv hid hlt hsgn.1 hsgn.2).1
    -- route `cols.a = quotient = quotient_comp` (E184 + E48/E59), as in DIVU stage 4.
    ...
    have hgoal : Word.toBitVec64 (Vector.map (Expression.eval env)
        (Vector.mapRange 4 fun i => var { index := i₀ + 8 + 4 + i }) : Word (ZMod p))
        = RV64.div (Word.toBitVec64 input_op_c_val) (Word.toBitVec64 input_op_b_val) := by
      rw [haqc]; exact hdiv_id
    rw [hbeq, hceq] at hgoal
    exact hgoal

end SP1Clean.DivRemChip.SoundDiv

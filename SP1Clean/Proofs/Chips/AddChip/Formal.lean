...
theorem soundness : GeneralFormalCircuit.Soundness (ZMod p) main Assumptions Spec := by
  ...
  -- G1: the CPUState sub-`Spec`'s two clock byte bounds are what discharge the *push* side of the
  -- memory channel's new `MemoryMsg.ClkBound` guarantee — for `RTypeReader`'s two read-back pushes
  -- (`clk_low + 3` / `+ 2`) and `RegisterWrite`'s op_a write push (`clk_low + 4`). The offsets are
  -- left to unification, so this line never names the destructured state columns.
  ...
  have h_addspec := h_add ⟨fun hr => ⟨(h_trio hr).2.1, (h_trio hr).2.2.1⟩, h_bin⟩
...
def circuit : GeneralFormalCircuit (ZMod p) Inputs Columns where
  ...
  -- W11: expose the State-bus `[pulledIf is_real cur, pushedIf is_real next]` pair (the gated State-bus
  -- interactions, descended from the composed `CPUState` subcircuit) as chip-owned interactions (the
  -- Clean `VmTables` re-base that motivated the shape was investigated and deferred — roadmap W11).

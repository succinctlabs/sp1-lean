...
/-- The RV64 shift-count extraction `extractLsb 5 0` is reduction mod 64. Shared by the three
`RV64.srl`/`RV64.sra` `.toNat` bridges below. -/
private lemma extractLsb_5_0_toNat (c : BitVec 64) :
    (BitVec.extractLsb 5 0 c).toNat = c.toNat % 64 := by
...
lemma sra_toNat_false (c b : BitVec 64) (h_msb : b.msb = false) :
  ...
  rw [BitVec.sshiftRight_eq', BitVec.toNat_sshiftRight_of_msb_false h_msb,
      Nat.shiftRight_eq_div_pow, extractLsb_5_0_toNat]
...
/-- The low-6-bit shift amount of a `Word` depends only on its bottom limb. Shared by the three
`RV64.srl`/`RV64.sra` goal-shape conversions; every `2 ^ N` is masked to a concrete literal so the
kernel never deep-recurses on a symbolic power. -/
private lemma toBitVec64_toNat_mod64 {rs2 : Word (ZMod p)} (h_rs2U : Word.isU64 rs2) :
    (Word.toBitVec64 rs2).toNat % 64 = rs2[0].val % 64 := by
...
lemma srl_div_to_bitvec (W rs1 rs2 : Word (ZMod p)) (h_rs2U : Word.isU64 rs2)
  ...
  rw [srl_toNat, toBitVec64_toNat_mod64 h_rs2U]
  exact hdiv
...
lemma sra_div_to_bitvec_false (W rs1 rs2 : Word (ZMod p)) (h_rs2U : Word.isU64 rs2)
  ...
  apply BitVec.eq_of_toNat_eq
  rw [sra_toNat_false _ _ h_msb, toBitVec64_toNat_mod64 h_rs2U]
  exact hdiv
...
lemma sra_div_to_bitvec_true (W rs1 rs2 : Word (ZMod p)) (h_rs2U : Word.isU64 rs2)
    ...
    Word.toBitVec64 W = RV64.sra (Word.toBitVec64 rs2) (Word.toBitVec64 rs1) := by
  apply BitVec.eq_of_toNat_eq
  rw [sra_toNat_true _ _ h_msb, toBitVec64_toNat_mod64 h_rs2U]
  exact hsra
...
omit [Fact (Nat.Prime p)] in
/-- The low 32 bits of a `Word`'s `toBitVec64` are its bottom two limbs, little-endian.
Shared by the three W-variant goal-shape conversions below. -/
private lemma low32_toNat {rs1 : Word (ZMod p)} (h_rs1U : Word.isU64 rs1) :
    (BitVec.extractLsb' 0 32 (Word.toBitVec64 rs1)).toNat = rs1[0].val + rs1[1].val * 2 ^ 16 := by
  obtain ⟨_, _, _, _⟩ := Word.lt_cases_of_isU64 h_rs1U
  rw [BitVec.extractLsb'_toNat, Word.toBitVec64_toNat h_rs1U, Word.toNat_def,
      Nat.shiftRight_zero]
  omega

omit [Fact (Nat.Prime p)] in
/-- The 5-bit W-variant shift amount reduces to the bottom limb mod 32.
Shared by the two SRAW arms. -/
private lemma shamt5_toNat {rs2 : Word (ZMod p)} (h_rs2U : Word.isU64 rs2) :
    (BitVec.extractLsb' 0 5 (Word.toBitVec64 rs2)).toNat = rs2[0].val % 32 := by
  obtain ⟨_, _, _, _⟩ := Word.lt_cases_of_isU64 h_rs2U
...
lemma srlw_div_to_bitvec (Wd rs1 rs2 : Word (ZMod p))
  ...
  have hX : (BitVec.extractLsb' 0 32 (Word.toBitVec64 rs1) >>>
    ...
    rw [BitVec.ushiftRight_eq', BitVec.toNat_ushiftRight, Nat.shiftRight_eq_div_pow,
        low32_toNat h_rs1U, hshamt]; exact hdiv.symm
...
lemma sraw_div_to_bitvec_false (Wd rs1 rs2 : Word (ZMod p))
  ...
  have hX : (BitVec.sshiftRight' (BitVec.extractLsb' 0 32 (Word.toBitVec64 rs1))
    ...
    rw [BitVec.sshiftRight_eq', BitVec.toNat_sshiftRight_of_msb_false h_msb,
        Nat.shiftRight_eq_div_pow, low32_toNat h_rs1U, shamt5_toNat h_rs2U]; exact hdiv.symm
...
lemma cbsum_val_lt_64 {cb0 cb1 cb2 cb3 cb4 cb5 : ZMod p}
  ...
  have e0 := bool_val_le b_cb0; have e1 := bool_val_le b_cb1; have e2 := bool_val_le b_cb2
  have e3 := bool_val_le b_cb3; have e4 := bool_val_le b_cb4; have e5 := bool_val_le b_cb5
  omega

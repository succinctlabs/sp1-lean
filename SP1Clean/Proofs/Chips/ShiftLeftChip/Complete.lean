...
the `ALUTypeReader.Spec` immediate conjuncts inline:

    (is_real - 1) * imm_c = 0,   (is_real - imm_c = 0 ∨ is_real - imm_c = 1),
    imm_c * (op_c_memory.prev_value[i] - op_c[i]) = 0   (four)

On register rows the selector is zero; on immediate `SLLI`/`SLLIW` rows the builder's
`aluTypeOpCCols` copies the committed `op_c` word into `prev_value`, exactly as the four pins ask.
The shared two-form reader lemmas discharge both cases, so this completeness path admits the same
register/immediate split as the chip's soundness and Sail bridge. -/
...
/--
**A well-formed `ShiftLeft` trace event builds a row the honest prover can complete, at the hint the
same event builds.** Every conjunct of `ShiftLeftChip.ProverAssumptions` follows from
...
theorem proverAssumptions_of_event {e : ALUTypeEvent} (h : e.WellFormed) (hop : e.IsShiftLeft)
  ...
  · rw [show (e.toShiftLeftInputs (p := p)).is_real = 1 from rfl, sub_self, zero_mul]
  · rcases aluTypeReaderCols_imm_c_bool (p := p) h.immC_bool with h0 | h1
    · exact Or.inr (by
        rw [show (e.toShiftLeftInputs (p := p)).is_real = 1 from rfl,
          ALUTypeEvent.toShiftLeftInputs_adapter, h0, sub_zero])
    · exact Or.inl (by
        rw [show (e.toShiftLeftInputs (p := p)).is_real = 1 from rfl,
          ALUTypeEvent.toShiftLeftInputs_adapter, h1, sub_self])
...
theorem traceTable_constraints (events : List ALUTypeEvent) (padding : ℕ)
    (data : ProverData (ZMod p)) (h : ∀ e ∈ events, e.WellFormed ∧ e.IsShiftLeft) :

...
Legacy transitional Clean circuit generated from `LtOperationSigned::eval`: the `Inputs` struct,
the `main` do-block, and the
Clean-derived `ElaboratedCircuit` instance + `@[circuit_norm]` rfl-lemmas. Generated by
`update_extracted.py` from the `sp1-constraint-compiler` (`--operation LtOperationSigned --format

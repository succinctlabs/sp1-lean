...
SP1's `AddOperation::eval` as a Clean `Circuit`: the `Inputs` struct (the `eval` params
...
`update_extracted.py` from the `sp1-constraint-compiler` (`--operation AddOperation --format

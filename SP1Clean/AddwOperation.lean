...
iff lemma `AddwOperation.allHold_constraints_iff_poly` exposes both: a
...
/-- Pilot Spec, mirroring `AddwOperation.allHold_constraints_iff_poly` RHS
...
theorem iff_sp1 (a b : Word (ZMod p)) (cols : _root_.AddwOperation (ZMod p)) :
    (_root_.AddwOperation.constraints a b cols 1).allHold_poly ↔
      Spec a b cols :=
  _root_.AddwOperation.allHold_constraints_iff_poly a b cols

...
Documentation (no proofs). Contrasts the Clean-native, semantically-specified Add built here
(`SP1Clean.AddOperation` + `AddChip`) with the structural-`FormalAssertion` + `SailBridge` style, to
show why a semantic spec yields a smaller, self-contained, axiom-clean proof.
...
* **Structural** — a chip `FormalSpec` is a conjunction of *structural* sub-Specs
  (`CPUState.Gated.Spec ∧ RTypeReader.Gated.Spec ∧ op_a_0 = 0 ∧ …`) plus the
  semantic clause; the operation completeness routes through `AddOperation.iff_sp1_full`.
  The structural reader Specs are bus-send restatements.
...
* **Structural** — borrows ALL arithmetic from `_root_.AddOperation.*` and ALL RISC-V
...
* **Native, Add-specific:** `Operations/AddOperation.lean` (306) + `Chips/AddChip.lean` (88) = ~394
  ...
  `Foundations.lean` (110, shared by every chip).
* **Structural:** `Operations/AddOperation.lean` (385) + `Chips/ALU/AddChip/*` (586) =
  ~971 lines that **still borrow** the arithmetic (~256 lines more from SP1Operations'
  AddOperation) and Sail (from SP1Chips' `correct_add`).
* The native figures count the operation and chip proper; the parts they omit (the borrowed
  arithmetic and borrowed Sail correctness) are exactly the ones the structural version
...
**public Clean `main` + Lean 4.28 + mathlib v4.28**.
...
completeness run through. This is operation-level; the chip-level anchor to the full generated
`Add.constraints` (which also composes reader/CPU constraints) builds on the native readers.
...
axiom-clean in one unified `lake build SP1Clean`, smaller than the structural borrowing version and
free of any `SailBridge`/extracted-constraint coupling. -/

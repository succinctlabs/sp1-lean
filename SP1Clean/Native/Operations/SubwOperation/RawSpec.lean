...
/-! # `SubwOperation` — the arithmetic core (`RawSpec` + the sign-extension keystones)

The borrow-form carry-bool + low-limb-range `RawSpec` (two-limb subtract chain, carry init `1`,
complement `65535 - b[i]`), the forward `subwSemantics_of_carries`, and the converse
`carries_of_subwSemantics` — the borrow-form analog of `AddwOperation`. Mirrors
`sp1/crates/core/machine/src/operations/subw.rs`. -/
...
theorem subwSemantics_of_carries {a b : Word (ZMod p)} {cols : Extracted.SubwOperation (ZMod p)}
  ...
  set c1 : ZMod p := (a[1] + 65535 - b[1] - cols.value[1] + c0) * (65536 : ZMod p)⁻¹ with hc1_def
  have e0 : a[0] + (65535 - b[0]) + (1 : ZMod p) = cols.value[0] + c0 * 65536 := by
    rw [hc0_def]; linear_combination -1 * (a[0] + 65535 - b[0] - cols.value[0] + 1) * h65inv
  have e1 : a[1] + (65535 - b[1]) + c0 = cols.value[1] + c1 * 65536 := by
    rw [hc1_def]; linear_combination -1 * (a[1] + 65535 - b[1] - cols.value[1] + c0) * h65inv
  ...
  have hX : (BitVec.setWidth 32 (Word.toBitVec64 a - Word.toBitVec64 b)).toNat
    ...
    rw [BitVec.toNat_setWidth, BitVec.toNat_sub, Word.toBitVec64_toNat ha,
...
theorem carries_of_subwSemantics {a b : Word (ZMod p)} {cols : Extracted.SubwOperation (ZMod p)}
  ...
  -- the operand low-32 borrow sum (mod 2^32)
  have hsum32 : (Word.toBitVec64 a - Word.toBitVec64 b).toNat % 2 ^ 32
      = ((a[0].val + 65535 - b[0].val + 1) + (a[1].val + 65535 - b[1].val) * 2 ^ 16) % 2 ^ 32 := by
    rw [BitVec.toNat_sub, Word.toBitVec64_toNat ha, Word.toBitVec64_toNat hb,
  ...
  have hbvN : v0.val + v1.val * 2 ^ 16 + M * 2 ^ 32 + M * 2 ^ 48
      = ((a[0].val + 65535 - b[0].val + 1) + (a[1].val + 65535 - b[1].val) * 2 ^ 16) % 2 ^ 32
    ...
    rw [h, Nat.mod_eq_of_lt (a := (Word.toBitVec64 a - Word.toBitVec64 b).toNat % 2 ^ 32)
      ...
      hsum32]
  -- the borrow chain (from the low-32 relation), lifted to `ZMod`
  have hcarries : v0.val + v1.val * 2 ^ 16
    ...
    have e0n : a[0].val + 65535 + 1 = v0.val + q0 * 65536 + b[0].val := by rw [hq0_def]; omega
    have e1n : a[1].val + 65535 + q0 = v1.val + q1 * 65536 + b[1].val := by rw [hq1_def]; omega
    have h65inv : (65536 : ZMod p) * (65536 : ZMod p)⁻¹ = 1 := mul_inv_cancel₀ val_65536_ne_zero
    have hc0_lift : (a[0] : ZMod p) + 65535 + 1 = v0 + (q0 : ZMod p) * 65536 + b[0] := by
    ...
    have hc1_lift : (a[1] : ZMod p) + 65535 + (q0 : ZMod p) = v1 + (q1 : ZMod p) * 65536 + b[1] := by
      have hcast := congrArg (Nat.cast : ℕ → ZMod p) e1n
      push_cast at hcast
      rw [ZMod.natCast_zmod_val, ZMod.natCast_zmod_val, ZMod.natCast_zmod_val] at hcast
      rw [hv1def]; linear_combination hcast
    have hc0_eq : (a[0] + 65535 - b[0] - v0 + 1) * (65536 : ZMod p)⁻¹ = (q0 : ZMod p) := by
      have h : a[0] + 65535 - b[0] - v0 + 1 = (q0 : ZMod p) * 65536 := by linear_combination hc0_lift
      rw [h, mul_assoc, h65inv, mul_one]
    have hc1_eq : (a[1] + 65535 - b[1] - v1 + (a[0] + 65535 - b[0] - v0 + 1) * (65536 : ZMod p)⁻¹)
        * (65536 : ZMod p)⁻¹ = (q1 : ZMod p) := by
      rw [hc0_eq]
      have h : a[1] + 65535 - b[1] - v1 + (q0 : ZMod p) = (q1 : ZMod p) * 65536 := by
  ...
  · rw [if_pos hcond] at hbvN
    have hmsbge : 2 ^ 31 ≤ ((a[0].val + 65535 - b[0].val + 1) + (a[1].val + 65535 - b[1].val) * 2 ^ 16) % 2 ^ 32 := by
      have h := hcond
      rw [BitVec.msb_eq_decide, decide_eq_true_eq, BitVec.toNat_setWidth, hsum32] at h
      exact h
    have hlow : v0.val + v1.val * 2 ^ 16
        = ((a[0].val + 65535 - b[0].val + 1) + (a[1].val + 65535 - b[1].val) * 2 ^ 16) % 2 ^ 32 := by omega
  ...
  · rw [if_neg hcond] at hbvN
    have hmsblt : ((a[0].val + 65535 - b[0].val + 1) + (a[1].val + 65535 - b[1].val) * 2 ^ 16) % 2 ^ 32 < 2 ^ 31 := by
      have h := hcond
      rw [BitVec.msb_eq_decide, decide_eq_true_eq, BitVec.toNat_setWidth, hsum32] at h
      omega
    have hlow : v0.val + v1.val * 2 ^ 16

...
the difference; `isEqualWord_of_raw` derives the semantic equality indicator. The hand-maintained native
circuit (`Inputs`/`main`/`elaborated`) lives in `Defs`; the `populate` witness in

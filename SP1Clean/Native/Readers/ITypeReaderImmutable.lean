...
theorem completeness :
  ...
  obtain ⟨⟨z0, z1, z2, z3⟩, hbin, hrac_a, hrac_b, _hdec, hisu⟩ := h_spec
  ...
  · -- The program pull now supplies `ProgTruth` (not `RowSpec`) — `decodedInROM` is not provable
    ...
    -- the goal message is exactly `progMsgOf input` (all other fields already value-form), so `exact
    -- h_prog` closes it opaquely (no `ProgTruth`/`decodedInROM` unfold).
    intro ht

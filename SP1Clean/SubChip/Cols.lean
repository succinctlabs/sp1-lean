...
Mirrors `SP1Clean.Add.FormalSpec` swapping `AddOp.Spec` → `SubOp.Spec` and
opcode `0` → `2`. The pure BitVec `RV64.sub` semantic is the trailing
conjunct (conditional on `is_real = 1`); the monadic Sail equivalence to
`_root_.Sub.spec_sub` is recovered externally via
`sail_correct_of_formalSpec` in `SailBridge.lean`. -/
def FormalSpec (cols : SubCols (ZMod p)) : Prop :=
  ...
  SP1Clean.RTypeReader.rtypeReaderSpec clk_low 2 cols.state.pc

...
`Vector.map (eval e) v` form. This one lemma bridges the two; it recurs across the Mul / Lt /
Bitwise / ShiftLeft / ShiftRight chip `Formal` proofs and the DivRem completeness `Driver`, so it

...
/-! # Chip-level faithfulness anchor — SP1's whole `StoreHalf` chip constraint list ↔ combined spec -/
...
theorem storehalfcols_constraints_faithful (cols : Extracted.StoreHalfColumns (ZMod p))
    (h_real : cols.is_real = 1) :
    (List.Forall (· = 0) (Extracted.StoreHalfColumns.asserts cols) ∧
      List.Forall Interaction.toProp (Extracted.StoreHalfColumns.interactions cols)) ↔
      ((SP1Clean.AddressOperation.RawSpec
            ...
            #v[cols.adapter.op_c_imm[0], cols.adapter.op_c_imm[1], cols.adapter.op_c_imm[2],
              cols.adapter.op_c_imm[3]] 0 cols.offset_bit[0] cols.offset_bit[1]
            { addr_operation :=
        ...
        ∧ ((cols.memory_access.access_timestamp.compare_low
              ...
              cols.store_value[0] - (cols.memory_access.prev_value[0]
                + (cols.adapter.op_a_memory.prev_value[0] - cols.memory_access.prev_value[0])
                  * (1 - cols.offset_bit[0]) * (1 - cols.offset_bit[1])) = 0 ∧
              cols.store_value[1] - (cols.memory_access.prev_value[1]
                + (cols.adapter.op_a_memory.prev_value[0] - cols.memory_access.prev_value[1])
                  * cols.offset_bit[0] * (1 - cols.offset_bit[1])) = 0 ∧
              cols.store_value[2] - (cols.memory_access.prev_value[2]
                + (cols.adapter.op_a_memory.prev_value[0] - cols.memory_access.prev_value[2])
                  * (1 - cols.offset_bit[0]) * cols.offset_bit[1]) = 0 ∧
              cols.store_value[3] - (cols.memory_access.prev_value[3]
                + (cols.adapter.op_a_memory.prev_value[0] - cols.memory_access.prev_value[3])
                  * cols.offset_bit[0] * cols.offset_bit[1]) = 0) ∧
  ...
  simp only [Extracted.StoreHalfColumns.asserts, Extracted.StoreHalfColumns.interactions]

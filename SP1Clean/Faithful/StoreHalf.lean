...
def storeHalfChipRowCodec :
  ...
  assignment cols data := {
    ...
    rowInput_eq := by
      exact rowInput_inputFirstRow (StoreHalfChip.circuit (p := p))
...
private theorem storeHalfStateInteractionsEq
    ...
    ((StoreHalfChip.main input).operations offset).interactionsWith
        stateChannel.toRaw =
      (storeHalfStateInteractions input).map ChannelInteraction.toRaw := by
  exact (StoreHalfChip.circuit (p := p)).interactionsWith_eq_of_mem_exposedChannels
    input offset
    ⟨stateChannel.toRaw,
...
private theorem storeHalfProgramInteractionsEq
    ...
    ((StoreHalfChip.main input).operations offset).interactionsWith
      ...
      (storeHalfProgramInteractions input).map ChannelInteraction.toRaw := by
  exact (StoreHalfChip.circuit (p := p)).interactionsWith_eq_of_mem_exposedChannels
    input offset
    ⟨programChannel.toRaw,

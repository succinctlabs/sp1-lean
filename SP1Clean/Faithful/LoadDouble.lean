import SP1Clean.Faithful.ChipOracle
import SP1Clean.Extracted.ChipOracle.LoadDouble
import SP1Clean.Proofs.Chips.LoadDoubleChip.Formal

/-!
# Exact whole-chip faithfulness for SP1 `LoadDouble`

This file relates the native Clean `LoadDoubleChip` row to the complete generated row-level oracle
for pinned SP1 v6.3.1. The `ChipFaithful` theorem at the bottom covers every `assertZero` expression and the
entire interaction multiset, including inactive rows.
-/

namespace SP1Clean.Faithful

open SP1Clean
open SP1Clean.Extracted
open scoped SP1Clean.ConstraintCoe

variable {p : ℕ} [Fact p.Prime] [Fact (2 ^ 17 < p)]

/-- Rebuild the shared standalone `AddressOperation` block as the byte-identical struct embedded in
the generated LoadDouble oracle namespace. -/
def loadDoubleOracleAddressOperation {F : Type} (cols : Extracted.AddressOperation F) :
    Extracted.LoadDoubleOracle.AddressOperation F :=
  { addr_operation := { value := cols.addr_operation.value }
    top_two_limb_inv := cols.top_two_limb_inv }

/-- Inverse of `loadDoubleOracleAddressOperation`. -/
def loadDoubleNativeAddressOperation {F : Type}
    (cols : Extracted.LoadDoubleOracle.AddressOperation F) :
    Extracted.AddressOperation F :=
  { addr_operation := { value := cols.addr_operation.value }
    top_two_limb_inv := cols.top_two_limb_inv }

/-- Whole-chip row reconfiguration. The reader and memory-access blocks are already the canonical
generated substrate; the address block is copied into Rust's chip-private operation row. This is not
an operation-level faithfulness claim. -/
def loadDoubleChipReconfigure {F : Type} (cols : LoadDoubleChip.Columns F) :
    Extracted.LoadDoubleOracle.LoadDoubleColumns F :=
  { state := cols.state
    adapter := cols.adapter
    address_operation := loadDoubleOracleAddressOperation cols.address_operation
    memory_access := cols.memory_access
    is_real := cols.is_real }

/-- Inverse whole-row map used to reconstruct the native proof row from an arbitrary Rust row. -/
def loadDoubleChipDeconfigure {F : Type} (cols : Extracted.LoadDoubleOracle.LoadDoubleColumns F) :
    LoadDoubleChip.Columns F :=
  { state := cols.state
    adapter := cols.adapter
    address_operation := loadDoubleNativeAddressOperation cols.address_operation
    memory_access := cols.memory_access
    is_real := cols.is_real }

/-- SP1 Rust's complete LoadDouble-chip oracle, viewed from the native Lean row. -/
def loadDoubleChipOracle {F : Type} [FiniteField F] [CoeHead F ℕ] :
    ChipOracle F LoadDoubleChip.Columns Extracted.LoadDoubleOracle.LoadDoubleColumns where
  reconfigure := loadDoubleChipReconfigure
  deconfigure := loadDoubleChipDeconfigure
  reconfigure_deconfigure := by intro cols; cases cols; rfl
  deconfigure_reconfigure := by intro cols; cases cols; rfl
  assertZeros := Extracted.LoadDoubleOracle.LoadDoubleColumns.asserts
  interactions := Extracted.LoadDoubleOracle.LoadDoubleColumns.interactions

/- Namespace bridges between the LoadDouble oracle's embedded chip-private helper copies and the
canonical standalone generated modules. The two bodies are rendered from the same compiler output,
so each bridge is a definitional unfolding, not a mathematical claim. They let the address-op
lemmas below stay stated once against the standalone modules (also consumed by the other load and
store chips). -/

private theorem loadDoubleOracle_addrAdd_asserts_eq {F : Type} [Field F] [CoeHead F ℕ]
    (a b : Word F) (value : Vector F 3) (is_real : F) :
    Extracted.LoadDoubleOracle.AddrAddOperation.asserts a b ⟨value⟩ is_real =
      Extracted.AddrAddOperation.asserts a b ⟨value⟩ is_real := by
  rw [Extracted.LoadDoubleOracle.AddrAddOperation.asserts,
    Extracted.AddrAddOperation.asserts]

private theorem loadDoubleOracle_addrAdd_interactions_eq {F : Type} [Field F] [CoeHead F ℕ]
    (a b : Word F) (value : Vector F 3) (is_real : F) :
    Extracted.LoadDoubleOracle.AddrAddOperation.interactions a b ⟨value⟩ is_real =
      Extracted.AddrAddOperation.interactions a b ⟨value⟩ is_real := by
  rw [Extracted.LoadDoubleOracle.AddrAddOperation.interactions,
    Extracted.AddrAddOperation.interactions]

private theorem loadDoubleOracle_address_asserts_eq {F : Type} [Field F] [CoeHead F ℕ]
    (b cc : Word F) (offset_bit0 offset_bit1 offset_bit2 is_real : F)
    (value : Vector F 3) (top_two_limb_inv : F) :
    Extracted.LoadDoubleOracle.AddressOperation.asserts b cc offset_bit0 offset_bit1 offset_bit2
        is_real ⟨⟨value⟩, top_two_limb_inv⟩ =
      Extracted.AddressOperation.asserts b cc offset_bit0 offset_bit1 offset_bit2
        is_real ⟨⟨value⟩, top_two_limb_inv⟩ := by
  rw [Extracted.LoadDoubleOracle.AddressOperation.asserts,
    Extracted.AddressOperation.asserts]
  simp only [loadDoubleOracle_addrAdd_asserts_eq]

private theorem loadDoubleOracle_address_interactions_eq {F : Type} [Field F] [CoeHead F ℕ]
    (b cc : Word F) (offset_bit0 offset_bit1 offset_bit2 is_real : F)
    (value : Vector F 3) (top_two_limb_inv : F) :
    Extracted.LoadDoubleOracle.AddressOperation.interactions b cc offset_bit0 offset_bit1
        offset_bit2 is_real ⟨⟨value⟩, top_two_limb_inv⟩ =
      Extracted.AddressOperation.interactions b cc offset_bit0 offset_bit1 offset_bit2
        is_real ⟨⟨value⟩, top_two_limb_inv⟩ := by
  rw [Extracted.LoadDoubleOracle.AddressOperation.interactions,
    Extracted.AddressOperation.interactions]
  simp only [loadDoubleOracle_addrAdd_interactions_eq]

def loadDoubleChipInput {F : Type}
    (cols : LoadDoubleChip.Columns F) : LoadDoubleChip.Inputs F :=
  { is_real := cols.is_real
    state := cols.state
    adapter := cols.adapter
    memory_access := cols.memory_access }

def loadDoubleChipLocals {F : Type}
    (cols : LoadDoubleChip.Columns F) : Vector F 4 :=
  #v[cols.address_operation.addr_operation.value[0],
    cols.address_operation.addr_operation.value[1],
    cols.address_operation.addr_operation.value[2],
    cols.address_operation.top_two_limb_inv]

def loadDoubleChipPhysicalRow {F : Type}
    (cols : LoadDoubleChip.Columns F) : Array F :=
  inputFirstRow (loadDoubleChipInput cols) (loadDoubleChipLocals cols)

def loadDoubleChipColumnsOfInput {F : Type}
    (input : LoadDoubleChip.Inputs F) (locals : Vector F 4) :
    LoadDoubleChip.Columns F :=
  ⟨input.state, input.adapter,
    ⟨⟨#v[locals[0], locals[1], locals[2]]⟩, locals[3]⟩,
    input.memory_access, input.is_real⟩

private theorem loadDoubleVec3Eta {F : Type} (value : Vector F 3) :
    #v[value[0], value[1], value[2]] = value := by
  apply Vector.ext
  intro i hi
  interval_cases i <;> rfl

private theorem loadDoubleVec4Eta {F : Type} (value : Vector F 4) :
    #v[value[0], value[1], value[2], value[3]] = value := by
  apply Vector.ext
  intro i hi
  interval_cases i <;> rfl

private theorem loadDoubleEvalVec4Components
    {F : Type} [FiniteField F] (env : Environment F)
    (value : Vector (Expression F) 4) :
    #v[(Eval.eval env value)[0], (Eval.eval env value)[1],
      (Eval.eval env value)[2], (Eval.eval env value)[3]] =
      #v[Expression.eval env value[0], Expression.eval env value[1],
        Expression.eval env value[2], Expression.eval env value[3]] := by
  apply Vector.ext
  intro i hi
  interval_cases i
  · exact (ProvableType.getElem_eval_fields env value 0 (by decide)).symm
  · exact (ProvableType.getElem_eval_fields env value 1 (by decide)).symm
  · exact (ProvableType.getElem_eval_fields env value 2 (by decide)).symm
  · exact (ProvableType.getElem_eval_fields env value 3 (by decide)).symm

private theorem loadDoubleAddressEta {F : Type}
    (cols : Extracted.AddressOperation F) :
    ({ addr_operation := { value := cols.addr_operation.value }
       top_two_limb_inv := cols.top_two_limb_inv } :
      Extracted.AddressOperation F) = cols := by
  cases cols with
  | mk addr top =>
    cases addr
    rfl

private theorem loadDoubleCpuEta {F : Type}
    (cols : Extracted.CPUState F) :
    ({ clk_high := cols.clk_high
       clk_16_24 := cols.clk_16_24
       clk_0_16 := cols.clk_0_16
       pc := cols.pc } : Extracted.CPUState F) = cols := by
  cases cols
  rfl

private theorem loadDoubleITypeEta {F : Type}
    (cols : Extracted.ITypeReader F) :
    ({ op_a := cols.op_a
       op_a_memory :=
         { prev_value := cols.op_a_memory.prev_value
           access_timestamp := cols.op_a_memory.access_timestamp }
       op_a_0 := cols.op_a_0
       op_b := cols.op_b
       op_b_memory :=
         { prev_value := cols.op_b_memory.prev_value
           access_timestamp := cols.op_b_memory.access_timestamp }
       op_c_imm := cols.op_c_imm } : Extracted.ITypeReader F) = cols := by
  cases cols with
  | mk opA opAMem opA0 opB opBMem opC =>
    cases opAMem
    cases opBMem
    rfl

theorem loadDoubleChipColumnsOfInput_roundtrip {F : Type}
    (cols : LoadDoubleChip.Columns F) :
    loadDoubleChipColumnsOfInput
        (loadDoubleChipInput cols) (loadDoubleChipLocals cols) = cols := by
  cases cols
  simp [loadDoubleChipColumnsOfInput, loadDoubleChipInput,
    loadDoubleChipLocals, loadDoubleVec3Eta]

@[circuit_norm] private theorem loadDoubleEvalAddress
    {F : Type} [FiniteField F] (env : Environment F)
    (cols : Extracted.AddressOperation (Expression F)) :
    Eval.eval env cols =
      ({ addr_operation := Eval.eval env cols.addr_operation
         top_two_limb_inv := Eval.eval env cols.top_two_limb_inv } :
        Extracted.AddressOperation F) := by
  rw [ProvableStruct.eval_eq_eval]
  rfl

@[circuit_norm] private theorem loadDoubleEvalAddrAdd
    {F : Type} [FiniteField F] (env : Environment F)
    (cols : Extracted.AddrAddOperation (Expression F)) :
    Eval.eval env cols =
      ({ value := Eval.eval env cols.value } :
        Extracted.AddrAddOperation F) := by
  rw [ProvableStruct.eval_eq_eval]
  rfl

@[circuit_norm] private theorem loadDoubleEvalAddrAddInput
    {F : Type} [FiniteField F] (env : Environment F)
    (input : AddrAddOperation.Inputs (Expression F)) :
    Eval.eval env input =
      ({ a := Eval.eval env input.a
         b := Eval.eval env input.b
         cols := Eval.eval env input.cols
         is_real := Eval.eval env input.is_real } :
        AddrAddOperation.Inputs F) := by
  rw [ProvableStruct.eval_eq_eval]
  rfl

@[circuit_norm] private theorem loadDoubleEvalAddressInput
    {F : Type} [FiniteField F] (env : Environment F)
    (input : AddressOperation.Inputs (Expression F)) :
    Eval.eval env input =
      ({ b := Eval.eval env input.b
         cc := Eval.eval env input.cc
         offset_bit0 := Eval.eval env input.offset_bit0
         offset_bit1 := Eval.eval env input.offset_bit1
         offset_bit2 := Eval.eval env input.offset_bit2
         is_real := Eval.eval env input.is_real } :
        AddressOperation.Inputs F) := by
  rw [ProvableStruct.eval_eq_eval]
  rfl

@[circuit_norm] private theorem loadDoubleEvalMemoryInput
    {F : Type} [FiniteField F] (env : Environment F)
    (input : Readers.MemoryAccess.Inputs (Expression F)) :
    Eval.eval env input =
      ({ mem := Eval.eval env input.mem
         clk_high := Eval.eval env input.clk_high
         clk_low := Eval.eval env input.clk_low
         addr0 := Eval.eval env input.addr0
         addr1 := Eval.eval env input.addr1
         addr2 := Eval.eval env input.addr2
         new_value := Eval.eval env input.new_value
         is_real := Eval.eval env input.is_real } :
        Readers.MemoryAccess.Inputs F) := by
  rw [ProvableStruct.eval_eq_eval]
  rfl

@[circuit_norm] private theorem loadDoubleEvalMemoryTimestamp
    {F : Type} [FiniteField F] (env : Environment F)
    (cols : Extracted.MemoryAccessTimestamp (Expression F)) :
    Eval.eval env cols =
      ({ prev_high := Eval.eval env cols.prev_high
         prev_low := Eval.eval env cols.prev_low
         compare_low := Eval.eval env cols.compare_low
         diff_low_limb := Eval.eval env cols.diff_low_limb
         diff_high_limb := Eval.eval env cols.diff_high_limb } :
        Extracted.MemoryAccessTimestamp F) := by
  rw [ProvableStruct.eval_eq_eval]
  rfl

@[circuit_norm] private theorem loadDoubleEvalMemoryCols
    {F : Type} [FiniteField F] (env : Environment F)
    (cols : Extracted.MemoryAccessCols (Expression F)) :
    Eval.eval env cols =
      ({ prev_value := Eval.eval env cols.prev_value
         access_timestamp := Eval.eval env cols.access_timestamp } :
        Extracted.MemoryAccessCols F) := by
  rw [ProvableStruct.eval_eq_eval]
  rfl

theorem evalLoadDoubleDirectOutput
    (input : LoadDoubleChip.Inputs (ZMod p))
    (locals : Vector (ZMod p) 4) (data : ProverData (ZMod p)) :
    ProvableType.eval (Environment.fromArray (inputFirstRow input locals) data)
        ((LoadDoubleChip.elaborated (p := p)).output
          (varFromOffset LoadDoubleChip.Inputs 0)
          (size LoadDoubleChip.Inputs)) =
      loadDoubleChipColumnsOfInput input locals := by
  rw [LoadDoubleChip.directOutput_eq]
  rw [← CircuitType.eval_expression, LoadDoubleChip.eval_columns]
  unfold loadDoubleChipColumnsOfInput
  rw [LoadDoubleChip.Columns.mk.injEq]
  dsimp only
  have hinputEval := eval_inputFirstRow input locals data
  rw [LoadDoubleChip.eval_inputs, LoadDoubleChip.Inputs.mk.injEq] at hinputEval
  refine ⟨hinputEval.2.1, hinputEval.2.2.1, ?_, hinputEval.2.2.2, hinputEval.1⟩
  rw [loadDoubleEvalAddress, Extracted.AddressOperation.mk.injEq]
  constructor
  · rw [loadDoubleEvalAddrAdd, Extracted.AddrAddOperation.mk.injEq]
    apply Vector.ext
    intro i hi
    change
      (Eval.eval (Environment.fromArray (inputFirstRow input locals) data)
        (Vector.mapRange 3 fun i =>
          var { index := size LoadDoubleChip.Inputs + i }))[i] =
        #v[locals[0], locals[1], locals[2]][i]
    rw [← ProvableType.getElem_eval_fields
      (Environment.fromArray (inputFirstRow input locals) data)
      (Vector.mapRange 3 fun i =>
        var { index := size LoadDoubleChip.Inputs + i }) i hi]
    rw [Vector.getElem_mapRange]
    interval_cases i
    · exact eval_local_inputFirstRow input locals data 0 (by decide)
    · exact eval_local_inputFirstRow input locals data 1 (by decide)
    · exact eval_local_inputFirstRow input locals data 2 (by decide)
  · exact (ProvableType.eval_field
      (Environment.fromArray (inputFirstRow input locals) data)
      (var { index := size LoadDoubleChip.Inputs + 3 })).trans
        (eval_local_inputFirstRow input locals data 3 (by decide))

def loadDoubleChipRowCodec :
    ChipRowCodec LoadDoubleChip.Inputs LoadDoubleChip.Columns
      (LoadDoubleChip.circuit (p := p)) where
  assignment cols data := {
    row := loadDoubleChipPhysicalRow cols
    input := loadDoubleChipInput cols
    width_eq := by
      rw [loadDoubleChipPhysicalRow, inputFirstRow_size,
        Air.Flat.Component.width, LoadDoubleChip.circuit_size_eq]
    rowInput_eq := by
      exact rowInput_inputFirstRow (LoadDoubleChip.circuit (p := p))
        (loadDoubleChipInput cols) (loadDoubleChipLocals cols) data
    rowOutput_eq := by
      change ProvableType.eval _ ((LoadDoubleChip.main _).output _) = _
      rw [LoadDoubleChip.elaborated.output_eq]
      rw [Air.Flat.Component.rowInputVar_mk,
        Air.Flat.Component.rowOffset_mk]
      exact (evalLoadDoubleDirectOutput (p := p)
        (loadDoubleChipInput cols) (loadDoubleChipLocals cols) data).trans
          (loadDoubleChipColumnsOfInput_roundtrip cols) }

theorem loadDoubleChipLookupsEmpty :
    (⟨LoadDoubleChip.circuit (p := p)⟩ :
      Air.Flat.Component (ZMod p)).operations.lookups = [] := by
  rw [Air.Flat.Component.lookups_eq, Air.Flat.Component.rowOperations_mk,
    LoadDoubleChip.circuit_main_eq]
  simp [LoadDoubleChip.main, Readers.CPUState.circuit,
    Readers.CPUState.main, AddressOperation.circuit, AddressOperation.main,
    AddrAddOperation.circuit, AddrAddOperation.main,
    Readers.MemoryAccess.circuit, Readers.MemoryAccess.main,
    Readers.ITypeReader.circuit, Readers.ITypeReader.main,
    Readers.RegisterWrite.circuit, Readers.RegisterWrite.main,
    Readers.RegisterAccessCols.circuit, Readers.RegisterAccessCols.main,
    Readers.RegisterAccessTimestamp.circuit,
    Readers.RegisterAccessTimestamp.main, Gadgets.Equality.main, circuit_norm]

private def loadDoubleAddressCols (offset : ℕ) :
    Extracted.AddressOperation (Expression (ZMod p)) :=
  ⟨⟨Vector.mapRange 3 fun i => var { index := offset + i }⟩,
    var { index := offset + 3 }⟩

omit [Fact (2 ^ 17 < p)] in
private theorem loadDoubleEvalAddressCols
    (env : Environment (ZMod p)) (offset : ℕ) :
    Eval.eval env (loadDoubleAddressCols (p := p) offset) =
      ({ addr_operation :=
          { value := #v[env.get offset, env.get (offset + 1),
            env.get (offset + 2)] }
         top_two_limb_inv := env.get (offset + 3) } :
        Extracted.AddressOperation (ZMod p)) := by
  simp only [loadDoubleAddressCols]
  rw [loadDoubleEvalAddress, Extracted.AddressOperation.mk.injEq]
  constructor
  · rw [loadDoubleEvalAddrAdd, Extracted.AddrAddOperation.mk.injEq]
    apply Vector.ext
    intro i hi
    rw [← ProvableType.getElem_eval_fields env
      (Vector.mapRange 3 fun i => var { index := offset + i }) i hi]
    rw [Vector.getElem_mapRange]
    interval_cases i <;> rfl
  · simp only [ProvableType.eval_field, Expression.eval]

private def loadDoubleAddressInput
    (input : Var LoadDoubleChip.Inputs (ZMod p)) :
    Var AddressOperation.Inputs (ZMod p) :=
  ⟨input.op_b_val, input.op_c_imm, 0, 0, 0, input.is_real⟩

private def loadDoubleAddressValue
    (input : Var LoadDoubleChip.Inputs (ZMod p)) (offset : ℕ) :
    Vector (Expression (ZMod p)) 3 :=
  AddressOperation.alignedValue
    (loadDoubleAddressInput input) (loadDoubleAddressCols offset)

private def loadDoubleCpuInput
    (input : Var LoadDoubleChip.Inputs (ZMod p)) :
    Var Readers.CPUState.Inputs (ZMod p) :=
  ⟨input.state,
    #v[input.state.pc[0] + 4, input.state.pc[1], input.state.pc[2]],
    8, input.is_real⟩

private def loadDoubleMemoryInput
    (input : Var LoadDoubleChip.Inputs (ZMod p)) (offset : ℕ) :
    Var Readers.MemoryAccess.Inputs (ZMod p) :=
  ⟨input.memory_access, input.state.clk_high,
    input.state.clk_0_16 + input.state.clk_16_24 * 65536,
    (loadDoubleAddressValue input offset)[0],
    (loadDoubleAddressValue input offset)[1],
    (loadDoubleAddressValue input offset)[2],
    input.memory_access.prev_value, input.is_real⟩

private def loadDoubleITypeInput
    (input : Var LoadDoubleChip.Inputs (ZMod p)) :
    Var Readers.ITypeReader.Inputs (ZMod p) :=
  ⟨input.adapter, input.is_real, input.is_real, input.state.clk_high,
    input.state.clk_0_16 + input.state.clk_16_24 * 65536,
    input.state.pc, 35,
    input.memory_access.prev_value[0],
    input.memory_access.prev_value[1],
    input.memory_access.prev_value[2],
    input.memory_access.prev_value[3]⟩

private def loadDoubleRegisterWriteInput
    (input : Var LoadDoubleChip.Inputs (ZMod p)) :
    Var Readers.RegisterWrite.Inputs (ZMod p) :=
  ⟨input.state.clk_high,
    input.state.clk_0_16 + input.state.clk_16_24 * 65536 + 4,
    input.adapter.op_a, input.memory_access.prev_value, input.is_real⟩

private theorem loadDoubleNativeConstraintsDecompose
    (env : Environment (ZMod p))
    (input : Var LoadDoubleChip.Inputs (ZMod p)) (offset : ℕ) :
    List.Forall (· = 0)
        (nativeAssertZeros env
          ((LoadDoubleChip.main input).operations offset)) ↔
      List.Forall (· = 0)
          (nativeAssertZeros env
            ((Readers.CPUState.main
              (loadDoubleCpuInput input)).operations offset)) ∧
        List.Forall (· = 0)
          (nativeAssertZeros env
            ((AddressOperation.main
              (loadDoubleAddressInput input)).operations offset)) ∧
        List.Forall (· = 0)
          (nativeAssertZeros env
            ((Readers.MemoryAccess.main
              (loadDoubleMemoryInput input offset)).operations (offset + 4))) ∧
        List.Forall (· = 0)
          (nativeAssertZeros env
            ((Readers.ITypeReader.main
              (loadDoubleITypeInput input)).operations (offset + 4))) ∧
        List.Forall (· = 0)
          (nativeAssertZeros env
            ((Readers.RegisterWrite.main
              (loadDoubleRegisterWriteInput input)).operations (offset + 4))) ∧
        List.Forall (· = 0)
          (nativeAssertZeros env
            ((Gadgets.Equality.main (M := field)
              (input.adapter.op_a_0, 0)).operations (offset + 4))) ∧
        Expression.eval env
          (input.is_real * (input.is_real - 1)) = 0 := by
  simp only [nativeAssertZeros, LoadDoubleChip.main,
    loadDoubleCpuInput, loadDoubleAddressInput, loadDoubleAddressCols,
    loadDoubleAddressValue, loadDoubleMemoryInput, loadDoubleITypeInput,
    loadDoubleRegisterWriteInput,
    Readers.CPUState.circuit, AddressOperation.circuit,
    Readers.MemoryAccess.circuit, Readers.ITypeReader.circuit,
    Readers.RegisterWrite.circuit,
    circuit_norm, List.map_append, List.forall_append]

omit [Fact (2 ^ 17 < p)] in
set_option maxHeartbeats 1000000 in
private theorem loadDoubleAddrAddAssertions
    (env : Environment (ZMod p))
    (input : Var AddrAddOperation.Inputs (ZMod p)) (offset : ℕ) :
    List.Forall (· = 0)
        (Extracted.AddrAddOperation.asserts
          #v[Expression.eval env input.a[0],
            Expression.eval env input.a[1],
            Expression.eval env input.a[2],
            Expression.eval env input.a[3]]
          #v[Expression.eval env input.b[0],
            Expression.eval env input.b[1],
            Expression.eval env input.b[2],
            Expression.eval env input.b[3]]
          ⟨#v[Expression.eval env input.cols.value[0],
            Expression.eval env input.cols.value[1],
            Expression.eval env input.cols.value[2]]⟩
          (Expression.eval env input.is_real)) ↔
      List.Forall (· = 0)
        (nativeAssertZeros env
          ((AddrAddOperation.main input).operations offset)) := by
  rw [Extracted.AddrAddOperation.asserts]
  simp only [nativeAssertZeros, AddrAddOperation.main, circuit_norm,
    List.map_append, List.map_cons, List.Forall]
  repeat' rw [CanonicalReader.equalityAssertionList]
  simp only [eval_sub, Expression.eval, add_zero, sub_zero]
  simp only [← ProvableStruct.eval_eq_eval,
    loadDoubleEvalAddrAddInput, loadDoubleEvalAddrAdd,
    ProvableType.eval_field, ProvableType.getElem_eval_fields]
  simp only [List.singleton_append, List.Forall]

set_option maxHeartbeats 2000000 in
private theorem loadDoubleAddressAssertions
    (env : Environment (ZMod p))
    (input : Var AddressOperation.Inputs (ZMod p))
    (offset : ℕ) :
    List.Forall (· = 0)
        (Extracted.AddressOperation.asserts
          #v[Expression.eval env input.b[0],
            Expression.eval env input.b[1],
            Expression.eval env input.b[2],
            Expression.eval env input.b[3]]
          #v[Expression.eval env input.cc[0],
            Expression.eval env input.cc[1],
            Expression.eval env input.cc[2],
            Expression.eval env input.cc[3]]
          (Expression.eval env input.offset_bit0)
          (Expression.eval env input.offset_bit1)
          (Expression.eval env input.offset_bit2)
          (Expression.eval env input.is_real)
          ⟨⟨#v[env.get offset, env.get (offset + 1),
            env.get (offset + 2)]⟩, env.get (offset + 3)⟩) ↔
      List.Forall (· = 0)
        (nativeAssertZeros env
          ((AddressOperation.main input).operations offset)) := by
  let cols := loadDoubleAddressCols (p := p) offset
  let addrAddInput : Var AddrAddOperation.Inputs (ZMod p) :=
    ⟨input.b, input.cc, cols.addr_operation, input.is_real⟩
  have hAddrAdd := loadDoubleAddrAddAssertions (p := p) env addrAddInput
    (offset + 3)
  rw [Extracted.AddressOperation.asserts]
  simp only [nativeAssertZeros, AddressOperation.main,
    AddrAddOperation.circuit, circuit_norm, List.map_append,
    List.forall_append, List.Forall]
  simp only [addrAddInput, cols, loadDoubleAddressCols] at hAddrAdd
  simp only [← ProvableStruct.eval_eq_eval,
    loadDoubleEvalAddressInput, ProvableType.eval_field,
    ProvableType.getElem_eval_fields,
    Vector.getElem_mapRange, Expression.eval, Nat.add_zero] at hAddrAdd ⊢
  rw [hAddrAdd]
  rw [CanonicalReader.equalityAssertionList]
  rw [CanonicalReader.equalityAssertionList]
  rw [CanonicalReader.equalityAssertionList]
  rw [CanonicalReader.equalityAssertionList]
  simp only [eval_sub, Expression.eval]
  simp only [List.singleton_append, List.Forall, sub_zero]
  tauto

private def loadDoubleMemoryAssertionValues
    (env : Environment (ZMod p))
    (input : Var Readers.MemoryAccess.Inputs (ZMod p)) :
    List (ZMod p) :=
  let ts := input.mem.access_timestamp
  [ Expression.eval env (input.is_real * (input.is_real - 1)),
    Expression.eval env
      (input.is_real * (ts.compare_low * (ts.compare_low - 1))) -
        Expression.eval env 0,
    Expression.eval env
      (input.is_real * (ts.compare_low * (input.clk_high - ts.prev_high))) -
        Expression.eval env 0,
    Expression.eval env
      (input.is_real *
        ((ts.compare_low * (input.clk_low + 1) +
            (1 - ts.compare_low) * input.clk_high -
            (ts.compare_low * ts.prev_low +
              (1 - ts.compare_low) * ts.prev_high) - 1) -
          (ts.diff_low_limb + ts.diff_high_limb * 65536))) -
        Expression.eval env 0 ]

set_option maxHeartbeats 1000000 in
omit [Fact (2 ^ 17 < p)] in
private theorem loadDoubleMemoryAssertionList
    (env : Environment (ZMod p))
    (input : Var Readers.MemoryAccess.Inputs (ZMod p))
    (offset : ℕ) :
    List.map (Expression.eval env)
        (Operations.constraints
          ((Readers.MemoryAccess.main input).operations offset)) =
      loadDoubleMemoryAssertionValues env input := by
  simp only [Readers.MemoryAccess.main, circuit_norm]
  simp only [List.map_append]
  repeat' rw [CanonicalReader.equalityAssertionList]
  simp only [loadDoubleMemoryAssertionValues,
    List.singleton_append]
  rw [← ProvableStruct.eval_eq_eval, loadDoubleEvalMemoryInput]
  simp only [ProvableType.eval_field, eval_sub, Expression.eval]

private def loadDoubleChipRustColumns
    (env : Environment (ZMod p))
    (input : Var LoadDoubleChip.Inputs (ZMod p)) (offset : ℕ) :
    LoadDoubleChip.Columns (ZMod p) :=
  { state := Eval.eval env input.state
    adapter := Eval.eval env input.adapter
    address_operation := Eval.eval env (loadDoubleAddressCols (p := p) offset)
    memory_access := Eval.eval env input.memory_access
    is_real := Expression.eval env input.is_real }

private def loadDoubleNativeCpuMeaning
    (env : Environment (ZMod p))
    (input : Var LoadDoubleChip.Inputs (ZMod p)) (offset : ℕ) : Prop :=
  List.Forall (· = 0)
    (nativeAssertZeros env
      ((Readers.CPUState.main
        (loadDoubleCpuInput input)).operations offset))

private def loadDoubleNativeAddressMeaning
    (env : Environment (ZMod p))
    (input : Var LoadDoubleChip.Inputs (ZMod p)) (offset : ℕ) : Prop :=
  List.Forall (· = 0)
    (nativeAssertZeros env
      ((AddressOperation.main
        (loadDoubleAddressInput input)).operations offset))

private def loadDoubleNativeMemoryMeaning
    (env : Environment (ZMod p))
    (input : Var LoadDoubleChip.Inputs (ZMod p)) (offset : ℕ) : Prop :=
  List.Forall (· = 0)
    (loadDoubleMemoryAssertionValues env
      (loadDoubleMemoryInput input offset))

private def loadDoubleNativeITypeMeaning
    (env : Environment (ZMod p))
    (input : Var LoadDoubleChip.Inputs (ZMod p)) (offset : ℕ) : Prop :=
  List.Forall (· = 0)
    (nativeAssertZeros env
      ((Readers.ITypeReader.main
        (loadDoubleITypeInput input)).operations (offset + 4)))

private def loadDoubleNativeMeaning
    (env : Environment (ZMod p))
    (input : Var LoadDoubleChip.Inputs (ZMod p)) (offset : ℕ) : Prop :=
  loadDoubleNativeCpuMeaning env input offset ∧
    loadDoubleNativeAddressMeaning env input offset ∧
    loadDoubleNativeMemoryMeaning env input offset ∧
    loadDoubleNativeITypeMeaning env input offset ∧
    Expression.eval env input.adapter.op_a_0 = 0 ∧
    Expression.eval env (input.is_real * (input.is_real - 1)) = 0

private def loadDoubleRustAddressMeaning
    (env : Environment (ZMod p))
    (input : Var LoadDoubleChip.Inputs (ZMod p)) (offset : ℕ) : Prop :=
  let cols := loadDoubleChipRustColumns env input offset
  List.Forall (fun x : ZMod p => x = 0)
    (Extracted.AddressOperation.asserts
      #v[cols.adapter.op_b_memory.prev_value[0],
        cols.adapter.op_b_memory.prev_value[1],
        cols.adapter.op_b_memory.prev_value[2],
        cols.adapter.op_b_memory.prev_value[3]]
      #v[cols.adapter.op_c_imm[0], cols.adapter.op_c_imm[1],
        cols.adapter.op_c_imm[2], cols.adapter.op_c_imm[3]]
      (0 : ZMod p) (0 : ZMod p) (0 : ZMod p)
      cols.is_real
      { addr_operation :=
          { value := cols.address_operation.addr_operation.value }
        top_two_limb_inv := cols.address_operation.top_two_limb_inv })

private def loadDoubleRustCpuMeaning
    (env : Environment (ZMod p))
    (input : Var LoadDoubleChip.Inputs (ZMod p)) (offset : ℕ) : Prop :=
  let cols := loadDoubleChipRustColumns env input offset
  List.Forall (· = 0)
    (Extracted.CPUState.asserts
      { clk_high := cols.state.clk_high
        clk_16_24 := cols.state.clk_16_24
        clk_0_16 := cols.state.clk_0_16
        pc := cols.state.pc }
      #v[cols.state.pc[0] + 4, cols.state.pc[1], cols.state.pc[2]]
      8 cols.is_real)

private def loadDoubleRustITypeMeaning
    (env : Environment (ZMod p))
    (input : Var LoadDoubleChip.Inputs (ZMod p)) (offset : ℕ) : Prop :=
  let cols := loadDoubleChipRustColumns env input offset
  let clkLow := cols.state.clk_0_16 + cols.state.clk_16_24 * 65536
  List.Forall (· = 0)
    (Extracted.ITypeReader.asserts cols.state.clk_high clkLow
      cols.state.pc 35 cols.memory_access.prev_value
      { op_a := cols.adapter.op_a
        op_a_memory :=
          { prev_value := cols.adapter.op_a_memory.prev_value
            access_timestamp := cols.adapter.op_a_memory.access_timestamp }
        op_a_0 := cols.adapter.op_a_0
        op_b := cols.adapter.op_b
        op_b_memory :=
          { prev_value := cols.adapter.op_b_memory.prev_value
            access_timestamp := cols.adapter.op_b_memory.access_timestamp }
        op_c_imm := cols.adapter.op_c_imm }
      cols.is_real cols.is_real)

private def loadDoubleRustTailMeaning
    (env : Environment (ZMod p))
    (input : Var LoadDoubleChip.Inputs (ZMod p)) (offset : ℕ) : Prop :=
  let cols := loadDoubleChipRustColumns env input offset
  let ts := cols.memory_access.access_timestamp
  let clkLow := cols.state.clk_0_16 + cols.state.clk_16_24 * 65536
  List.Forall (· = 0)
    ([ cols.is_real * (cols.is_real - 1),
      cols.is_real * (cols.is_real - 1),
      cols.is_real * (ts.compare_low * (ts.compare_low - 1)),
      cols.is_real * (ts.compare_low *
        (cols.state.clk_high - ts.prev_high)),
      cols.is_real *
        ((ts.compare_low * (clkLow + 1) +
            (1 - ts.compare_low) * cols.state.clk_high -
            (ts.compare_low * ts.prev_low +
              (1 - ts.compare_low) * ts.prev_high) - 1) -
          (ts.diff_low_limb + ts.diff_high_limb * 65536)) ]
      ++ [cols.adapter.op_a_0])

private def loadDoubleRustMeaning
    (env : Environment (ZMod p))
    (input : Var LoadDoubleChip.Inputs (ZMod p)) (offset : ℕ) : Prop :=
  loadDoubleRustAddressMeaning env input offset ∧
    loadDoubleRustCpuMeaning env input offset ∧
    loadDoubleRustITypeMeaning env input offset ∧
    loadDoubleRustTailMeaning env input offset

private theorem loadDoubleNativeAssertionsDecompose
    (env : Environment (ZMod p))
    (input : Var LoadDoubleChip.Inputs (ZMod p)) (offset : ℕ) :
    List.Forall (· = 0)
        (nativeAssertZeros env
          ((LoadDoubleChip.main input).operations offset)) ↔
      loadDoubleNativeMeaning env input offset := by
  rw [loadDoubleNativeConstraintsDecompose]
  rw [show nativeAssertZeros env
      ((Readers.MemoryAccess.main
        (loadDoubleMemoryInput input offset)).operations (offset + 4)) =
        loadDoubleMemoryAssertionValues env
          (loadDoubleMemoryInput input offset) by
    exact loadDoubleMemoryAssertionList env
      (loadDoubleMemoryInput input offset) (offset + 4)]
  rw [CanonicalReader.registerWriteAssertions]
  rw [CanonicalReader.equalityAssertions]
  simp only [Expression.eval, true_and]
  rfl

omit [Fact (2 ^ 17 < p)] in
private theorem loadDoubleRustAssertionsDecompose
    (env : Environment (ZMod p))
    (input : Var LoadDoubleChip.Inputs (ZMod p)) (offset : ℕ) :
    List.Forall (· = 0)
        (loadDoubleChipOracle.nativeAssertZeros
          (loadDoubleChipRustColumns env input offset)) ↔
      loadDoubleRustMeaning env input offset := by
  simp only [ChipOracle.nativeAssertZeros, loadDoubleChipOracle]
  rw [Extracted.LoadDoubleOracle.LoadDoubleColumns.asserts]
  dsimp only [loadDoubleChipReconfigure, loadDoubleOracleAddressOperation]
  simp only [loadDoubleOracle_address_asserts_eq]
  unfold loadDoubleRustMeaning loadDoubleRustAddressMeaning
    loadDoubleRustCpuMeaning loadDoubleRustITypeMeaning
    loadDoubleRustTailMeaning
  simp only [List.forall_append, List.forall_cons,
    loadDoubleVec3Eta, loadDoubleVec4Eta]
  tauto

private theorem loadDoubleAddressMeaningFaithful
    (env : Environment (ZMod p))
    (input : Var LoadDoubleChip.Inputs (ZMod p)) (offset : ℕ) :
    loadDoubleRustAddressMeaning env input offset ↔
      loadDoubleNativeAddressMeaning env input offset := by
  have hAddress := loadDoubleAddressAssertions (p := p) env
    (loadDoubleAddressInput input) offset
  unfold loadDoubleRustAddressMeaning loadDoubleNativeAddressMeaning
  dsimp only [loadDoubleChipRustColumns]
  rw [loadDoubleEvalAddressCols]
  have hb :
      #v[(Eval.eval env input.adapter).op_b_memory.prev_value[0],
        (Eval.eval env input.adapter).op_b_memory.prev_value[1],
        (Eval.eval env input.adapter).op_b_memory.prev_value[2],
        (Eval.eval env input.adapter).op_b_memory.prev_value[3]] =
        #v[Expression.eval env input.adapter.op_b_memory.prev_value[0],
          Expression.eval env input.adapter.op_b_memory.prev_value[1],
          Expression.eval env input.adapter.op_b_memory.prev_value[2],
          Expression.eval env input.adapter.op_b_memory.prev_value[3]] := by
    rw [Readers.ITypeReader.eval_cols]
    dsimp only
    rw [eval_registerAccessCols]
    dsimp only
    change
      #v[(Eval.eval env input.adapter.op_b_memory.prev_value)[0],
        (Eval.eval env input.adapter.op_b_memory.prev_value)[1],
        (Eval.eval env input.adapter.op_b_memory.prev_value)[2],
        (Eval.eval env input.adapter.op_b_memory.prev_value)[3]] = _
    exact loadDoubleEvalVec4Components env
      input.adapter.op_b_memory.prev_value
  have hc :
      #v[(Eval.eval env input.adapter).op_c_imm[0],
        (Eval.eval env input.adapter).op_c_imm[1],
        (Eval.eval env input.adapter).op_c_imm[2],
        (Eval.eval env input.adapter).op_c_imm[3]] =
        #v[Expression.eval env input.adapter.op_c_imm[0],
          Expression.eval env input.adapter.op_c_imm[1],
          Expression.eval env input.adapter.op_c_imm[2],
          Expression.eval env input.adapter.op_c_imm[3]] := by
    rw [Readers.ITypeReader.eval_cols]
    dsimp only
    change
      #v[(Eval.eval env input.adapter.op_c_imm)[0],
        (Eval.eval env input.adapter.op_c_imm)[1],
        (Eval.eval env input.adapter.op_c_imm)[2],
        (Eval.eval env input.adapter.op_c_imm)[3]] = _
    exact loadDoubleEvalVec4Components env input.adapter.op_c_imm
  rw [hb, hc]
  simp only [loadDoubleAddressInput] at hAddress
  exact hAddress

omit [Fact (2 ^ 17 < p)] in
private theorem loadDoubleCpuMeaningFaithful
    (env : Environment (ZMod p))
    (input : Var LoadDoubleChip.Inputs (ZMod p)) (offset : ℕ) :
    loadDoubleRustCpuMeaning env input offset ↔
      loadDoubleNativeCpuMeaning env input offset := by
  let cpu := loadDoubleCpuInput input
  have hCpu := CanonicalReader.cpuStateAssertions
    (p := p) env cpu offset
    (Eval.eval env input.state)
    #v[Expression.eval env (input.state.pc[0] + 4),
      Expression.eval env input.state.pc[1],
      Expression.eval env input.state.pc[2]]
    8 (Expression.eval env input.is_real) (by
      simp only [cpu, loadDoubleCpuInput,
        ProvableStruct.structEvalLiteralProc])
  have hNext :
      #v[(Eval.eval env input.state).pc[0] + 4,
        (Eval.eval env input.state).pc[1],
        (Eval.eval env input.state).pc[2]] =
        #v[Expression.eval env (input.state.pc[0] + 4),
          Expression.eval env input.state.pc[1],
          Expression.eval env input.state.pc[2]] := by
    rw [eval_cpuState]
    dsimp only
    apply Vector.ext
    intro i hi
    interval_cases i
    · simpa only [Vector.getElem_mk, List.getElem_toArray,
        List.getElem_cons_zero, eval_add, Expression.eval] using
        congrArg (· + 4)
          (ProvableType.getElem_eval_fields env input.state.pc 0
            (by decide)).symm
    · simpa only [Vector.getElem_mk, List.getElem_toArray,
        List.getElem_cons_succ, List.getElem_cons_zero] using
        (ProvableType.getElem_eval_fields env input.state.pc 1
          (by decide)).symm
    · simpa only [Vector.getElem_mk, List.getElem_toArray,
        List.getElem_cons_succ, List.getElem_cons_zero] using
        (ProvableType.getElem_eval_fields env input.state.pc 2
          (by decide)).symm
  unfold loadDoubleRustCpuMeaning loadDoubleNativeCpuMeaning
  dsimp only [loadDoubleChipRustColumns]
  rw [hNext]
  simp only [cpu, loadDoubleCpuInput] at hCpu
  exact hCpu

set_option maxHeartbeats 1000000 in
private theorem loadDoubleITypeMeaningFaithful
    (env : Environment (ZMod p))
    (input : Var LoadDoubleChip.Inputs (ZMod p)) (offset : ℕ) :
    loadDoubleRustITypeMeaning env input offset ↔
      loadDoubleNativeITypeMeaning env input offset := by
  let readerInput := loadDoubleITypeInput input
  have hIType := CanonicalReader.iTypeAssertionsExact
    (p := p) env readerInput (offset + 4)
    (Expression.eval env input.state.clk_high)
    (Expression.eval env
      (input.state.clk_0_16 + input.state.clk_16_24 * 65536))
    35 (Expression.eval env input.is_real)
    (Expression.eval env input.is_real)
    (Eval.eval env input.state.pc)
    (Eval.eval env input.memory_access.prev_value)
    (Eval.eval env input.adapter)
    (by
      simp only [readerInput, loadDoubleITypeInput,
        ProvableStruct.eval_eq_eval,
        ProvableStruct.structEvalLiteralProc])
    (by
      simp only [readerInput, loadDoubleITypeInput,
        ProvableStruct.eval_eq_eval,
        ProvableStruct.structEvalLiteralProc])
    (by
      simp only [readerInput, loadDoubleITypeInput]
      rw [Readers.ITypeReader.eval_cols]
      simp only [ProvableType.eval_field])
    (by
      simp only [readerInput, loadDoubleITypeInput]
      exact ProvableType.getElem_eval_fields env
        input.memory_access.prev_value 0 (by decide))
    (by
      simp only [readerInput, loadDoubleITypeInput]
      exact ProvableType.getElem_eval_fields env
        input.memory_access.prev_value 1 (by decide))
    (by
      simp only [readerInput, loadDoubleITypeInput]
      exact ProvableType.getElem_eval_fields env
        input.memory_access.prev_value 2 (by decide))
    (by
      simp only [readerInput, loadDoubleITypeInput]
      exact ProvableType.getElem_eval_fields env
        input.memory_access.prev_value 3 (by decide))
    rfl
  unfold loadDoubleRustITypeMeaning loadDoubleNativeITypeMeaning
  dsimp only [loadDoubleChipRustColumns]
  simp only [readerInput, loadDoubleITypeInput] at hIType
  simpa only [loadDoubleITypeInput, eval_cpuState, loadDoubleEvalMemoryCols,
    Readers.ITypeReader.eval_cols, ProvableType.eval_field,
    eval_add, eval_mul, Expression.eval] using hIType

omit [Fact (2 ^ 17 < p)] in
private theorem loadDoubleTailMeaningFaithful
    (env : Environment (ZMod p))
    (input : Var LoadDoubleChip.Inputs (ZMod p)) (offset : ℕ) :
    loadDoubleRustTailMeaning env input offset ↔
      loadDoubleNativeMemoryMeaning env input offset ∧
        Expression.eval env input.adapter.op_a_0 = 0 ∧
        Expression.eval env
          (input.is_real * (input.is_real - 1)) = 0 := by
  unfold loadDoubleRustTailMeaning loadDoubleNativeMemoryMeaning
  dsimp only [loadDoubleChipRustColumns]
  simp only [loadDoubleMemoryAssertionValues, loadDoubleMemoryInput,
    List.Forall, Readers.ITypeReader.eval_opA0,
    eval_cpuState, loadDoubleEvalMemoryCols,
    loadDoubleEvalMemoryTimestamp, ProvableType.eval_field,
    eval_sub, Expression.eval, sub_zero, List.forall_append]
  tauto

private theorem loadDoubleChipMeaningFaithful
    (env : Environment (ZMod p))
    (input : Var LoadDoubleChip.Inputs (ZMod p)) (offset : ℕ) :
    loadDoubleRustMeaning env input offset ↔
      loadDoubleNativeMeaning env input offset := by
  unfold loadDoubleRustMeaning loadDoubleNativeMeaning
  rw [loadDoubleAddressMeaningFaithful, loadDoubleCpuMeaningFaithful,
    loadDoubleITypeMeaningFaithful, loadDoubleTailMeaningFaithful]
  tauto

private theorem loadDoubleChipConstraintsFaithfulOutput
    (env : Environment (ZMod p))
    (input : Var LoadDoubleChip.Inputs (ZMod p)) (offset : ℕ) :
    List.Forall (· = 0)
        (loadDoubleChipOracle.nativeAssertZeros
          (loadDoubleChipRustColumns env input offset)) ↔
      List.Forall (· = 0)
        (nativeAssertZeros env
          ((LoadDoubleChip.main input).operations offset)) :=
  (loadDoubleRustAssertionsDecompose (p := p) env input offset).trans
    ((loadDoubleChipMeaningFaithful (p := p) env input offset).trans
      (loadDoubleNativeAssertionsDecompose (p := p) env input offset).symm)

theorem loadDoubleChipConstraintsFaithful
    (env : Environment (ZMod p))
    (input : Var LoadDoubleChip.Inputs (ZMod p)) (offset : ℕ)
    (cols : LoadDoubleChip.Columns (ZMod p))
    (hbind : BindsChipOutput LoadDoubleChip.main env input offset cols) :
    List.Forall (· = 0)
        (loadDoubleChipOracle.nativeAssertZeros cols) ↔
      List.Forall (· = 0)
        (nativeAssertZeros env
          ((LoadDoubleChip.main input).operations offset)) := by
  replace hbind := BindsChipOutput.ofElaborated
    (LoadDoubleChip.elaborated (p := p)) hbind
  rw [LoadDoubleChip.directOutput_eq] at hbind
  rw [← ProvableStruct.eval_eq_eval,
    LoadDoubleChip.eval_columns] at hbind
  simp only [ProvableType.eval_field] at hbind
  change loadDoubleChipRustColumns env input offset = cols at hbind
  rw [← hbind]
  exact loadDoubleChipConstraintsFaithfulOutput
    (p := p) env input offset

theorem loadDoubleChipConstraintsConstructive
    (rustCols : Extracted.LoadDoubleOracle.LoadDoubleColumns (ZMod p))
    (data : ProverData (ZMod p)) :
    let assignment := loadDoubleChipRowCodec.assignment
      (loadDoubleChipOracle.deconfigure rustCols) data
    List.Forall (· = 0)
        (loadDoubleChipOracle.assertZeros rustCols) ↔
      (⟨LoadDoubleChip.circuit (p := p)⟩ :
        Air.Flat.Component (ZMod p)).operations.ConstraintsHold
          assignment.environment := by
  dsimp only
  let cols := loadDoubleChipOracle.deconfigure rustCols
  let assignment := loadDoubleChipRowCodec.assignment cols data
  have hbind :
      BindsChipOutput LoadDoubleChip.main assignment.environment
        (⟨LoadDoubleChip.circuit (p := p)⟩ :
          Air.Flat.Component (ZMod p)).rowInputVar
        (⟨LoadDoubleChip.circuit (p := p)⟩ :
          Air.Flat.Component (ZMod p)).rowOffset cols := by
    have h := NativeRowAssignment.bindsOutput assignment
    rw [LoadDoubleChip.circuit_main_eq] at h
    exact h
  have hfaithful := loadDoubleChipConstraintsFaithful
    (p := p) assignment.environment
    (⟨LoadDoubleChip.circuit (p := p)⟩ :
      Air.Flat.Component (ZMod p)).rowInputVar
    (⟨LoadDoubleChip.circuit (p := p)⟩ :
      Air.Flat.Component (ZMod p)).rowOffset cols hbind
  have hassertions :
      List.Forall (· = 0)
          (loadDoubleChipOracle.assertZeros rustCols) ↔
        List.Forall (· = 0)
          (nativeAssertZeros assignment.environment
            (⟨LoadDoubleChip.circuit (p := p)⟩ :
              Air.Flat.Component (ZMod p)).rowOperations) := by
    simpa only [cols,
      ChipOracle.nativeAssertZeros_deconfigure,
      Air.Flat.Component.rowOperations_mk,
      Air.Flat.Component.rowInputVar_mk,
      Air.Flat.Component.rowOffset_mk,
      LoadDoubleChip.circuit_main_eq] using hfaithful
  exact hassertions.trans
    (constraintsHold_iff_nativeAssertZeros
      (LoadDoubleChip.circuit (p := p))
      assignment.environment loadDoubleChipLookupsEmpty).symm

open SP1Clean.Channels
  (stateChannel byteChannel memoryChannel programChannel)
open SP1Clean.InteractionRecovery

private def loadDoubleStateInteractions
    (input : Var LoadDoubleChip.Inputs (ZMod p)) :
    List (ChannelInteraction (stateChannel (p := p))) :=
  [ stateChannel.pulledIf input.is_real
      ⟨input.state.clk_high,
       input.state.clk_0_16 + input.state.clk_16_24 * 65536,
       input.state.pc[0], input.state.pc[1], input.state.pc[2]⟩,
    stateChannel.pushedIf input.is_real
      ⟨input.state.clk_high,
       input.state.clk_0_16 + input.state.clk_16_24 * 65536 + 8,
       input.state.pc[0] + 4, input.state.pc[1], input.state.pc[2]⟩ ]

private def loadDoubleProgramInteractions
    (input : Var LoadDoubleChip.Inputs (ZMod p)) :
    List (ChannelInteraction (programChannel (p := p))) :=
  [ programChannel.pulledIf input.is_real
      ⟨input.state.pc[0], input.state.pc[1], input.state.pc[2], 35,
       input.adapter.op_a, #v[input.adapter.op_b, 0, 0, 0],
       input.adapter.op_c_imm, input.adapter.op_a_0, 0, 1⟩ ]

private theorem loadDoubleStateInteractionsEq
    (input : Var LoadDoubleChip.Inputs (ZMod p)) (offset : ℕ) :
    ((LoadDoubleChip.main input).operations offset).interactionsWith
        stateChannel.toRaw =
      (loadDoubleStateInteractions input).map ChannelInteraction.toRaw := by
  exact (LoadDoubleChip.circuit (p := p)).interactionsWith_eq_of_mem_exposedChannels
    input offset
    ⟨stateChannel.toRaw,
      (loadDoubleStateInteractions input).map ChannelInteraction.toRaw⟩
    (by simp [LoadDoubleChip.circuit, loadDoubleStateInteractions, expose])

private theorem loadDoubleProgramInteractionsEq
    (input : Var LoadDoubleChip.Inputs (ZMod p)) (offset : ℕ) :
    ((LoadDoubleChip.main input).operations offset).interactionsWith
        programChannel.toRaw =
      (loadDoubleProgramInteractions input).map ChannelInteraction.toRaw := by
  exact (LoadDoubleChip.circuit (p := p)).interactionsWith_eq_of_mem_exposedChannels
    input offset
    ⟨programChannel.toRaw,
      (loadDoubleProgramInteractions input).map ChannelInteraction.toRaw⟩
    (by simp [LoadDoubleChip.circuit, loadDoubleProgramInteractions, expose])

private theorem loadDoubleStateInteractionsFaithful
    (env : Environment (ZMod p))
    (input : Var LoadDoubleChip.Inputs (ZMod p)) (offset : ℕ) :
    ((((loadDoubleStateInteractions input).map
        ChannelInteraction.toRaw).map
          (AbstractInteraction.toAccess env))) =
      (((Extracted.LoadDoubleOracle.LoadDoubleColumns.interactions
          (loadDoubleChipReconfigure
            (loadDoubleChipRustColumns env input offset))).map
            Extracted.Interaction.toAccess).filter
        (fun access => access.1 = InteractionKind.State)) := by
  have hStatePull :
      ∀ (gate : Expression (ZMod p))
        (msg : Channels.StateMsg (Expression (ZMod p))),
        AbstractInteraction.toAccess env
            (((stateChannel (p := p)).pulledIf gate msg).toRaw) =
          (InteractionKind.State, "SP1State",
            [(Expression.eval env msg.clk_high).val,
             (Expression.eval env msg.clk_low).val,
             (Expression.eval env msg.pc0).val,
             (Expression.eval env msg.pc1).val,
             (Expression.eval env msg.pc2).val],
            signedVal (Expression.eval env (-gate))) :=
    fun gate msg => toAccess_pullIf_state env gate msg
  have hStatePush :
      ∀ (mult : Expression (ZMod p))
        (msg : Channels.StateMsg (Expression (ZMod p))),
        AbstractInteraction.toAccess env
            (((stateChannel (p := p)).pushedIf mult msg).toRaw) =
          (InteractionKind.State, "SP1State",
            [(Expression.eval env msg.clk_high).val,
             (Expression.eval env msg.clk_low).val,
             (Expression.eval env msg.pc0).val,
             (Expression.eval env msg.pc1).val,
             (Expression.eval env msg.pc2).val],
            signedVal (Expression.eval env mult)) :=
    fun mult msg => toAccess_pushIf_state env mult msg
  simp only [loadDoubleStateInteractions,
    List.map_cons, List.map_nil, hStatePull, hStatePush]
  simp [Extracted.LoadDoubleOracle.LoadDoubleColumns.interactions,
    loadDoubleChipReconfigure, loadDoubleOracleAddressOperation,
    Extracted.LoadDoubleOracle.AddressOperation.interactions,
    Extracted.LoadDoubleOracle.AddrAddOperation.interactions,
    Extracted.CPUState.interactions,
    Extracted.ITypeReader.interactions,
    loadDoubleChipRustColumns, loadDoubleEvalAddressCols,
    eval_cpuState, Readers.ITypeReader.eval_cols,
    eval_registerAccessCols, eval_registerAccessTimestamp,
    ← ProvableType.getElem_eval_fields, ProvableType.eval_field,
    Expression.eval, Extracted.Interaction.toAccess,
    Extracted.Dir.sign]

private theorem loadDoubleProgramInteractionsFaithful
    (env : Environment (ZMod p))
    (input : Var LoadDoubleChip.Inputs (ZMod p)) (offset : ℕ) :
    (((((loadDoubleProgramInteractions input).map
        ChannelInteraction.toRaw).map
          (AbstractInteraction.toAccess env)).map
            LookupAccessList.negMult)) =
      (((Extracted.LoadDoubleOracle.LoadDoubleColumns.interactions
          (loadDoubleChipReconfigure
            (loadDoubleChipRustColumns env input offset))).map
            Extracted.Interaction.toAccess).filter
        (fun access => access.1 = InteractionKind.Program)) := by
  have hp2 : 2 < p := by
    have := Fact.out (p := 2 ^ 17 < p)
    omega
  have h35 : (35 : ZMod p).val = 35 := by
    have hp : 2 ^ 17 < p := Fact.out
    have hsmall : (35 : ℕ) < 2 ^ 17 := by norm_num
    exact ZMod.val_natCast_of_lt (Nat.lt_trans hsmall hp)
  have hProgramPull :
      ∀ (gate : Expression (ZMod p))
        (msg : Channels.ProgramMsg (Expression (ZMod p))),
        AbstractInteraction.toAccess env
            (((programChannel (p := p)).pulledIf gate msg).toRaw) =
          (InteractionKind.Program, "SP1Program",
            [(Expression.eval env msg.pc0).val,
             (Expression.eval env msg.pc1).val,
             (Expression.eval env msg.pc2).val,
             (Expression.eval env msg.opcode).val,
             (Expression.eval env msg.op_a).val,
             (Expression.eval env msg.op_b[0]).val,
             (Expression.eval env msg.op_b[1]).val,
             (Expression.eval env msg.op_b[2]).val,
             (Expression.eval env msg.op_b[3]).val,
             (Expression.eval env msg.op_c[0]).val,
             (Expression.eval env msg.op_c[1]).val,
             (Expression.eval env msg.op_c[2]).val,
             (Expression.eval env msg.op_c[3]).val,
             (Expression.eval env msg.op_a_0).val,
             (Expression.eval env msg.imm_b).val,
             (Expression.eval env msg.imm_c).val],
            signedVal (Expression.eval env (-gate))) :=
    fun gate msg => toAccess_pullIf_program env gate msg
  simp only [loadDoubleProgramInteractions,
    List.map_cons, List.map_nil, hProgramPull]
  simp [Extracted.LoadDoubleOracle.LoadDoubleColumns.interactions,
    loadDoubleChipReconfigure, loadDoubleOracleAddressOperation,
    Extracted.LoadDoubleOracle.AddressOperation.interactions,
    Extracted.LoadDoubleOracle.AddrAddOperation.interactions,
    Extracted.CPUState.interactions,
    Extracted.ITypeReader.interactions,
    loadDoubleChipRustColumns, loadDoubleEvalAddressCols,
    eval_cpuState, Readers.ITypeReader.eval_cols,
    eval_registerAccessCols, eval_registerAccessTimestamp,
    ← ProvableType.getElem_eval_fields, ProvableType.eval_field,
    Expression.eval, LookupAccessList.negMult,
    signedVal_neg hp2, Extracted.Interaction.toAccess,
    Extracted.Dir.sign, Opcode.ofNat, h35]

private theorem loadDoublePermMemoryBlocks {α : Type}
    (ram : List α) (opAPull opBPull opBPush opAPush : α) :
    List.Perm (ram ++ [opAPull, opBPull, opBPush, opAPush])
      ([opAPull, opAPush, opBPull, opBPush] ++ ram) := by
  have hrotate :
      List.Perm (ram ++ [opAPull, opBPull, opBPush, opAPush])
        ([opAPull, opBPull, opBPush, opAPush] ++ ram) :=
    List.perm_append_comm
  have htail :
      List.Perm [opAPull, opBPull, opBPush, opAPush]
        [opAPull, opAPush, opBPull, opBPush] := by
    exact List.Perm.cons opAPull
      ((List.Perm.cons opBPull
        (List.Perm.swap opBPush opAPush []).symm).trans
          (List.Perm.swap opBPull opAPush [opBPush]).symm)
  exact hrotate.trans (htail.append_right ram)

private theorem loadDoubleMemoryInteractionsFaithful
    (env : Environment (ZMod p))
    (input : Var LoadDoubleChip.Inputs (ZMod p)) (offset : ℕ) :
    List.Perm
      (((((LoadDoubleChip.exposedMemoryInteractions input offset).map
        ChannelInteraction.toRaw).map
          (AbstractInteraction.toAccess env)).map
            LookupAccessList.negMult))
      (((Extracted.LoadDoubleOracle.LoadDoubleColumns.interactions
          (loadDoubleChipReconfigure
            (loadDoubleChipRustColumns env input offset))).map
            Extracted.Interaction.toAccess).filter
        (fun access => access.1 = InteractionKind.Memory)) := by
  have hp2 : 2 < p := by
    have := Fact.out (p := 2 ^ 17 < p)
    omega
  have hMemoryPull :
      ∀ (gate : Expression (ZMod p))
        (msg : Channels.MemoryMsg (Expression (ZMod p))),
        AbstractInteraction.toAccess env
            (((memoryChannel (p := p)).pulledIf gate msg).toRaw) =
          (InteractionKind.Memory, "SP1Memory",
            [(Expression.eval env msg.clk_high).val,
             (Expression.eval env msg.clk_low).val,
             (Expression.eval env msg.addr0).val,
             (Expression.eval env msg.addr1).val,
             (Expression.eval env msg.addr2).val,
             (Expression.eval env msg.value[0]).val,
             (Expression.eval env msg.value[1]).val,
             (Expression.eval env msg.value[2]).val,
             (Expression.eval env msg.value[3]).val],
            signedVal (Expression.eval env (-gate))) :=
    fun gate msg => toAccess_pullIf_memory env gate msg
  have hMemoryPush :
      ∀ (mult : Expression (ZMod p))
        (msg : Channels.MemoryMsg (Expression (ZMod p))),
        AbstractInteraction.toAccess env
            (((memoryChannel (p := p)).pushedIf mult msg).toRaw) =
          (InteractionKind.Memory, "SP1Memory",
            [(Expression.eval env msg.clk_high).val,
             (Expression.eval env msg.clk_low).val,
             (Expression.eval env msg.addr0).val,
             (Expression.eval env msg.addr1).val,
             (Expression.eval env msg.addr2).val,
             (Expression.eval env msg.value[0]).val,
             (Expression.eval env msg.value[1]).val,
             (Expression.eval env msg.value[2]).val,
             (Expression.eval env msg.value[3]).val],
            signedVal (Expression.eval env mult)) :=
    fun mult msg => toAccess_pushIf_memory env mult msg
  simp only [LoadDoubleChip.exposedMemoryInteractions,
    List.map_cons, List.map_nil, hMemoryPull, hMemoryPush]
  simp [Extracted.LoadDoubleOracle.LoadDoubleColumns.interactions,
    loadDoubleChipReconfigure, loadDoubleOracleAddressOperation,
    Extracted.LoadDoubleOracle.AddressOperation.interactions,
    Extracted.LoadDoubleOracle.AddrAddOperation.interactions,
    Extracted.CPUState.interactions,
    Extracted.ITypeReader.interactions,
    Extracted.LoadDoubleOracle.AddressOperation.value,
    loadDoubleChipRustColumns, loadDoubleEvalAddressCols,
    eval_cpuState, Readers.ITypeReader.eval_cols,
    eval_registerAccessCols, eval_registerAccessTimestamp,
    loadDoubleEvalMemoryCols, loadDoubleEvalMemoryTimestamp,
    ← ProvableType.getElem_eval_fields, ProvableType.eval_field,
    eval_sub, Expression.eval, mul_zero, sub_zero,
    LookupAccessList.negMult,
    signedVal_neg hp2, Extracted.Interaction.toAccess,
    Extracted.Dir.sign]
  exact loadDoublePermMemoryBlocks [_, _] _ _ _ _

private def loadDoubleCpuByteInteractions
    (input : Var LoadDoubleChip.Inputs (ZMod p)) :
    List (ChannelInteraction (byteChannel (p := p))) :=
  [ byteChannel.pulledIf input.is_real
      ⟨6, (input.state.clk_0_16 - 1) * (8 : ZMod p)⁻¹,
       Expression.const ((13 : ℕ) : ZMod p), 0⟩,
    byteChannel.pulledIf input.is_real
      ⟨3, 0, input.state.clk_16_24, 0⟩ ]

private def loadDoubleAddressByteInteractions
    (input : Var LoadDoubleChip.Inputs (ZMod p)) (offset : ℕ) :
    List (ChannelInteraction (byteChannel (p := p))) :=
  [ byteChannel.pulledIf input.is_real
      ⟨6, var { index := offset },
       Expression.const ((16 : ℕ) : ZMod p), 0⟩,
    byteChannel.pulledIf input.is_real
      ⟨6, var { index := offset + 1 },
       Expression.const ((16 : ℕ) : ZMod p), 0⟩,
    byteChannel.pulledIf input.is_real
      ⟨6, var { index := offset + 2 },
       Expression.const ((16 : ℕ) : ZMod p), 0⟩,
    byteChannel.pulledIf input.is_real
      ⟨6, ((var { index := offset } : Expression (ZMod p)) -
          (4 : Expression (ZMod p)) * 0 -
          (2 : Expression (ZMod p)) * 0 - 0) *
        (8 : ZMod p)⁻¹,
       Expression.const ((13 : ℕ) : ZMod p), 0⟩ ]

private def loadDoubleMemoryByteInteractions
    (input : Var LoadDoubleChip.Inputs (ZMod p)) :
    List (ChannelInteraction (byteChannel (p := p))) :=
  [ byteChannel.pulledIf input.is_real
      ⟨6, input.memory_access.access_timestamp.diff_low_limb,
       Expression.const ((16 : ℕ) : ZMod p), 0⟩,
    byteChannel.pulledIf input.is_real
      ⟨3, 0, input.memory_access.access_timestamp.diff_high_limb, 0⟩ ]

private def loadDoubleITypeByteInteractions
    (input : Var LoadDoubleChip.Inputs (ZMod p)) :
    List (ChannelInteraction (byteChannel (p := p))) :=
  let clkLow := input.state.clk_0_16 +
    input.state.clk_16_24 * 65536
  [ byteChannel.pulledIf input.is_real
      ⟨6, input.adapter.op_a_memory.access_timestamp.diff_low_limb,
       Expression.const ((16 : ℕ) : ZMod p), 0⟩,
    byteChannel.pulledIf input.is_real
      ⟨3, 0, (clkLow + 4 -
          input.adapter.op_a_memory.access_timestamp.prev_low - 1 -
          input.adapter.op_a_memory.access_timestamp.diff_low_limb) *
        (65536 : ZMod p)⁻¹, 0⟩,
    byteChannel.pulledIf input.is_real
      ⟨6, input.adapter.op_b_memory.access_timestamp.diff_low_limb,
       Expression.const ((16 : ℕ) : ZMod p), 0⟩,
    byteChannel.pulledIf input.is_real
      ⟨3, 0, (clkLow + 3 -
          input.adapter.op_b_memory.access_timestamp.prev_low - 1 -
          input.adapter.op_b_memory.access_timestamp.diff_low_limb) *
        (65536 : ZMod p)⁻¹, 0⟩ ]

private def loadDoubleByteInteractions
    (input : Var LoadDoubleChip.Inputs (ZMod p)) (offset : ℕ) :
    List (ChannelInteraction (byteChannel (p := p))) :=
  loadDoubleCpuByteInteractions input ++
    loadDoubleAddressByteInteractions input offset ++
    loadDoubleMemoryByteInteractions input ++
    loadDoubleITypeByteInteractions input

omit [Fact (2 ^ 17 < p)] in
private theorem loadDoubleCpuByteInteractionsEq
    (input : Var LoadDoubleChip.Inputs (ZMod p)) (offset : ℕ) :
    ((Readers.CPUState.main
        (loadDoubleCpuInput input)).operations offset).interactionsWith
        byteChannel.toRaw =
      (loadDoubleCpuByteInteractions input).map ChannelInteraction.toRaw := by
  simp [loadDoubleCpuInput, loadDoubleCpuByteInteractions,
    Readers.CPUState.main, Operations.interactionsWith, circuit_norm]

private theorem loadDoubleAddressByteInteractionsEq
    (input : Var LoadDoubleChip.Inputs (ZMod p)) (offset : ℕ) :
    ((AddressOperation.main
        (loadDoubleAddressInput input)).operations offset).interactionsWith
        byteChannel.toRaw =
      (loadDoubleAddressByteInteractions input offset).map
        ChannelInteraction.toRaw := by
  simp [loadDoubleAddressInput, loadDoubleAddressByteInteractions,
    AddressOperation.main, AddrAddOperation.circuit,
    AddrAddOperation.main, Operations.interactionsWith,
    Gadgets.Equality.main, FormalAssertion.toSubcircuit_interactions,
    circuit_norm]

omit [Fact (2 ^ 17 < p)] in
private theorem loadDoubleMemoryByteInteractionsEq
    (input : Var LoadDoubleChip.Inputs (ZMod p)) (offset : ℕ) :
    ((Readers.MemoryAccess.main
        (loadDoubleMemoryInput input offset)).operations
          (offset + 4)).interactionsWith byteChannel.toRaw =
      (loadDoubleMemoryByteInteractions input).map
        ChannelInteraction.toRaw := by
  have heq := fun (n : ℕ)
      (inp : Var (ProvablePair field field) (ZMod p)) =>
    @filter_interactions_formalAssertion_eq_nil
      (ZMod p) _ (ProvablePair field field) ProvablePair.instance
      (Gadgets.Equality.circuit field) byteChannel.toRaw n inp
      List.not_mem_nil List.not_mem_nil
  simp [loadDoubleMemoryInput, loadDoubleMemoryByteInteractions,
    Readers.MemoryAccess.main, Operations.interactionsWith,
    circuit_norm, heq]

private theorem loadDoubleITypeByteInteractionsEq
    (input : Var LoadDoubleChip.Inputs (ZMod p)) (offset : ℕ) :
    ((Readers.ITypeReader.main
        (loadDoubleITypeInput input)).operations
          (offset + 4)).interactionsWith byteChannel.toRaw =
      (loadDoubleITypeByteInteractions input).map
        ChannelInteraction.toRaw := by
  simp [loadDoubleITypeInput, loadDoubleITypeByteInteractions,
    Readers.ITypeReader.main,
    Readers.RegisterAccessCols.circuit,
    Readers.RegisterAccessCols.main,
    Readers.RegisterAccessTimestamp.circuit,
    Readers.RegisterAccessTimestamp.main,
    Operations.interactionsWith, Gadgets.Equality.main,
    FormalAssertion.toSubcircuit_interactions,
    circuit_norm]

private theorem loadDoubleByteInteractionsDecompose
    (input : Var LoadDoubleChip.Inputs (ZMod p)) (offset : ℕ) :
    ((LoadDoubleChip.main input).operations offset).interactionsWith
        byteChannel.toRaw =
      (loadDoubleByteInteractions input offset).map
        ChannelInteraction.toRaw := by
  rw [show
      ((LoadDoubleChip.main input).operations offset).interactionsWith
          byteChannel.toRaw =
        ((Readers.CPUState.main
            (loadDoubleCpuInput input)).operations offset).interactionsWith
            byteChannel.toRaw ++
        ((AddressOperation.main
            (loadDoubleAddressInput input)).operations offset).interactionsWith
            byteChannel.toRaw ++
        ((Readers.MemoryAccess.main
            (loadDoubleMemoryInput input offset)).operations
              (offset + 4)).interactionsWith byteChannel.toRaw ++
        ((Readers.ITypeReader.main
            (loadDoubleITypeInput input)).operations
              (offset + 4)).interactionsWith byteChannel.toRaw by
  simp [LoadDoubleChip.main, loadDoubleCpuInput, loadDoubleAddressInput,
    loadDoubleMemoryInput, loadDoubleAddressValue, loadDoubleAddressCols,
    loadDoubleITypeInput,
    Readers.CPUState.circuit,
    AddressOperation.circuit, Readers.MemoryAccess.circuit,
    Readers.ITypeReader.circuit, Readers.RegisterWrite.circuit,
    Readers.RegisterWrite.main, Gadgets.Equality.main, circuit_norm,
    FormalAssertion.toSubcircuit_interactions,
    GeneralFormalCircuit.toSubcircuit_interactions]
  simp only [Operations.interactionsWith]]
  rw [loadDoubleCpuByteInteractionsEq,
    loadDoubleAddressByteInteractionsEq,
    loadDoubleMemoryByteInteractionsEq,
    loadDoubleITypeByteInteractionsEq]
  simp only [loadDoubleByteInteractions, List.map_append]

private theorem loadDoublePermFourBlocks {α : Type}
    (a b c d : List α) :
    List.Perm (a ++ b ++ c ++ d)
      (b ++ a ++ d ++ c) := by
  have hab : List.Perm (a ++ b ++ c ++ d)
      (b ++ a ++ c ++ d) := by
    simpa only [List.append_assoc] using
      (List.perm_append_comm (l₁ := a) (l₂ := b)).append_right (c ++ d)
  have hcd : List.Perm (b ++ a ++ c ++ d)
      (b ++ a ++ d ++ c) := by
    simpa only [List.append_assoc] using
      (List.perm_append_comm (l₁ := c) (l₂ := d)).append_left (b ++ a)
  exact hab.trans hcd

set_option maxRecDepth 2000 in
private theorem loadDoubleByteInteractionsFaithful
    (env : Environment (ZMod p))
    (input : Var LoadDoubleChip.Inputs (ZMod p)) (offset : ℕ) :
    List.Perm
      ((((LoadDoubleChip.main input).operations offset).interactionsWith
          byteChannel.toRaw).map
            (AbstractInteraction.toAccess env))
      (((Extracted.LoadDoubleOracle.LoadDoubleColumns.interactions
          (loadDoubleChipReconfigure
            (loadDoubleChipRustColumns env input offset))).map
            Extracted.Interaction.toAccess).filter
        (fun access => access.1 = InteractionKind.Byte)) := by
  have h6 : (6 : ZMod p).val = 6 := by
    exact ZMod.val_natCast_of_lt (Nat.lt_trans (by norm_num)
      (Fact.out (p := 2 ^ 17 < p)))
  have h3 : (3 : ZMod p).val = 3 := by
    exact ZMod.val_natCast_of_lt (Nat.lt_trans (by norm_num)
      (Fact.out (p := 2 ^ 17 < p)))
  have hBytePull :
      ∀ (gate : Expression (ZMod p))
        (msg : ByteRow (Expression (ZMod p))),
        AbstractInteraction.toAccess env
            (((byteChannel (p := p)).pulledIf gate msg).toRaw) =
          (InteractionKind.Byte, "SP1Byte",
            [(Expression.eval env msg.opcode).val,
             (Expression.eval env msg.a).val,
             (Expression.eval env msg.b).val,
             (Expression.eval env msg.c).val],
            signedVal (Expression.eval env (-gate))) :=
    fun gate msg => toAccess_pullIf_byte env gate msg
  rw [loadDoubleByteInteractionsDecompose]
  simp only [loadDoubleByteInteractions, List.map_append]
  simp only [loadDoubleCpuByteInteractions,
    loadDoubleAddressByteInteractions, loadDoubleMemoryByteInteractions,
    loadDoubleITypeByteInteractions, List.map_cons, List.map_nil,
    hBytePull]
  simp [Extracted.LoadDoubleOracle.LoadDoubleColumns.interactions,
    loadDoubleChipReconfigure, loadDoubleOracleAddressOperation,
    Extracted.LoadDoubleOracle.AddressOperation.interactions,
    Extracted.LoadDoubleOracle.AddrAddOperation.interactions,
    Extracted.CPUState.interactions,
    Extracted.ITypeReader.interactions,
    Extracted.LoadDoubleOracle.AddressOperation.value,
    loadDoubleChipRustColumns, loadDoubleEvalAddressCols,
    eval_cpuState, Readers.ITypeReader.eval_cols,
    eval_registerAccessCols, eval_registerAccessTimestamp,
    loadDoubleEvalMemoryCols, loadDoubleEvalMemoryTimestamp,
    ← ProvableType.getElem_eval_fields, ProvableType.eval_field,
    eval_sub, Expression.eval, mul_zero, sub_zero,
    h6, h3, Extracted.Interaction.toAccess,
    Extracted.Dir.sign]
  simp only [← ProvableStruct.eval_eq_eval,
    LoadDoubleChip.eval_inputs, eval_cpuState,
    Readers.ITypeReader.eval_cols, eval_registerAccessCols,
    eval_registerAccessTimestamp, loadDoubleEvalMemoryCols,
    loadDoubleEvalMemoryTimestamp, ProvableType.eval_field]
  exact loadDoublePermFourBlocks
    [_, _] [_, _, _, _] [_, _] [_, _, _, _]

private theorem loadDoubleUnexpectedInteractionsEmpty
    (input : Var LoadDoubleChip.Inputs (ZMod p)) (offset : ℕ) :
    unexpectedInteractions
        ((LoadDoubleChip.main input).operations offset) = [] := by
  unfold unexpectedInteractions
  apply List.filter_eq_nil_iff.mpr
  intro interaction hmem hunexpected
  have hchannel :
      interaction.channel ∈
        ((LoadDoubleChip.main input).operations offset).channels := by
    rw [Operations.channels]
    exact List.mem_map.mpr ⟨interaction, hmem, rfl⟩
  have hknown :=
    (LoadDoubleChip.circuit (p := p)).channels_subset
      input offset hchannel
  simp only [LoadDoubleChip.circuit,
    FormalCircuitBase.channelsWithGuarantees_def,
    FormalCircuitBase.channelsWithRequirements_def,
    circuit_norm] at hknown
  simp only [decide_eq_true_eq] at hunexpected
  tauto

theorem loadDoubleChipInteractionsFaithful
    (env : Environment (ZMod p))
    (input : Var LoadDoubleChip.Inputs (ZMod p)) (offset : ℕ)
    (cols : LoadDoubleChip.Columns (ZMod p))
    (hbind : BindsChipOutput LoadDoubleChip.main env input offset cols) :
    List.Perm
      (nativeAccesses env
        ((LoadDoubleChip.main input).operations offset))
      (loadDoubleChipOracle.accesses cols) := by
  replace hbind := BindsChipOutput.ofElaborated
    (LoadDoubleChip.elaborated (p := p)) hbind
  rw [LoadDoubleChip.directOutput_eq] at hbind
  rw [← ProvableStruct.eval_eq_eval,
    LoadDoubleChip.eval_columns] at hbind
  simp only [ProvableType.eval_field] at hbind
  change loadDoubleChipRustColumns env input offset = cols at hbind
  subst cols
  let rustAccesses :=
    (Extracted.LoadDoubleOracle.LoadDoubleColumns.interactions
      (loadDoubleChipReconfigure
        (loadDoubleChipRustColumns env input offset))).map
        Extracted.Interaction.toAccess
  simp only [nativeAccesses]
  rw [loadDoubleUnexpectedInteractionsEmpty]
  simp only [List.map_nil, List.append_nil]
  simp only [ChipOracle.accesses,
    ChipOracle.nativeInteractions, loadDoubleChipOracle]
  rw [loadDoubleStateInteractionsEq,
    LoadDoubleChip.interactionsWith_memory_eq,
    loadDoubleProgramInteractionsEq]
  have hState :=
    loadDoubleStateInteractionsFaithful (p := p) env input offset
  have hByte :=
    loadDoubleByteInteractionsFaithful (p := p) env input offset
  have hMemory :=
    loadDoubleMemoryInteractionsFaithful (p := p) env input offset
  have hProgram :=
    loadDoubleProgramInteractionsFaithful (p := p) env input offset
  refine List.Perm.trans ?_
    (LookupAccessList.perm_filter_by_kind rustAccesses).symm
  dsimp only [rustAccesses] at hState hByte hMemory hProgram ⊢
  rw [hState, hProgram]
  simpa only [List.append_assoc] using
    ((hByte.append_left _).append hMemory).append_right _

theorem loadDoubleChipInteractionsConstructive
    (rustCols : Extracted.LoadDoubleOracle.LoadDoubleColumns (ZMod p))
    (data : ProverData (ZMod p)) :
    let assignment := loadDoubleChipRowCodec.assignment
      (loadDoubleChipOracle.deconfigure rustCols) data
    List.Perm
      (nativeAccesses assignment.environment
        (⟨LoadDoubleChip.circuit (p := p)⟩ :
          Air.Flat.Component (ZMod p)).operations)
      (loadDoubleChipOracle.rustAccesses rustCols) := by
  dsimp only
  let cols := loadDoubleChipOracle.deconfigure rustCols
  let assignment := loadDoubleChipRowCodec.assignment cols data
  have hbind :
      BindsChipOutput LoadDoubleChip.main assignment.environment
        (⟨LoadDoubleChip.circuit (p := p)⟩ :
          Air.Flat.Component (ZMod p)).rowInputVar
        (⟨LoadDoubleChip.circuit (p := p)⟩ :
          Air.Flat.Component (ZMod p)).rowOffset cols := by
    have h := NativeRowAssignment.bindsOutput assignment
    rw [LoadDoubleChip.circuit_main_eq] at h
    exact h
  have hfaithful := loadDoubleChipInteractionsFaithful
    (p := p) assignment.environment
    (⟨LoadDoubleChip.circuit (p := p)⟩ :
      Air.Flat.Component (ZMod p)).rowInputVar
    (⟨LoadDoubleChip.circuit (p := p)⟩ :
      Air.Flat.Component (ZMod p)).rowOffset cols hbind
  rw [nativeAccesses_component_eq_rowOperations
    (LoadDoubleChip.circuit (p := p))
    assignment.environment]
  simpa only [cols, ChipOracle.accesses_deconfigure,
    Air.Flat.Component.rowOperations_mk,
    Air.Flat.Component.rowInputVar_mk,
    Air.Flat.Component.rowOffset_mk,
    LoadDoubleChip.circuit_main_eq] using hfaithful

theorem loadDoubleChip_faithful :
    ChipFaithful (p := p) LoadDoubleChip.Inputs
      LoadDoubleChip.Columns Extracted.LoadDoubleOracle.LoadDoubleColumns
      LoadDoubleChip.circuit loadDoubleChipRowCodec
      loadDoubleChipOracle where
  constraints := loadDoubleChipConstraintsConstructive (p := p)
  interactions := fun rustCols data _ =>
    LookupAccessList.active_perm
      (loadDoubleChipInteractionsConstructive (p := p) rustCols data)

end SP1Clean.Faithful

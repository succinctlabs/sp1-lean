...
/-! # Chip-level faithfulness anchor — SP1's whole `StoreByte` chip constraint list ↔ combined spec -/
...
theorem storebytecols_constraints_faithful (cols : Extracted.StoreByteColumns (ZMod p))
    ...
    (List.Forall (· = 0) (Extracted.StoreByteColumns.asserts cols) ∧
      List.Forall Interaction.toProp (Extracted.StoreByteColumns.interactions cols)) ↔
      ((SP1Clean.AddressOperation.RawSpec
        ...
        ∧ ((cols.memory_access.access_timestamp.compare_low
              ...
              (cols.offset_bit[1] - 1) * ((cols.offset_bit[2] - 1)
                  * (cols.mem_limb - cols.memory_access.prev_value[0])) = 0 ∧
              cols.offset_bit[1] * ((cols.offset_bit[2] - 1)
                  * (cols.mem_limb - cols.memory_access.prev_value[1])) = 0 ∧
              (cols.offset_bit[1] - 1) * (cols.offset_bit[2]
                  * (cols.mem_limb - cols.memory_access.prev_value[2])) = 0 ∧
              cols.offset_bit[1] * (cols.offset_bit[2]
                  * (cols.mem_limb - cols.memory_access.prev_value[3])) = 0 ∧
              cols.increment - ((cols.register_low_byte - cols.mem_limb_low_byte)
                    * (1 - cols.offset_bit[0]) +
                  256 * (cols.register_low_byte
                      - (cols.mem_limb - cols.mem_limb_low_byte) * (256 : ZMod p)⁻¹)
                    * cols.offset_bit[0]) = 0 ∧
              cols.store_value[0] - (cols.increment * (1 - cols.offset_bit[1]) * (1 - cols.offset_bit[2])
                  + cols.memory_access.prev_value[0]) = 0 ∧
              cols.store_value[1] - (cols.increment * cols.offset_bit[1] * (1 - cols.offset_bit[2])
                  + cols.memory_access.prev_value[1]) = 0 ∧
              cols.store_value[2] - (cols.increment * (1 - cols.offset_bit[1]) * cols.offset_bit[2]
                  + cols.memory_access.prev_value[2]) = 0 ∧
              cols.store_value[3] - (cols.increment * cols.offset_bit[1] * cols.offset_bit[2]
                  + cols.memory_access.prev_value[3]) = 0) ∧
            cols.memory_access.access_timestamp.diff_low_limb.val < 65536 ∧
              cols.memory_access.access_timestamp.diff_high_limb.val < 256 ∧
              (cols.register_low_byte.val < 256 ∧
                  ((cols.adapter.op_a_memory.prev_value[0] - cols.register_low_byte)
                    * (256 : ZMod p)⁻¹).val < 256) ∧
                cols.mem_limb_low_byte.val < 256 ∧
                  ((cols.mem_limb - cols.mem_limb_low_byte) * (256 : ZMod p)⁻¹).val < 256) := by
  haveI : NeZero p := ⟨by have := Fact.out (p := 2 ^ 17 < p); omega⟩
  simp only [Extracted.StoreByteColumns.asserts, Extracted.StoreByteColumns.interactions]

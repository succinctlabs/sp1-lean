...
/-! # Chip-level faithfulness anchor — SP1's whole `LoadHalf` chip constraint list ↔ combined spec -/
...
theorem loadhalfcols_constraints_faithful (cols : Extracted.LoadHalfColumns (ZMod p))
    ...
    (List.Forall (· = 0) (Extracted.LoadHalfColumns.asserts cols) ∧
      List.Forall Interaction.toProp (Extracted.LoadHalfColumns.interactions cols)) ↔
      (((SP1Clean.AddressOperation.RawSpec
            ...
            ∧ (cols.is_lh * (cols.is_lh - 1) = 0 ∧ cols.msb.msb * (cols.msb.msb - 1) = 0)
              ∧ (¬cols.is_lh = 0 →
                  (2 * cols.selected_half - cols.msb.msb * 65536 : ZMod p).val < 2 ^ 16))
          ∧ ((cols.state.clk_0_16 - 1) * (8 : ZMod p)⁻¹).val < 2 ^ 13
              ∧ cols.state.clk_16_24.val < 2 ^ 8)
        ∧ ((cols.adapter.op_a_0 * cols.selected_half = 0 ∧
        ...
        ∧ ((cols.is_lh * (cols.is_lh - 1) = 0 ∧
              ...
              (cols.offset_bit[0] - 1) * ((cols.offset_bit[1] - 1)
                  * (cols.selected_half - cols.memory_access.prev_value[0])) = 0 ∧
              cols.offset_bit[0] * ((cols.offset_bit[1] - 1)
                  * (cols.selected_half - cols.memory_access.prev_value[1])) = 0 ∧
              (cols.offset_bit[0] - 1) * (cols.offset_bit[1]
                  * (cols.selected_half - cols.memory_access.prev_value[2])) = 0 ∧
              cols.offset_bit[0] * (cols.offset_bit[1]
                  * (cols.selected_half - cols.memory_access.prev_value[3])) = 0 ∧
              cols.is_lhu * cols.msb.msb = 0) ∧
  ...
  simp only [Extracted.LoadHalfColumns.asserts, Extracted.LoadHalfColumns.interactions]

...
`diff = clk − prev − 1` decomposition) and the two byte range checks (`diff_low < 2^16`,
...
/-- **Chip-level faithfulness anchor.** Under `is_real = 1`, SP1's generated `StoreDouble` chip
constraint list holds iff: the `AddressOperation` raw spec on the `rs1` register read and the immediate
(offset bits `0`); the two CPUState clock bounds; the `ITypeReaderImmutable` per-row well-formedness
(the four `op_a_0` *read*-zeroing gates on the rs2 word + the op_a/op_b timestamp byte bounds); and the
`MemoryAccess` timestamp consistency (`compare_low` binary, the high-timestamp selection gate, the
`clk − prev − 1 = diff` decomposition, and the `diff_low < 2^16` / `diff_high < 2^8` byte ranges). -/
theorem storedoublecols_constraints_faithful (cols : Extracted.StoreDoubleColumns (ZMod p))
    (h_real : cols.is_real = 1) :
    (List.Forall (· = 0) (Extracted.StoreDoubleColumns.asserts cols) ∧
      List.Forall Interaction.toProp (Extracted.StoreDoubleColumns.interactions cols)) ↔
  ...
  simp only [Extracted.StoreDoubleColumns.asserts, Extracted.StoreDoubleColumns.interactions]

...
- **Build concurrency.** `lake build` jobs on heavy chips (`SP1Chips.DivRem.Constraints` etc.) take 17–40 min and consume 5–15 GB RSS each. Before starting any new `lake build`, **either let the running build finish or kill it explicitly** (`pkill -f "lake build"` / `pkill -f "lake env lean SP1Chips"`). Hard cap: **2–3 builds at once, full stop** — never start a fourth speculative build. A `run_in_background` build whose parent shell died gets reparented to init and survives session boundaries; check with `ps -ef | grep -E "lake|lean" | grep -v lsp` before spawning a new one. The lean LSP server (`lean --server` from `uvx lean-lsp-mcp`) also keeps several GB warm and competes for CPU on elaboration — if you're running a heavy build, expect it to add to memory pressure. When iterating heavily on a single heavy file, prefer extracting the proof you're editing into a small helper file that imports `SP1Chips.<Chip>.Constraints` — once the dependency's olean is cached, the helper file rebuilds in 3–4 min per iteration instead of 17+.
...
### Faithful sub-circuit composition (SP1Clean operations and chips)

SP1Clean files must structurally mirror the SP1 constraint composition graph. The principle has three parts; violations are bugs to be fixed.

1. **Use sub-circuits, not inline constraints.** When `SP1Operations/Operation/<Op>/Constraints.lean` sub-calls another operation's `constraints` (e.g. `AddwOperation.constraints` appends `U16MSBOperation.constraints`), the Clean mirror in `SP1Clean/Operations/<Op>.lean` must compose the sub-operation as a true Clean `FormalAssertion` subcircuit (`SP1Clean.<SubOp>.assertion`) — never inline the sub-constraints. Likewise chip-level `<Chip>/Circuit.lean` files compose reader/adapter `FormalAssertion`s rather than inlining their lookups.

2. **One `main`, one `Spec` per file.** Each `SP1Clean/Operations/<Op>.lean` and `SP1Clean/<Chip>Chip/Circuit.lean` should expose exactly one `main : Var Inputs → Circuit Unit` and one `Spec : Inputs → Prop` (plus the `assertion` / `elaborated` glue and a single `iff_sp1` bridge to SP1's `allHold`). The `Spec` references sub-operation Specs by direct field application (`SP1Clean.<SubOp>.Assertion.Spec ⟨…⟩`), NEVER by re-wrapping sub-constraints in the SP1-native `List.Forall SP1Constraint.toProp ...` envelope. No separate top-level standalone `main (a b ...)` / `Spec (a b ...)` form parallel to the Inputs-shaped one.

3. **No `InlinedSpec` / `inlinedSpec_iff_spec`-style bridging helpers.** They are a smell — they exist only when the `main` and the `Spec` were defined in mismatched forms. The fix is to align them, not paper over the mismatch with a bridge.

Concrete examples:
- ✅ `SP1Clean.AddwOp.main` calls `SP1Clean.U16MSBOp.assertion` as a subcircuit; `AddwOp.Spec` composes `U16MSBOp.Assertion.Spec` directly. One `main`, one `Spec`, one `iff_sp1`.
- ✅ `SP1Clean.AddChip.main` composes `RTypeReader.assertion`; `AddwChip.main` composes `ALUTypeReader.assertion`; both have one `FormalSpec`.
- ❌ Avoid: a chip-level `main` that inlines byte lookups when a reader-level `OperandAccess.assertion` exists.
- ❌ Avoid: defining `Spec` with `List.Forall SP1Constraint.toProp (<Sub>.constraints …)` when a `<Sub>.Assertion.Spec` is available.

Benefits: chip-level `circuit_proof_start` produces a clean tuple of sub-Specs rather than a flat conjunction of low-level constraint facts; bridging helpers become unnecessary; the Lean type-checker enforces structural alignment with SP1's composition graph (mismatches surface as elaboration errors).
...
- `docs/CLEAN_NATIVE_DIRECTION.md` — architecture decision brief + single-chip **prototype plan** for the Clean-native end-state (retiring `SP1Chips/*` + `update_constraints.py`). Verdict: the current structural-`FormalAssertion` + `SailBridge` chip shape is a reusable stepping-stone, but the structural `FormalSpec` and the borrowed `_root_.<X>.Poly.correct_*` are legacy artifacts; the target is witnessed `FormalCircuit`/`GeneralFormalCircuit` gadgets with **semantic** specs (à la Clean's FemtoCairo example) composed into a `FormalTable`. Read before scaling the current chip pattern or starting the retirement.
- `docs/MULTIPLICITY_BUS.md` — **work-in-progress** doc for the multiplicity-aware lookup bus (`SP1Clean/SP1Lookup.lean`). New canonical representation mirroring SP1's `AirInteraction<E>` from `sp1/crates/core/machine/src/air/memory.rs` (`InteractionKind`-tagged buses, `lookupGated` primitive with hint-witness completeness, parallel `LookupAccessList` aggregator). Phase 1+2 foundation landed and verified axiom-clean; Phase 3–5 chip refactor + sorry closure (AddwChip op_c, MemoryConsistency.lean addw discharge) remains. **Read this if you hit either of the two `sorry`s at `SP1Clean/AddwChip/Circuit.lean:152` or `SP1Clean/Soundness/MemoryConsistency.lean:1067`** — both have clear closure paths documented.

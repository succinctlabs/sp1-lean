...
- **`DivRemChip` and `ShiftRightChip` dominate full-build time** (their `Constraints.lean` files are an order of magnitude larger than the others, and the bundled `correct_*` proofs each chew through high heartbeat budgets). For low-level edits — Foundations helpers, Operation lemmas, single chip changes — iterate via `lake env lean <file>` on the affected files (and a few representative chips), and only run `lake build` once the targeted files are clean. A full build at the start of every loop will burn most of your wall-clock on DivRem/ShiftRight even when your change can't possibly affect them.
...
- **`SP1Foundations`** — the `SP1Constraint` / `AirInteraction` datatype (`Constraint.lean`), the KoalaBear field (`Field.lean`, abbreviated `KB`), `Word`/`BitVec` helpers, the `SailM` monad bridge to `LeanRV64D`, shared tactics, and a few load-bearing extras: `Assumptions.lean` (the `@[simp]`-tagged reader constraint bundles `i_type_constraints`, `shift_i_type_constraints`, `w_shift_i_type_constraints`, `r_type_constraints`, `b_type_constraints`, plus `trusted_instr` — the raw layer that each chip's `allHold_constraints_iff_is_real` lemma rests on), `MemChecks.lean` and `MemoryConsistency.lean` (MMIO/memory access correctness lemmas — `run_within_mmio_writable_mmio`, `run_within_mmio_readable_mmio`, `run_checked_mem_write_one_byte_of_isInitialized`, etc., load-bearing for every Load*/Store* chip proof), and small helpers `Opcode.lean` / `ByteOpcode.lean` / `Register.lean` / `Unsigned.lean` / `Misc.lean`.
- **`SP1Operations`** — reusable constraint fragments. Each child has both a `<Name>.lean` wrapper and a `<Name>/` subdirectory with the auto-generated constraints; see `SP1Operations.lean` for the full import index. The actual directory names carry suffixes:
  - `Operation/` — `AddOperation`, `AddwOperation`, `SubOperation`, `SubwOperation`, `MulOperation`, `AddrAddOperation`, `AddressOperation`, `BitwiseOperation`, `BitwiseU16Operation`, `U16MSBOperation`, `U16toU8OperationSafe`, `U16toU8OperationUnsafe`.
  - `Compare/` — `IsZeroOperation`, `IsZeroWordOperation`, `IsEqualWordOperation`, `LtOperationSigned`, `LtOperationUnsigned`, `U16CompareOperation`.
  - `Reader/` — `RTypeReader`, `ITypeReader`, `ITypeReaderImmutable`, `JTypeReader`, `ALUTypeReader`, plus `CPUState` for clk/pc bookkeeping.
- **`SP1Chips`** — one subdirectory per instruction chip. The top-level `<Chip>Chip.lean` contains `spec_*`, `sp1_*`, and `correct_*`; it imports `<Chip>/Constraints.lean`, which contains the auto-generated `constraints` function. Load and Store fan out: `Load/{LoadByte,LoadDouble,LoadHalf,LoadWord,LoadX0}/Constraints.lean` and `Store/{StoreByte,StoreDouble,StoreHalf,StoreWord}/Constraints.lean`, with matching top-level `Load<Width>Chip.lean` / `Store<Width>Chip.lean` (no top-level chip for `LoadX0` — constraints only). A single `<Chip>Chip.lean` may also prove several variants in one file (e.g. `BitwiseChip` and `BranchChip` bundle 6 `correct_*` theorems each, `ShiftRightChip` bundles 8, `DivRemChip` bundles 9; `ShiftLeftChip` and `LtChip` bundle 4).
...
- `DecidableEq` synthesis for `regidx` (defined in `LeanRV64D.Defs`) used to close with `by simp; infer_instance` since the inner type `BitVec (if false then 4 else 5)` reduces to `BitVec 5`. In 4.29 that path no longer finds the instance; use `decidable_of_iff (v = v') (by simp)` instead (see the `DecidableEq regidx` instance in `SP1Foundations/Register.lean`).
- `simp_all` no longer derives `Main[i] = 1` from a long conjunction of disjuncts in 4.29, even when one of them is `(... ∨ -1 = 0)` (which is False in `Fin KB`). The brittle pattern `have h : Main[i] = 1 := by simp_all [sub_eq_zero]` now leaves an unsolved goal. Replace with direct conjunction projection: `(h_cstrs.2.2…1).resolve_right (by decide)` then `omega`/`sub_eq_zero` to recover the equation. The `extract_from_and` macro fails when the conjunction contains a non-`And` term (e.g. `∀ i, …`) — projection is more robust than recursive destructuring. See `op_a_lt32_of_constraints` in `SP1Chips/JalChip.lean` for the canonical pattern.
...
`.mcp.json` declares `lean-lsp` (live goal/tactic state from the Lean LSP). It launches via `uvx`, so `uv` must be on `PATH` — if it's missing, install with `curl -LsSf https://astral.sh/uv/install.sh | sh` (puts `uv`/`uvx` in `~/.local/bin`). Local enable/disable is in `.claude/settings.local.json` (gitignored; uses `enabledMcpjsonServers` / `disabledMcpjsonServers` lists to opt servers in or out). Restart Claude Code after installing or toggling.

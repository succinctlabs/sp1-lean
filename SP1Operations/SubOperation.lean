...
def spec (cols : SubOperation (Fin p)) (a b : Word (Fin p)) : Prop :=
...
def extractedConstraints (cols : SubOperation (Fin p))
...
def idealizedConstraints (cols : SubOperation (Fin p))
    (a b : Word (Fin p)) : Prop :=
  let carry0 : Fin p := 1
  let carry1 : Fin p := (a[0] + base - 1 - b[0] - cols[0] + carry0) * baseInv
  let carry2 : Fin p := (a[1] + base - 1 - b[1] - cols[1] + carry1) * baseInv
...
lemma extractedConstraints_iff_idealizedConstraints
    (cols : SubOperation (Fin p)) (a b : Word (Fin p)) :
...
theorem correct [Fact (Nat.Prime p)] (cols : SubOperation (Fin p))

...
/-- AndOperation is only operating on `U8` because it's used solely for
computing the and operation over four **bytes**. -/
structure AndOperation where
  value : Vector U8 WORD_BYTE_SIZE

namespace AndOperation

/-- `AndOperation` should result in and of all 4 limbs -/
def spec (cols : AndOperation)
...
Sends: AirInteraction { values: [0, cols.value[0], a[0], b[0]], multiplicity: is_real, kind: Byte }
Sends: AirInteraction { values: [0, cols.value[1], a[1], b[1]], multiplicity: is_real, kind: Byte }
Sends: AirInteraction { values: [0, cols.value[2], a[2], b[2]], multiplicity: is_real, kind: Byte }
...
theorem correct (cols : AndOperation)
    (a b : Vector (BabyBear) WORD_BYTE_SIZE) (is_real : BabyBear) :

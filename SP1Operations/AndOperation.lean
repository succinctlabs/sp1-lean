...
-- def spec (cols : AndOperation (BabyBear))
...
--     List (SP1Constraint) :=
--   [.byteAirInteraction (ByteOpcode.ofNat 0) cols[0] a[0] b[0] is_real,
--    .byteAirInteraction (ByteOpcode.ofNat 0) cols[1] a[1] b[1] is_real,
--    .byteAirInteraction (ByteOpcode.ofNat 0) cols[2] a[2] b[2] is_real,
--    .byteAirInteraction (ByteOpcode.ofNat 0) cols[3] a[3] b[3] is_real,]
...
-- theorem correct (cols : AndOperation (BabyBear))
--     (a b : Vector (BabyBear) WORD_BYTE_SIZE) (is_real : BabyBear) :
...
/-- `AndOperation` should result in and of all 4 limbs -/
def spec (cols : AndOperation2 (BabyBear))
    (a b : Vector (BabyBear) WORD_BYTE_SIZE)
    (is_real : BabyBear) : Prop :=
  is_real ≠ 0 → ∀ i : Fin WORD_BYTE_SIZE,
    a[i] &&& b[i] = cols[i]
...
Sends: AirInteraction { values: [0, cols[2], a[2], b[2]], multiplicity: is_real, kind: Byte }
Sends: AirInteraction { values: [0, cols[3], a[3], b[3]], multiplicity: is_real, kind: Byte } -/
def extractedConstraints (cols : AndOperation2 (BabyBear))
    (a b : Vector (BabyBear) WORD_BYTE_SIZE) (is_real : BabyBear) : Prop :=
...
/-- Cleaned up representation of the `AndOperation` constraints. -/
def idealizedConstraints (cols : AndOperation2 (BabyBear))
    (a b : Vector (BabyBear) WORD_BYTE_SIZE) : Prop :=
  a[0] &&& b[0] = cols[0] ∧
  a[1] &&& b[1] = cols[1] ∧
  a[2] &&& b[2] = cols[2] ∧
...
/-- The idealized constraints are logically equivalent to the extracted ones when `is_real := 1` -/
lemma extractedConstraints_iff_idealizedConstraints
    (cols : AndOperation2 (BabyBear)) (a b : Vector (BabyBear) WORD_BYTE_SIZE) :
    cols.extractedConstraints a b 1 ↔ cols.idealizedConstraints a b := by
  simp [extractedConstraints, idealizedConstraints, ByteOpcode.ofNat, ByteOpcode.constrain]
  tauto

/-- The extracted constraints on `AndOperation` imply the spec. -/
theorem correct (cols : AndOperation2 (BabyBear))
    (a b : Vector (BabyBear) WORD_BYTE_SIZE) (is_real : BabyBear) :
    cols.idealizedConstraints a b → cols.spec a b is_real := by
  simp [idealizedConstraints, spec, sub_eq_zero, mul_eq_zero]
  intros h1 h2 h3 h4 hreal i

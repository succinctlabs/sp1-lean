...
Sends: AirInteraction { values: [1, cols[0], a[0], b[0]], multiplicity: is_real, kind: Byte }
Sends: AirInteraction { values: [1, cols[1], a[1], b[1]], multiplicity: is_real, kind: Byte }
Sends: AirInteraction { values: [1, cols[2], a[2], b[2]], multiplicity: is_real, kind: Byte }
Sends: AirInteraction { values: [1, cols[3], a[3], b[3]], multiplicity: is_real, kind: Byte } -/
def extractedConstraints (cols : XorOperation (BabyBear))
    (a b : Vector (BabyBear) WORD_BYTE_SIZE) (is_real : BabyBear) :
    List (SP1Constraint) :=
  [
  .sendAirInteraction_byte (.ofNat 1) cols[0] a[0] b[0] is_real,
  .sendAirInteraction_byte (.ofNat 1) cols[1] a[1] b[1] is_real,
  .sendAirInteraction_byte (.ofNat 1) cols[2] a[2] b[2] is_real,
  .sendAirInteraction_byte (.ofNat 1) cols[3] a[3] b[3] is_real,
  ]

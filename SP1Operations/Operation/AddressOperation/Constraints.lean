...
def constraints
  ...
  let CS0 : SP1ConstraintList := AddrAddOperation.constraints #v[b[0], b[1], b[2], b[3]] #v[cc[0], cc[1], cc[2], cc[3]] { value := #v[cols.addr_word_operation.value[0], cols.addr_word_operation.value[1], cols.addr_word_operation.value[2]] } is_real
  let E8 : Fin BB := cols.addr_word_operation.value[1] + cols.addr_word_operation.value[2]
  let E9 : Fin BB := cols.top_two_limb_inv * E8
  let E10 : Fin BB := E9 - is_real
  let E11 : Fin BB := 4 * offset_bit2
  let E12 : Fin BB := cols.addr_word_operation.value[0] - E11
  ...
  let E18 : Fin BB := cols.addr_word_operation.value[0] - E17
  let E19 : Fin BB := 2 * offset_bit1
  let E20 : Fin BB := E18 - E19
  let E21 : Fin BB := E20 - offset_bit0
  ⟨#v[E21, cols.addr_word_operation.value[1], cols.addr_word_operation.value[2]], CS0 ++ [
    (.assertZero E1),
    (.assertZero E3),
    (.assertZero E5),
    (.assertZero E7),

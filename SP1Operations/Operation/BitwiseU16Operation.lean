...
lemma spec.and {b cc : Word (Fin KB)} {cols : BitwiseU16Operation (Fin KB)}
    ...
    List.Forall SP1Constraint.toProp (constraints b cc cols 0 1).2 →
      Word.toBitVec64 (constraints b cc cols 0 1).1 = execute_RTYPE_pure_w b cc .AND
        := by
  intro cstrs
  have h_bw_b := @U16toU8OperationSafe.spec.unsafe.return b
    ...
    aesop)
  have h_bw_c := @U16toU8OperationSafe.spec.unsafe.return cc
...
-- Polymorphic versions follow the same `bv_decide`-after-byte-decomposition
-- recipe as the `Fin KB` proofs, but use the polymorphic helpers
-- (`spec.unsafe.return_poly`, `Word.toBitVec64_poly`) and require explicit
-- `.val` arithmetic to remove `% p` from the byte-AND/OR/XOR equations
-- (since `byte.val < 256 < p` makes `% p` an identity).
/-- Polymorphic counterpart of `spec.and`. -/
...
/-- Polymorphic counterpart of `spec.or`. -/
...
/-- Polymorphic counterpart of `spec.xor`. -/

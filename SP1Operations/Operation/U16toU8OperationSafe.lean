...
to a concrete `Word.toBWord_poly` byte vector. The 8 byte-level equations
follow from applying `u16_to_u8_decomposition_poly` per limb. -/
...
differs), so we can dispatch to `spec.unsafe.return_poly` after a
definitional bridge. -/

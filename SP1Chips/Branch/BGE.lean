...
private theorem h_Main_31_is_bge
  (cstrs : (constraints Main).allHold)
  (h_is_bge : Main[31] = 1)
  -- (h_is_real : Main[28] + Main[29] + Main[30] + Main[31] + Main[32] + Main[33] = 1)
  : Main[28] = 0 ∧ Main[29] = 0 ∧ Main[30] = 0 ∧ Main[32] = 0 ∧ Main[33] = 0 := by
  simp [SP1ConstraintList.allHold, Branch.constraints] at cstrs
  obtain ⟨_, _, _, ⟨h_28, h_29, h_30, h_31, h_32, h_33, h_all_add, _⟩⟩ := cstrs
  clear * - h_is_bge h_28 h_29 h_30 h_31 h_32 h_33 h_all_add
  rw [sub_eq_zero] at *
  split_ands
  ...
  <;> rw [h_is_bge, h_28, h_29, h_30, h_32, h_33] at h_all_add
  <;> trivial
...
def sp1_op_a : BitVec 5 :=
  by
    ...
    show Main[6] < 32
    obtain ⟨h_28, h_29, h_30, h_32, h_33⟩ := h_Main_31_is_bge Main cstrs h_is_bge
    simp [SP1ConstraintList.allHold, Branch.constraints] at cstrs
    obtain ⟨_, reader_cstrs, _, _⟩ := cstrs
    simp [SP1ConstraintList.allHold, ITypeReaderImmutable.constraints, SP1Constraint.toProp, h_is_bge, h_28, h_29, h_30, h_32, h_33, Opcode.ofNat, Nat.beq, Nat.ble] at reader_cstrs
...
def sp1_bge : SailM ExecutionResult := do
...
theorem correct_bge
    ...
    : let imm := sp1_imm Main
      let op_b := sp1_op_b Main cstrs h_is_bge
      let op_a := sp1_op_a Main cstrs h_is_bge
    (spec_bge imm (.Regidx op_b) (.Regidx op_a)).run s = (sp1_bge Main).run s := by
  ...
  simp [ITypeReaderImmutable.constraints, SP1Constraint.toProp,
    h_is_bge, h_28, h_29, h_30, h_32, h_33, Opcode.ofNat, Nat.ble, Nat.beq] at reader_cstrs
  ...
  simp [SP1ConstraintList.initialState, constraints, SP1Constraint.toStateProp, List.Forall,
    ...
    h_is_bge, h_28, h_29, h_30, h_32, h_33, Opcode.ofNat, Nat.ble, Nat.beq] at state_cstrs
  ...
  by_cases h_eq : op_b_val.toInt ≤ op_a_val.toInt <;> simp [op_a_val, op_b_val] at h_eq
  ·
    simp [zopz0zKzJ_s, h_eq]
    rw [run_readReg]
    ...
    have h35 : Main[35] = 0 := by
      simp [BitVec.slt, lt_iff_not_ge, h_eq] at spec_lt
      clear * - spec_lt
      tauto
    simp [h35, sub_eq_zero] at chip_cstrs
    have h34 : Main[34] = 1 := by clear * - chip_cstrs; aesop
  ...
  ·
    rw [lt_iff_not_ge] at h_eq
    simp [zopz0zKzJ_s, h_eq]

    have h35 : Main[35] = 1 := by
      simp [BitVec.slt, lt_iff_not_ge, h_eq] at spec_lt
      clear * - spec_lt
      tauto
    ...
    -- have h_opcode : (Main[28] * 27 + Main[29] * 28 + Main[30] * 29 + Main[31] * 30 + Main[32] * 31 + Main[33] * 32) = 30 :=
    --   by
    --     aesop
    ...
    -- clear cpu_cstrs
    -- simp [ITypeReaderImmutable.constraints, SP1Constraint.toProp, h_is_bge, h_28, h_29, h_30, h_32, h_33, h_is_real, h_opcode, Opcode.ofNat, Nat.ble, Nat.beq] at reader_cstrs
    ...
    --   by exact Word.isU64_of_cases h_imm_0 h_imm_1 h_imm_2 h_imm_3
    ...
    -- simp [h_is_bge, h_28, h_29, h_30, h_32, h_33, Opcode.ofNat, Nat.ble, Nat.beq] at state_cstrs
    -- obtain ⟨h_pc_read, h_op_a_read, h_op_b_read⟩ := state_cstrs
    ...
    --       clear * - spec_lt
    --       aesop
    --   have h_is_lt : Main[35] = 1 := by clear * - spec_lt; aesop

    --   simp [h_is_neq, h_is_lt, sub_eq_zero] at chip_cstrs
    --   have h_no_branching : Main[34] = 0 := by aesop
    --   simp [h_no_branching] at chip_cstrs

    --   obtain ⟨h_limb0, h_limb1, h_limb2, h_limb3, h_bound_checks⟩ := chip_cstrs

    --   have h_pc_0 : Main[3].val < 65536 := by show Main[3] < 65536; clear * - reader_cstrs; aesop
    --   have h_pc_1 : Main[4].val < 65536 := by show Main[4] < 65536; clear * - reader_cstrs; aesop
    --   have h_pc_2 : Main[5].val < 65536 := by show Main[5] < 65536; clear * - reader_cstrs; aesop
    --   have h_pc_is_u64 : Main[3].val + Main[4].val * 65536 + Main[5].val * 4294967296 < 2^64 := by omega
    --   rw [←BitVec.ofNatLT_eq_ofNat h_pc_is_u64]
    --   simp [BitVec.add_def]
    --   have h_pc_add4_is_u64 : Main[3].val + Main[4].val * 65536 + Main[5].val * 4294967296 + 4 < 2^64 := by omega
    ...
    --   -- This should come from the spec of LtOperationSigned
    --   simp [h_eq, BitVec.slt, h_ges, h_30, h_is_bge] at spec_lt
    ...
    --   simp [h_is_bge, h_eq, h_is_ge, h_is_eq, h_28, h_29, h_30, h_32, h_33, Opcode.ofNat, Nat.ble, Nat.beq] at chip_cstrs
    --   simp [sub_eq_zero] at chip_cstrs
    ...
    --     clear * - spec_lt
    --     aesop
    -- have h_is_ge : Main[35] = 0 := by clear * - h_ges spec_lt; rw [if_neg] at spec_lt <;> aesop

    -- simp [h_is_bge, h_neq, h_is_ge, h_is_neq, h_28, h_29, h_30, h_32, h_33, Opcode.ofNat, Nat.ble, Nat.beq] at chip_cstrs
    -- simp [sub_eq_zero] at chip_cstrs

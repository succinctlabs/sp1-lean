...
theorem correct_bne
  ...
  (spec_bne imm (.Regidx op_b) (.Regidx op_a)).run s = (sp1_bne Main).run s
  := by
    extract_lets
    ...
    have h_opcode : (Main[28] * 27 + Main[29] * 28 + Main[30] * 29 + Main[31] * 30 + Main[32] * 31 + Main[33] * 32) = 28 :=
      by
        aesop
    ...
    clear cpu_cstrs
    simp [ITypeReaderImmutable.constraints, SP1Constraint.toProp, h_is_bne, h_28, h_30, h_31, h_32, h_33, h_is_real, h_opcode, Opcode.ofNat, Nat.ble, Nat.beq] at reader_cstrs
    ...
    simp [SP1ConstraintList.initialState, constraints, SP1Constraint.toStateProp, List.Forall, CPUState.constraints, ITypeReaderImmutable.constraints, LtOperationSigned.constraints, LtOperationUnsigned.constraints, U16MSBOperation.constraints, U16CompareOperation.constraints, h_is_bne, h_28, h_30, h_31, h_32, h_33, Opcode.ofNat, Nat.ble, Nat.beq] at state_cstrs
    ...
    · rename_i h_ne
      ...
      have h_is_branching : Main[36] + Main[37] + Main[38] + Main[39] = 1 :=
        by
          simp [h_ne, h_30, h_31] at spec_lt
          clear * - spec_lt
          aesop

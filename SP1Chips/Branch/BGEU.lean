...
theorem correct_bgeu
  ...
  (spec_bgeu imm (.Regidx op_b) (.Regidx op_a)).run s = (sp1_bgeu Main).run s := by
  ...
  simp [ITypeReaderImmutable.constraints, SP1Constraint.toProp,
    h_is_bgeu, h_28, h_30, h_31, h_29, h_33, Opcode.ofNat, Nat.ble, Nat.beq] at reader_cstrs
  ...
  simp [SP1ConstraintList.initialState, constraints, SP1Constraint.toStateProp, List.Forall,
    ...
    h_is_bgeu, h_28, h_30, h_31, h_29, h_33, Opcode.ofNat, Nat.ble, Nat.beq] at state_cstrs
  ...
  by_cases h_eq : op_b_val ≤ op_a_val <;> simp [op_a_val, op_b_val] at h_eq
  · have h_eq' : ¬ Word.toBitVec64 #v[Main[7], Main[8], Main[9], Main[10]] <
      Word.toBitVec64 #v[Main[15], Main[16], Main[17], Main[18]] := by aesop

    have : Main[35] = 0 := by
      clear * - spec_lt h_eq'
      simp [BitVec.ult, BitVec.toNat_lt_toNat_iff, h_eq'] at spec_lt
      -- simp_rw [lt_iff_not_ge] at spec_lt
      aesop
    ...
    simp [BitVec.toNat_le_toNat_iff, h_eq]
    rw [run_readReg]
  ...
  · have h_eq' : ¬ Word.toBitVec64 #v[Main[15], Main[16], Main[17], Main[18]] ≤
      Word.toBitVec64 #v[Main[7], Main[8], Main[9], Main[10]] := by aesop

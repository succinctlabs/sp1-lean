...
theorem correct_blt
    ...
    (spec_blt imm (.Regidx op_b) (.Regidx op_a)).run s = (sp1_blt Main).run s := by
  extract_lets imm op_b op_a
  obtain ⟨h_28, h_29, h_30, h_31, h_33⟩ := h_Main32_is_blt Main cstrs h_is_blt
  ...
  simp [ITypeReaderImmutable.constraints, SP1Constraint.toProp,
    h_is_blt, h_28, h_29, h_30, h_31, h_33, Opcode.ofNat, Nat.ble, Nat.beq] at reader_cstrs
  ...
  simp [SP1ConstraintList.initialState, constraints, SP1Constraint.toStateProp, List.Forall,
    ...
    h_is_blt, h_28, h_29, h_30, h_31, h_33, Opcode.ofNat, Nat.ble, Nat.beq] at state_cstrs
  ...
  by_cases h_eq :  op_a_val.toInt < op_b_val.toInt <;> simp [op_a_val, op_b_val] at h_eq
  ·
    simp [zopz0zI_s, h_eq]
    rw [run_readReg]
    ...
    have h35 : Main[35] = 1 := by
      simp [BitVec.slt, h_eq] at spec_lt
      clear * - spec_lt
      tauto
    ...
    have h_imm_0 : Main[21] < 65536 := by aesop
    have h_imm_1 : Main[22] < 65536 := by aesop
    have h_imm_2 : Main[23] < 65536 := by aesop
    have h_imm_3 : Main[24] < 65536 := by aesop
    ...
    omega
  ·
    simp [zopz0zI_s, h_eq, lt_iff_not_ge]

...
private theorem h_Main32_is_blt
  (cstrs : (constraints Main).allHold)
  (h_is_blt : Main[30] = 1)
  : Main[28] = 0 ∧ Main[29] = 0 ∧ Main[31] = 0 ∧ Main[32] = 0 ∧ Main[33] = 0 := by
  simp [SP1ConstraintList.allHold, Branch.constraints] at cstrs
  obtain ⟨_, _, _, ⟨h_28, h_29, h_30, h_31, h_32, h_33, h_all_add, _⟩⟩ := cstrs
  clear * - h_is_blt h_28 h_29 h_30 h_31 h_32 h_33 h_all_add
  rw [sub_eq_zero] at *
  split_ands
  ...
  <;> rw [h_is_blt, h_28, h_29, h_31, h_32, h_33] at h_all_add
  <;> trivial
...
def sp1_op_a : BitVec 5 :=
  by
    ...
    show Main[6] < 32
    obtain ⟨h_28, h_29, h_31, h_32, h_33⟩ := h_Main32_is_blt Main cstrs h_is_blt
    simp [SP1ConstraintList.allHold, Branch.constraints] at cstrs
    obtain ⟨_, reader_cstrs, _, _⟩ := cstrs
    simp [SP1ConstraintList.allHold, ITypeReaderImmutable.constraints, SP1Constraint.toProp, h_is_blt, h_28, h_29, h_31, h_32, h_33, Opcode.ofNat, Nat.beq, Nat.ble] at reader_cstrs
    simp_all only
...
def sp1_blt : SailM ExecutionResult := do
...
theorem correct_blt
  ...
  : let imm := sp1_imm Main
    let op_b := sp1_op_b Main cstrs h_is_blt
    let op_a := sp1_op_a Main cstrs h_is_blt
  (spec_blt imm (.Regidx op_b) (.Regidx op_a)).run s = (sp1_blt Main).run s
  := by
    extract_lets
    ...
    have h_opcode : (Main[28] * 27 + Main[29] * 28 + Main[30] * 29 + Main[31] * 30 + Main[32] * 31 + Main[33] * 32) = 29 :=
      by
        aesop
    ...
    clear cpu_cstrs
    simp [ITypeReaderImmutable.constraints, SP1Constraint.toProp, h_is_blt, h_28, h_29, h_31, h_32, h_33, h_is_real, h_opcode, Opcode.ofNat, Nat.ble, Nat.beq] at reader_cstrs
    ...
    simp [h_is_blt, h_28, h_29, h_31, h_32, h_33, Opcode.ofNat, Nat.ble, Nat.beq] at state_cstrs
    ...
    simp [spec_blt, sp1_blt, execute_BTYPE]
    simpM
    ...
    · rename_i h_lts
      ...
      have h_ltsxt_pc_is_mul4 : (BitVec.ofNat 64 (Main[3].val + Main[4].val * 65536 + Main[5].val * 4294967296) + sign_extend imm) % 4 = 0 := by
        ...
        · simp [sign_extend, Sail.BitVec.signExtend, imm, sp1_imm]
          ...
          omega
      obtain ⟨h_ltsxt_pc_b0, h_ltsxt_pc_b1⟩ := mul4_means_0_1_are_0 h_ltsxt_pc_is_mul4
      ...
      simp [h_neq, BitVec.slt, h_lts, h_is_blt, h_31] at spec_lt
      ...
      simp [h_is_blt, h_neq, h_is_lt, h_is_neq, h_28, h_29, h_31, h_32, h_33, Opcode.ofNat, Nat.ble, Nat.beq] at chip_cstrs

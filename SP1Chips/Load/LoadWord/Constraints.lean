...
@[irreducible] def constraints (Main : Vector (Fin BB) 44) : SP1ConstraintList :=
  let E0 : Fin BB := Main[1] * 65536
  let E1 : Fin BB := Main[2] + E0
  let E2 : Fin BB := Main[42] * 21
  let E3 : Fin BB := Main[43] * 44
  let E4 : Fin BB := E2 + E3
  let E5 : Fin BB := Main[42] * 2
  let E6 : Fin BB := Main[43] * 6
  let E7 : Fin BB := E5 + E6
  ...
  let ⟨⟨⟨[E21, E22, E23]⟩, _⟩, CS0⟩ := AddressOperation.constraints #v[Main[15], Main[16], Main[17], Main[18]] #v[Main[21], Main[22], Main[23], Main[24]] 0 0 Main[38] E14 { addr_operation := { value := #v[Main[25], Main[26], Main[27]] }, top_two_limb_inv := Main[28] }
  ...
  let E47 : Fin BB := Main[39] - Main[29]
  let E48 : Fin BB := E46 * E47
  let E49 : Fin BB := Main[38] - 1
  let E50 : Fin BB := Main[40] - Main[30]
  let E51 : Fin BB := E49 * E50
  let E52 : Fin BB := Main[39] - Main[31]
  let E53 : Fin BB := Main[38] * E52
  let E54 : Fin BB := Main[40] - Main[32]
  let E55 : Fin BB := Main[38] * E54
  let CS1 : SP1ConstraintList := U16MSBOperation.constraints Main[40] { msb := Main[41] } Main[42]
  let E56 : Fin BB := Main[42] - 1
  let E57 : Fin BB := E56 * Main[41]
  let E58 : Fin BB := Main[3] + 4
  let CS2 : SP1ConstraintList := CPUState.constraints { clk_high := Main[0], clk_16_24 := Main[1], clk_0_16 := Main[2], pc := #v[Main[3], Main[4], Main[5]] } #v[E58, Main[4], Main[5]] 8 E14
  let E59 : Fin BB := 65535 * Main[41]
  let E60 : Fin BB := 65535 * Main[41]
  let CS3 : SP1ConstraintList := ITypeReader.constraints Main[0] E1 #v[Main[3], Main[4], Main[5]] E4 #v[E13, E7, E10] #v[Main[39], Main[40], E59, E60] { op_a := Main[6], op_a_memory := { prev_value := #v[Main[7], Main[8], Main[9], Main[10]], access_timestamp := { prev_low := Main[11], diff_low_limb := Main[12] } }, op_a_0 := Main[13], op_b := Main[14], op_b_memory := { prev_value := #v[Main[15], Main[16], Main[17], Main[18]], access_timestamp := { prev_low := Main[19], diff_low_limb := Main[20] } }, op_c_imm := #v[Main[21], Main[22], Main[23], Main[24]] } E14
  CS0 ++ CS1 ++ CS2 ++ CS3 ++ [
    (.assertZero E16),
    (.assertZero E18),
    (.assertZero E20),

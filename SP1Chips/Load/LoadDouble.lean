...
def spec_ld (imm : BitVec 12) (rs2 rs1 : regidx) : SailM ExecutionResult :=
  do
    Sail.writeReg Register.nextPC ((← Sail.readReg Register.PC) + 4#64)
    execute_LOAD imm rs2 rs1 false 8
...
def sp1_op_a : BitVec 5 :=
  by
    ...
    simp [constraints, AddressOperation.constraints, AddrAddOperation.constraints, CPUState.constraints, ITypeReader.constraints, SP1Constraint.toProp, List.Forall, h_is_real] at cstrs
    simp_all only
...
def sp1_op_b : BitVec 5 :=
  by
    ...
    simp [constraints, AddressOperation.constraints, AddrAddOperation.constraints, CPUState.constraints, ITypeReader.constraints, SP1Constraint.toProp, List.Forall, h_is_real, Opcode.ofNat, ByteOpcode.ofNat, Nat.beq, Nat.ble] at cstrs
    simp_all only

def sp1_imm : BitVec 12 := BitVec.ofNat 12 Main[21].val

def sp1_ld : SailM ExecutionResult :=
  do
...
theorem correct
  ...
  : let op_a := sp1_op_a Main cstrs h_is_real
    ...
    (spec_ld op_c (.Regidx op_b) (.Regidx op_a)).run s = (sp1_ld Main cstrs h_is_real).run s
  := by
    extract_lets op_a op_b op_c

    simp [-Word.add_toBitVec64_mod4, constraints, AddressOperation.constraints, AddrAddOperation.constraints, CPUState.constraints, ITypeReader.constraints, SP1Constraint.toStateProp, List.Forall, h_is_real, Opcode.ofNat, ByteOpcode.ofNat, Nat.beq, Nat.ble] at state_cstrs
    obtain ⟨h_read_pc, ⟨h_trusted_read, h_read_addr_within_range⟩, h_read_op_a, h_read_op_b, h_read_mem⟩ := state_cstrs

    simp [constraints, AddressOperation.constraints, SP1Constraint.toProp, List.Forall, h_is_real, Opcode.ofNat, ByteOpcode.ofNat, Nat.beq, Nat.ble, sub_eq_zero] at cstrs
    obtain ⟨addr_add_cstrs, addr_cstr0, addr_cstr1, cpu_cstrs, reader_cstrs, chip_cstrs⟩ := cstrs
    ...
    have h_is_aligned : is_aligned_vaddr (virtaddr.Virtaddr (Word.toBitVec64 #v[Main[15], Main[16], Main[17], Main[18]] + BitVec.signExtend 64 op_c)) 8 = true := by
    ...
    simp [Word.toNat] at h_read_limb0 h_read_limb1 h_read_limb2 h_read_limb3
    obtain ⟨h_limb0_mem0, h_limb0_mem1⟩ := h_read_limb0
    obtain ⟨h_limb1_mem2, h_limb1_mem3⟩ := h_read_limb1
    obtain ⟨h_limb2_mem4, h_limb2_mem5⟩ := h_read_limb2
    obtain ⟨h_limb3_mem6, h_limb3_mem7⟩ := h_read_limb3

    obtain
      ⟨ h_read_mem0, h_read_mem1, h_read_mem2, h_read_mem3, h_read_mem4, h_read_mem5, h_read_mem6, h_read_mem7 ⟩
      : s.mem[(Word.toBitVec64 #v[Main[15], Main[16], Main[17], Main[18]] + signExtend 64 op_c).toNat]? = some (BitVec.truncate 8 (BitVec.ofNat 16 ↑Main[29]))
        ...
        ∧ s.mem[(Word.toBitVec64 #v[Main[15], Main[16], Main[17], Main[18]] + signExtend 64 op_c).toNat + 2]? = some (BitVec.truncate 8 (BitVec.ofNat 16 ↑Main[30]))
        ∧ s.mem[(Word.toBitVec64 #v[Main[15], Main[16], Main[17], Main[18]] + signExtend 64 op_c).toNat + 3]? = some (BitVec.truncate 8 ((BitVec.ofNat 16 ↑Main[30]) >>> 8))
        ∧ s.mem[(Word.toBitVec64 #v[Main[15], Main[16], Main[17], Main[18]] + signExtend 64 op_c).toNat + 4]? = some (BitVec.truncate 8 (BitVec.ofNat 16 ↑Main[31]))
        ∧ s.mem[(Word.toBitVec64 #v[Main[15], Main[16], Main[17], Main[18]] + signExtend 64 op_c).toNat + 5]? = some (BitVec.truncate 8 ((BitVec.ofNat 16 ↑Main[31]) >>> 8))
        ∧ s.mem[(Word.toBitVec64 #v[Main[15], Main[16], Main[17], Main[18]] + signExtend 64 op_c).toNat + 6]? = some (BitVec.truncate 8 (BitVec.ofNat 16 ↑Main[32]))
        ∧ s.mem[(Word.toBitVec64 #v[Main[15], Main[16], Main[17], Main[18]] + signExtend 64 op_c).toNat + 7]? = some (BitVec.truncate 8 ((BitVec.ofNat 16 ↑Main[32]) >>> 8))
        := by
          simp only [op_c, sp1_imm, sign_extend, Sail.BitVec.signExtend]
          rw [←h_op_c_is_signExtend, ←h_addr_add_spec]
          clear * - h_limb0_mem0 h_limb0_mem1 h_limb1_mem2 h_limb1_mem3 h_limb2_mem4 h_limb2_mem5 h_limb3_mem6 h_limb3_mem7 h_add_addr_limb0 h_add_addr_limb1 h_add_addr_limb2
    ...
    simp only [
      ...
      at h_read_mem0 h_read_mem1 h_read_mem2 h_read_mem3
         h_read_mem4 h_read_mem5 h_read_mem6 h_read_mem7

    simp [-BitVec.toNat_add, spec_ld, execute_LOAD, Sail.readReg, PreSail.readReg, h_read_pc, Sail.assert, PreSail.assert,
         ...
         h_read_mem1, h_read_mem2, h_read_mem3, h_read_mem4, h_read_mem5, h_read_mem6, h_read_mem7,
         MemoryOpResult_drop_meta, h_op_a_not_x0, misaligned_order,
         sys_misaligned_order_decreasing, extend_value, sign_extend, Sail.BitVec.signExtend, sp1_ld, op_a, sp1_op_a,

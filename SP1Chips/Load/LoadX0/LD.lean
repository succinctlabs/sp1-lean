...
def sp1_op_a : BitVec 5 :=
  by
    ...
    show Main[6] < 32
    have ⟨h_lb, h_lbu, h_lh, h_lhu, h_lw, h_lwu, h_ld⟩ := (h_exactly_one Main cstrs).2.2.2.2.2.2 h_is_ld
...
def sp1_ld : SailM ExecutionResult := do
    let op_a := sp1_op_a Main cstrs h_is_ld
...
theorem correct_x0
  ...
  : let op_a := sp1_op_a Main cstrs h_is_ld
    let op_b := sp1_op_b Main cstrs h_is_ld
    let op_c := sp1_imm Main
    (spec_ld op_c (.Regidx op_b) (.Regidx op_a)).run s = (sp1_ld Main cstrs h_is_ld).run s
  := by
    ...
    simp [constraints, AddressOperation.constraints, SP1Constraint.toProp, List.Forall, h_is_ld, h_lb, h_lbu, h_lh, h_lhu, h_lw, h_lwu, h_ld, Opcode.ofNat, ByteOpcode.ofNat, Nat.beq, Nat.ble, sub_eq_zero] at cstrs
    ...
    have h_addr_no_shift1 : Main[39] = 0 := by simp_all only
    simp [h_addr_no_shift1] at addr_cstr1
    have h_addr_no_shift2 : Main[40] = 0 := by simp_all only
    simp [h_addr_no_shift2] at addr_cstr1
    ...
    simp [not_and_or.mpr h_over_addr, Word.toNat, h_addr_no_shift0, h_addr_no_shift1, h_addr_no_shift2] at h_read_mem
    ...
    simp [-BitVec.toNat_add, spec_ld, execute_LOAD, Sail.readReg, PreSail.readReg, h_read_pc, Sail.assert, PreSail.assert,
         ...
         read_ram, Sail.sail_mem_read, PreSail.sail_mem_read, PreSail.readBytes, PreSail.readByte,
         h_read_mem0, h_read_mem1, h_read_mem2, h_read_mem3, h_read_mem4, h_read_mem5, h_read_mem6, h_read_mem7,

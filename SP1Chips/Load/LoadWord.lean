...
def sp1_lw : SailM ExecutionResult :=
  do
    ...
    Sail.write_reg op_a (Word.toBitVec64 #v[Main[39], Main[40], Main[41] * 65536, Main[41] * 65536])
    pure RETIRE_SUCCESS
...
theorem correct
  ...
  := by
    ...
    have h_op_a_is_reg : Main[6] < 32 :=
      by
        simp [ITypeReader.constraints, SP1Constraint.toProp, List.Forall, h_is_lw, h_not_lwu, Opcode.ofNat, ByteOpcode.ofNat, Nat.beq, Nat.ble] at reader_cstrs
        simp_all only
    ...
    simp [h_has_shift] at chip_cstrs h_read_mem addr_cstr2
    ...
    obtain ⟨h_add_addr_limb0, h_add_addr_limb1, h_add_addr_limb2, h_addr_add_spec⟩ :=
      ...
      (Word.isU64_of_cases _
        (by simp; show Main[21] < 65536; simp_all only [reader_cstrs]) 
        (by simp; show Main[22] < 65536; simp_all only [reader_cstrs]) 
        (by simp; show Main[23] < 65536; simp_all only [reader_cstrs]) 
        (by simp; show Main[24] < 65536; simp_all only [reader_cstrs]))
    ...
    -- TODO(gzgz): this will come from the cursed mod thingy
    have h_addr_limb0_ge4 : Main[25] >= 4 :=
      by
        by_contra!
    ...
    have h_read_mem0 : s.mem[(Word.toBitVec64 #v[Main[15], Main[16], Main[17], Main[18]] + sign_extend op_c).toNat]? = some (BitVec.truncate 8 (BitVec.ofNat 16 ↑Main[31])) := by
      ...
      calc 
        ...
        _ = ↑Main[25] + ↑Main[26] * 65536 + ↑Main[27] * 4294967296 := by simp

    have h_read_mem1 : s.mem[(Word.toBitVec64 #v[Main[15], Main[16], Main[17], Main[18]] + sign_extend op_c).toNat + 1]? = some (BitVec.truncate 8 ((BitVec.ofNat 16 ↑Main[31]) >>> 8)) := by
      simp only [op_c, sp1_imm, sign_extend, Sail.BitVec.signExtend]
      rw [←h_op_c_is_signExtend]
      rw [←h_addr_add_spec]
    ...
    have h_read_mem2 : s.mem[(Word.toBitVec64 #v[Main[15], Main[16], Main[17], Main[18]] + sign_extend op_c).toNat + 2]? = some (BitVec.truncate 8 ((BitVec.ofNat 16 ↑Main[32]))) := by sorry

    have h_read_mem3 : s.mem[(Word.toBitVec64 #v[Main[15], Main[16], Main[17], Main[18]] + sign_extend op_c).toNat + 3]? = some (BitVec.truncate 8 (((BitVec.ofNat 16 ↑Main[32])) >>> 8)) := by sorry
    ...
    have h_op_a_not_x0 : op_a ≠ 0#5 := by
      ...
      have h_imm_c_iff_op_a_x0 : Main[13] = 1 ↔ Main[6] = 0 := by simp_all only [reader_cstrs]
      rw [Fin.mk_eq_mk]
      simp
    ...
    simp [op_a] at h_op_a_not_x0
    simp [sp1_lw, Sail.writeReg, PreSail.writeReg, Sail.write_reg, h_op_a_not_x0]
    ...
    have h_pc_is_u64 : Main[3].val + Main[4].val * 65536 + Main[5].val * 4294967296 < 2^64 := by
      simp
    ...
    have : (↑(Main[3] + 4) + ↑Main[4] * 65536 + ↑Main[5] * 4294967296 : ℕ) = ↑Main[3] + ↑Main[4] * 65536 + ↑Main[5] * 4294967296 + 4 := by
      simp [Fin.add_def]
...
lemma isprime : Nat.Prime 2013265921 := by native_decide

instance : Lean.Grind.NoNatZeroDivisors (ZMod 2013265921) where

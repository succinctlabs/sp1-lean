...
@[irreducible] def constraints {F : Type} [Field F] [CoeHead F ℕ] (Main : Vector F 44) : SP1ConstraintList F :=
  ...
  let E50 : F := Main[38] - 1
  let E51 : F := Main[39] - 1
  let E52 : F := Main[40] - Main[29]
  let E53 : F := E51 * E52
  let E54 : F := E50 * E53
  let E55 : F := Main[39] - 1
  let E56 : F := Main[40] - Main[30]
  let E57 : F := E55 * E56
  let E58 : F := Main[38] * E57
  let E59 : F := Main[38] - 1
  let E60 : F := Main[40] - Main[31]
  let E61 : F := Main[39] * E60
  let E62 : F := E59 * E61
  let E63 : F := Main[40] - Main[32]
  let E64 : F := Main[39] * E63
  let E65 : F := Main[38] * E64
  let E66 : F := Main[43] * Main[41]
  let CS1 : SP1ConstraintList F := U16MSBOperation.constraints Main[40] { msb := Main[41] } Main[42]
  let E67 : F := Main[3] + 4
  let CS2 : SP1ConstraintList F := CPUState.constraints { clk_high := Main[0], clk_16_24 := Main[1], clk_0_16 := Main[2], pc := #v[Main[3], Main[4], Main[5]] } #v[E67, Main[4], Main[5]] 8 E17
  let E68 : F := 65535 * Main[41]
  let E69 : F := 65535 * Main[41]
  let E70 : F := 65535 * Main[41]
  let CS3 : SP1ConstraintList F := ITypeReader.constraints Main[0] E1 #v[Main[3], Main[4], Main[5]] E4 #v[E16, E13, E7, E10] #v[Main[40], E68, E69, E70] { op_a := Main[6], op_a_memory := { prev_value := #v[Main[7], Main[8], Main[9], Main[10]], access_timestamp := { prev_low := Main[11], diff_low_limb := Main[12] } }, op_a_0 := Main[13], op_b := Main[14], op_b_memory := { prev_value := #v[Main[15], Main[16], Main[17], Main[18]], access_timestamp := { prev_low := Main[19], diff_low_limb := Main[20] } }, op_c_imm := #v[Main[21], Main[22], Main[23], Main[24]] } E17
  CS0 ++ CS1 ++ CS2 ++ CS3 ++ [
    (.assertZero E19),
    (.assertZero E21),
    (.assertZero E23),
...
lemma allHold_constraints_iff_of_is_lh (h_is_lh : is_lh Main) :
  List.Forall SP1Constraint.toProp (constraints Main) ↔
    ...
    ↑((Main[25] - 4 * Main[39] - 2 * Main[38]) * (8 : Fin KB)⁻¹) < 8192 ∧
...
@[simp] def is_lh_poly (Main : Vector (ZMod p) 44) : Prop := Main[42] = 1
@[simp] def is_lhu_poly (Main : Vector (ZMod p) 44) : Prop := Main[43] = 1
  ...
  List.Forall SP1Constraint.toProp_poly (constraints Main) ↔
    ...
    List.Forall SP1Constraint.toProp_poly
      (ITypeReader.constraints Main[0] (Main[2] + Main[1] * 65536)
        #v[Main[3], Main[4], Main[5]] 30 #v[4, 3, 1, 0]
...
lemma allHold_constraints_iff_of_is_lhu_poly (Main : Vector (ZMod p) 44)
  ...
  List.Forall SP1Constraint.toProp_poly (constraints Main) ↔
    ...
    List.Forall SP1Constraint.toProp_poly
      (ITypeReader.constraints Main[0] (Main[2] + Main[1] * 65536)
        #v[Main[3], Main[4], Main[5]] 33 #v[4, 3, 5, 0]
        #v[Main[40], 65535 * Main[41], 65535 * Main[41], 65535 * Main[41]]
        { op_a := Main[6], op_a_memory :=
        { prev_value := #v[Main[7], Main[8], Main[9], Main[10]],
          access_timestamp := { prev_low := Main[11], diff_low_limb := Main[12] } },

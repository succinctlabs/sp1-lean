...
@[simp] def is_lb (Main : Vector (ZMod p) 47) : Prop := Main[45] = 1
@[simp] def is_lbu (Main : Vector (ZMod p) 47) : Prop := Main[46] = 1
...
lemma allHold_constraints_iff_of_is_lb (Main : Vector (ZMod p) 47)
  ...
  List.Forall SP1Constraint.toProp (constraints Main) ↔
    ...
    Main[43] = Main[38] * ((Main[41] - Main[42]) * (256 : ZMod p)⁻¹) +
      (1 - Main[38]) * Main[42] ∧
    (Main[44] < (256 : ZMod p) ∧ Main[43] < (256 : ZMod p)) ∧
    (Main[44] = 0 ∨ Main[44] = 1) ∧
    (Main[44] = 1 ↔ (128 : ZMod p) ≤ Main[43])) := by

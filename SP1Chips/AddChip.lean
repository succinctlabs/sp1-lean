...
--   let CS0 : SP1ConstraintList := AddOperation.constraints #v[Main[15], Main[16]] #v[Main[22], Main[23]] { value := #v[Main[28], Main[29]] } Main[32]
...
--   let CS2 : SP1ConstraintList := RTypeReader.constraints Main[0] E4 #v[Main[3], Main[4], Main[5]] 0 #v[Main[28], Main[29]] { op_a := Main[6], op_a_memory := { prev_value := #v[Main[7], Main[8]], access_timestamp := { prev_low := Main[11], diff_low_limb := Main[12] } }, op_a_0 := Main[13], op_b := Main[14], op_b_memory := { prev_value := #v[Main[15], Main[16]], access_timestamp := { prev_low := Main[19], diff_low_limb := Main[20] } }, op_c := Main[21], op_c_memory := { prev_value := #v[Main[22], Main[23]], access_timestamp := { prev_low := Main[26], diff_low_limb := Main[27] } } } Main[32]
--   [
--     (.assertZero E1),
--   ] ++ CS0 ++ CS1 ++ CS2

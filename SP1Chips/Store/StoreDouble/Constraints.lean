...
@[irreducible] def constraints {F : Type} [Field F] [CoeHead F ℕ] (Main : Vector F 39) : SP1ConstraintList F :=
  ...
  CS0 ++ CS1 ++ CS2 ++ [
    ...
    (.receive (.memory Main[0] E7 E4 E5 E6 Main[7] Main[8] Main[9] Main[10]) Main[38]),
  ]
...
@[simp] def is_real_poly (Main : Vector (ZMod p) 39) : Prop := Main[38] = 1

...
noncomputable def spec_sb (imm : BitVec 12) (rs1 rs2 : regidx) : SailM ExecutionResult := do
  ...
  let width : ℕ := 2 -- two bytes
  execute_STORE imm rs1 rs2 width

def sp1_op_a (Main : Vector (ZMod p) 45) : BitVec 5 :=
  BitVec.ofNat 5 Main[6].val

def sp1_ob_b (Main : Vector (ZMod p) 45) : BitVec 5 :=
  BitVec.ofNat 5 Main[14].val

def sp1_imm_c (Main : Vector (ZMod p) 45) : BitVec 12 :=
  BitVec.ofNat 12 (Word.toNat #v[Main[21], Main[22], Main[23], Main[24]])

def sp1_sb (Main : Vector (ZMod p) 45) : SailM ExecutionResult := do
...
theorem correct (Main : Vector (ZMod p) 45)
    ...
    (h_cstrs : (StoreHalf.constraints Main).allHold)
    (state_cstrs : (StoreHalf.constraints Main).initialState s)
    (h_is_real : Main[44] = 1)
    (h_fits_in_mem :
    ...
    (h_is_aligned : is_aligned_vaddr (virtaddr.Virtaddr
      (Word.toBitVec64 #v[Main[15], Main[16], Main[17], Main[18]] + BitVec.signExtend 64
        (BitVec.ofNat 12 (Word.toNat #v[Main[21], Main[22], Main[23], Main[24]])))) 2 = true) :
    let op_a := sp1_op_a Main
  ...
  obtain ⟨h_add_addr, _h49, _h38, h_top, _h40, _h_cpu, h_reader, _h_cstrs_rest⟩ := h_cstrs
  ...
  have h37_lt : (37 : ℕ) < p := by omega
  have h37_val : (37 : ZMod p).val = 37 := ZMod.val_natCast_of_lt h37_lt
  simp [ITypeReaderImmutable.constraints,
      SP1Constraint.toProp, Opcode.ofNat, Nat.ble, h37_val] at h_reader
  ...
  simp [SP1ConstraintList.initialState, StoreHalf.constraints,
    ...
    Opcode.ofNat, Nat.ble, h_is_real, h6_32, h14_32, h37_val] at state_cstrs
  ...
  have h_in_range :
    ...
    · -- need `addr.toNat + 2 ≤ 2^48`. From `addr.toNat ≤ 2^48 - 1` and `addr.toNat % 2 = 0`
      -- so `addr.toNat ≤ 2^48 - 2`.
      omega
  simp [spec_sb]
  ...
  rw [run_vmem_write_of_width_2' (BitVec.ofNat 5 Main[14].val)
    (Word.toBitVec64 #v[Main[15], Main[16], Main[17], Main[18]])
  ...
  · -- Bullet 1: align the sp1-side `Word.toBitVec64` references and discharge
    ...
    exact store_half_post_vmem_eq s
      ...
      (Word.toBitVec64 #v[Main[15], Main[16], Main[17], Main[18]] +
        BitVec.signExtend 64 imm_c)
      (BitVec.setWidth 16 (Word.toBitVec64 #v[Main[7], Main[8], Main[9], Main[10]]))

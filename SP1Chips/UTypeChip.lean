...
theorem correct_lui
    (Main : Vector (Fin KB) 31) (s : SailState) (imm : BitVec 20)
    (cstrs : (constraints Main).allHold)
    (h_is_real : Main[30] = 1)
    (h_is_lui : Main[29] = 0)
    (h_imm : Word.toBitVec64 #v[Main[14], Main[15], Main[16], Main[17]] =
              BitVec.signExtend 64 (imm +++ 0x000#12))
    (state_cstrs : (constraints Main).initialState s) :
    let rd := sp1_op_a cstrs h_is_real
    (spec_lui imm (.Regidx rd)).run s = (sp1_utype Main cstrs h_is_real).run s := by
  ...
  obtain ⟨_h_trust, h6_lt, h14_lt, h15_lt, h16_lt, h17_lt,
          _h18, _h19, _h20, _h21,
          _h13_bool, h_a0_iff,
...
theorem correct_auipc
    ...
    (state_cstrs : (constraints Main).initialState s) :
    let rd := sp1_op_a cstrs h_is_real
    (spec_auipc imm (.Regidx rd)).run s = (sp1_utype Main cstrs h_is_real).run s := by
  simp only []
  -- Split the constraints into the four pieces.
  simp [constraints] at cstrs
  obtain ⟨_, add_op_cstrs, reader_cstrs, rest⟩ := cstrs

...
theorem u16_composition
  ...
  := by
    ...
    obtain ⟨u16msb1_cstrs, u16msb2_cstrs, u16msb3_cstrs, cpu_cstrs, alu_cstrs, chip_cstrs⟩ := cstrs

    -- First, establish that the sum of selectors is either 0 or 1
    have h_sum_bool : Main[64] + Main[65] + Main[66] + Main[67] = 0 ∨ 
    ...
    have h_main64_bool : Main[64] = 0 ∨ Main[64] = 1 := by
      simp [SP1Constraint.toProp, sub_eq_zero] at chip_cstrs
      extract_from_and chip_cstrs
    have h_main65_bool : Main[65] = 0 ∨ Main[65] = 1 := by
      simp [SP1Constraint.toProp, sub_eq_zero] at chip_cstrs
      extract_from_and chip_cstrs
    have h_main66_bool : Main[66] = 0 ∨ Main[66] = 1 := by
      simp [SP1Constraint.toProp, sub_eq_zero] at chip_cstrs
      extract_from_and chip_cstrs
    have h_main67_bool : Main[67] = 0 ∨ Main[67] = 1 := by
      simp [SP1Constraint.toProp, sub_eq_zero] at chip_cstrs
      extract_from_and chip_cstrs

    -- Since sum = 1 and each selector is 0 or 1, exactly one is 1
    have h_exactly_one : (Main[64] = 1 ∧ Main[65] = 0 ∧ Main[66] = 0 ∧ Main[67] = 0) ∨ 
                         (Main[64] = 0 ∧ Main[65] = 1 ∧ Main[66] = 0 ∧ Main[67] = 0) ∨
                         (Main[64] = 0 ∧ Main[65] = 0 ∧ Main[66] = 1 ∧ Main[67] = 0) ∨
                         (Main[64] = 0 ∧ Main[65] = 0 ∧ Main[66] = 0 ∧ Main[67] = 1) := by
      cases h_main64_bool <;> rename_i h64
      <;> cases h_main65_bool <;> rename_i h65  
      <;> cases h_main66_bool <;> rename_i h66
      <;> cases h_main67_bool <;> rename_i h67
      <;> simp [h64, h65, h66, h67] at h_sum_one
      <;> aesop
    ...
    · intro h_is_real
      ...
      · -- These come from the output of the shift operation
        -- The bounds depend on the specific shift right operation specification
        -- TODO: this comes from the spec of ShiftRight operations, which we don't have yet
        sorry
    ...
    · intro h_is_real
      ...
      rcases h_exactly_one with h_sel | h_sel | h_sel | h_sel
      -- Now simp with the concrete selector values to expand the opcode
      all_goals {
        obtain ⟨h_64, h_65, h_66, h_67⟩ := h_sel
        simp [h_64, h_65, h_66, h_67] at alu_cstrs
        simp [ALUTypeReader.constraints, SP1Constraint.toProp, h_sum_one, h_64, h_65, h_66, h_67, Opcode.ofNat, Nat.ble, Nat.beq, List.Forall] at alu_cstrs
        refine ⟨?_, ?_⟩
    ...
    · intro h_imm_c_ne
      ...
      all_goals {
        ...
        simp [h_64, h_65, h_66, h_67, h_sum_one] at h_imm_c_ne

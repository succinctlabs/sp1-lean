...
@[irreducible] def constraints {F : Type} [Field F] [CoeHead F ℕ] (Main : Vector F 31) : SP1ConstraintList F :=
  ...
  let CS2 : SP1ConstraintList F := JTypeReader.constraints Main[0] E46 #v[Main[3], Main[4], Main[5]] E7 #v[Main[25], Main[26], Main[27], Main[28]] { op_a := Main[6], op_a_memory := { prev_value := #v[Main[7], Main[8], Main[9], Main[10]], access_timestamp := { prev_low := Main[11], diff_low_limb := Main[12] } }, op_a_0 := Main[13], op_b_imm := #v[Main[14], Main[15], Main[16], Main[17]], op_c_imm := #v[Main[18], Main[19], Main[20], Main[21]] } Main[30] Main[30]
  CS0 ++ CS1 ++ CS2 ++ [
    (.assertZero E1),
    (.assertZero E3),
    (.assertZero E38),

...
variable (Main : Vector (Fin KB) 83)
def is_real : Prop := Main[78] = 1 ∨ Main[79] = 1 ∨ Main[80] = 1 ∨ Main[81] = 1 ∨ Main[82] = 1

section constraints

-- Generated Lean code for chip MulChip
@[irreducible] def constraints (Main : Vector (Fin KB) 83) : SP1ConstraintList :=
  let E0 : Fin KB := Main[78] + Main[79]
  let E1 : Fin KB := E0 + Main[80]
  let E2 : Fin KB := E1 + Main[81]
  let E3 : Fin KB := E2 + Main[82]
  ...
  let E4 : Fin KB := Main[78] - 1
  let E5 : Fin KB := Main[78] * E4
  let E6 : Fin KB := Main[79] - 1
  let E7 : Fin KB := Main[79] * E6
  let E8 : Fin KB := Main[80] - 1
  let E9 : Fin KB := Main[80] * E8
  let E10 : Fin KB := Main[82] - 1
  let E11 : Fin KB := Main[82] * E10
  let E12 : Fin KB := Main[81] - 1
  let E13 : Fin KB := Main[81] * E12
  let E14 : Fin KB := E3 - 1
  let E15 : Fin KB := E3 * E14
  let E16 : Fin KB := Main[78] * 11
  let E17 : Fin KB := Main[79] * 12
  let E18 : Fin KB := E16 + E17
  let E19 : Fin KB := Main[80] * 13
  let E20 : Fin KB := E18 + E19
  let E21 : Fin KB := Main[81] * 14
  let E22 : Fin KB := E20 + E21
  let E23 : Fin KB := Main[82] * 47
  let E24 : Fin KB := E22 + E23
  let E25 : Fin KB := Main[78] * 0
  let E26 : Fin KB := Main[79] * 1
  let E27 : Fin KB := E25 + E26
  let E28 : Fin KB := Main[80] * 3
  let E29 : Fin KB := E27 + E28
  let E30 : Fin KB := Main[81] * 2
  let E31 : Fin KB := E29 + E30
  let E32 : Fin KB := Main[82] * 0
  let E33 : Fin KB := E31 + E32
  let E34 : Fin KB := Main[78] * 1
  let E35 : Fin KB := Main[79] * 1
  let E36 : Fin KB := E34 + E35
  let E37 : Fin KB := Main[80] * 1
  let E38 : Fin KB := E36 + E37
  let E39 : Fin KB := Main[81] * 1
  let E40 : Fin KB := E38 + E39
  let E41 : Fin KB := Main[82] * 1
  let E42 : Fin KB := E40 + E41
  let E43 : Fin KB := Main[78] * 51
  let E44 : Fin KB := Main[79] * 51
  let E45 : Fin KB := E43 + E44
  let E46 : Fin KB := Main[80] * 51
  let E47 : Fin KB := E45 + E46
  let E48 : Fin KB := Main[81] * 51
  let E49 : Fin KB := E47 + E48
  let E50 : Fin KB := Main[82] * 59
  let E51 : Fin KB := E49 + E50
  let E52 : Fin KB := Main[78] * 8
  let E53 : Fin KB := Main[79] * 8
  let E54 : Fin KB := E52 + E53
  let E55 : Fin KB := Main[80] * 8
  let E56 : Fin KB := E54 + E55
  let E57 : Fin KB := Main[81] * 8
  let E58 : Fin KB := E56 + E57
  let E59 : Fin KB := Main[82] * 8

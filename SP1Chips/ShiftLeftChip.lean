...
def sp1_shift_left (Main : Vector (ZMod p) 65) : SailM Unit := do
...
theorem correct_sll
    ...
    let op_a := sp1_op_a Main
    (spec_sll (.Regidx op_c) (.Regidx op_b) (.Regidx op_a)).run s =
      (sp1_shift_left Main).run s := by
  ...
  have spec_eq := spec.sll Main ⟨eq_sll, eq_imm⟩ cstrs
  ...
  · -- a = 0 case: a's are all 0, so shift_result = 0; both spec/sp1 just bump PC.
    ...
    have h_shift_zero : (Word.toBitVec64 #v[Main[15], Main[16], Main[17], Main[18]] <<<
        ((Word.toBitVec64 #v[Main[25], Main[26], Main[27], Main[28]]).toNat % 64) : BitVec 64) =
          0#64 := by
      rw [← spec_eq]
...
theorem correct_slli
  ...
  have spec_eq := spec.slli Main ⟨eq_sll, eq_imm⟩ cstrs
  have hp : 2 ^ 17 < p := Fact.out
  -- Monadic chain.
  simp only [BitVec.ofNatLT_eq_ofNat] at *
  ...
  -- Bridge SHIFTIOP_pure to RTYPE_pure_w (before by_cases; preserves Fact instance).
...
theorem correct_sllw
    ...
    let op_b := sp1_op_b Main
    let op_a := sp1_op_a Main
    (spec_sllw (.Regidx op_c) (.Regidx op_b) (.Regidx op_a)).run s =
      (sp1_shift_left Main).run s := by
  haveI : NeZero p := ⟨by have := Fact.out (p := 2 ^ 17 < p); omega⟩
  ...
  rw [Sail.run_readReg, read_pc]
  simp [sp1_op_a, sp1_op_b, sp1_op_c, read_op_b, read_op_c]
  have spec_eq := spec.sllw Main ⟨eq_sllw, eq_imm⟩ cstrs
  have hp : 2 ^ 17 < p := Fact.out
  ...
  · -- a = 0 case: a's are all 0, so shift_result = 0; both spec/sp1 just bump PC.
    ...
    have h_shift_zero : BitVec.signExtend 64
        ((Word.low (#v[Main[15], Main[16], Main[17], Main[18]] : Word (ZMod p))).toBitVec32 <<<
          (((Word.low
              (#v[Main[25], Main[26], Main[27], Main[28]] : Word (ZMod p))).toBitVec32).toNat
            % 32)) = 0#64 := by
      rw [← spec_eq]
      simp [Word.toBitVec64, Word.toNat_def, ZMod.val_zero]
    rw [if_pos h_shift_zero]
...
theorem correct_slliw
  ...
  simp [SP1ConstraintList.initialState, constraints, SP1Constraint.toStateProp,
    ...
    h6_lt, h14_lt, h_real, eq_imm] at state_cstrs
  obtain ⟨read_pc, read_op_a, read_op_b⟩ := state_cstrs
  have spec_eq := spec.slliw Main ⟨eq_slliw, eq_imm⟩ cstrs
  have hp : 2 ^ 17 < p := Fact.out
  -- Monadic chain.
  simp only [BitVec.ofNatLT_eq_ofNat] at *
  ...
  have h_shamt_eq : (#v[((BitVec.ofNat 5 Main[21].val).toNat : ZMod p), 0, 0, 0] : Word (ZMod p))
    ...
    rw [h_21_toNat, ZMod.natCast_zmod_val]
    rw [e_25, h_26, h_27, h_28]
  -- Bridge SHIFTIWOP_pure to RTYPEW_pure_w.
  ...
  · -- a = 0 case.
    ...
    have h_shift_zero : BitVec.signExtend 64
      ((Word.low (#v[Main[15], Main[16], Main[17], Main[18]] : Word (ZMod p))).toBitVec32 <<<
        (((Word.low (#v[Main[25], (0 : ZMod p), 0, 0] : Word (ZMod p))).toBitVec32).toNat
          % 32)) = 0#64 := by
      rw [← spec_eq]
      simp [Word.toBitVec64, Word.toNat_def, ZMod.val_zero]
    rw [if_pos h_shift_zero]

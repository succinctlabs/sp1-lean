...
/-! ## Polymorphic correctness theorems (`_poly` companions over `ZMod p`).

Mirror the Fin KB `correct_<variant>` theorems above using the chip-level
`_poly` helpers in `Mul/Constraints.lean` and the op-level
`MulOperation.spec.<variant>_poly` lemmas. -/

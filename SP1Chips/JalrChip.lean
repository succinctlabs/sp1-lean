...
-- Pre-regen proof (Main was size 39; is_trusted at Main[25]; nextPC at Main[31..34]; op_a_val at Main[35..38]):
...
-- To port: is_real now at Main[25] (was Main[30]); is_trusted column gone —
-- the `h25 : Main[25] = 1` step and its downstream uses must be re-derived
-- from the new RTypeReader/ITypeReader iff lemmas; value columns move
-- Main[31..38] → Main[26..33]; the obtain projection chain on `reader_cstrs`
-- needs to be rewalked against the new ITypeReader.constraints shape.
theorem JALR_correct
    (cstrs : (constraints Main).allHold)
    (h_is_real : Main[25] = 1)
    (hs : isInitialized s)

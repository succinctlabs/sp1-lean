...
variable (Main : Vector (Fin KB) 52)
...
@[irreducible] def constraints (Main : Vector (Fin KB) 52) : SP1ConstraintList :=
  let E0 : Fin KB := Main[49] + Main[50]
  let E1 : Fin KB := E0 + Main[51]
  let E2 : Fin KB := Main[49] - 1
  let E3 : Fin KB := Main[49] * E2
  let E4 : Fin KB := Main[50] - 1
  let E5 : Fin KB := Main[50] * E4
  let E6 : Fin KB := Main[51] - 1
  let E7 : Fin KB := Main[51] * E6
  let E8 : Fin KB := E1 - 1
  let E9 : Fin KB := E1 * E8
  let E10 : Fin KB := Main[49] * 2
  let E11 : Fin KB := Main[50] * 1
  let E12 : Fin KB := E10 + E11
  let E13 : Fin KB := Main[51] * 0
  let E14 : Fin KB := E12 + E13
  let E15 : Fin KB := Main[49] * 3
  let E16 : Fin KB := Main[50] * 4
  let E17 : Fin KB := E15 + E16
  let E18 : Fin KB := Main[51] * 5
  let E19 : Fin KB := E17 + E18
  let E20 : Fin KB := Main[49] * 4
  let E21 : Fin KB := Main[50] * 6
  let E22 : Fin KB := E20 + E21
  let E23 : Fin KB := Main[51] * 7
  let E24 : Fin KB := E22 + E23
  let E25 : Fin KB := Main[49] * 0
  let E26 : Fin KB := Main[50] * 0
  let E27 : Fin KB := E25 + E26
  let E28 : Fin KB := Main[51] * 0
  let E29 : Fin KB := E27 + E28
  let E30 : Fin KB := Main[49] * 51
  let E31 : Fin KB := Main[50] * 51
  let E32 : Fin KB := E30 + E31
  let E33 : Fin KB := Main[51] * 51
  let E34 : Fin KB := E32 + E33
  let E35 : Fin KB := 32 * Main[31]
  let E36 : Fin KB := E34 - E35
  let E37 : Fin KB := Main[49] * 8
  let E38 : Fin KB := Main[50] * 8
  let E39 : Fin KB := E37 + E38
  let E40 : Fin KB := Main[51] * 8
  let E41 : Fin KB := E39 + E40
  let E42 : Fin KB := 4 * Main[31]
  let E43 : Fin KB := E41 - E42
  let ⟨⟨⟨[E44, E45, E46, E47]⟩, _⟩, CS0⟩ := BitwiseU16Operation.constraints #v[Main[15], Main[16], Main[17], Main[18]] #v[Main[25], Main[26], Main[27], Main[28]] { b_low_bytes := { low_bytes := #v[Main[33], Main[34], Main[35], Main[36]] }, c_low_bytes := { low_bytes := #v[Main[37], Main[38], Main[39], Main[40]] }, bitwise_operation := { result := #v[Main[41], Main[42], Main[43], Main[44], Main[45], Main[46], Main[47], Main[48]] } } E14 E1

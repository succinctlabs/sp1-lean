...
theorem correct (Main : Vector (ZMod p) 45)
    ...
    (h_cstrs : (StoreHalf.constraints Main).allHold_poly)
    (state_cstrs : (StoreHalf.constraints Main).initialState_poly s)
    (h_is_real : Main[44] = 1)
    (h_fits_in_mem :
    ...
    (h_is_aligned : is_aligned_vaddr (virtaddr.Virtaddr
      (Word.toBitVec64 #v[Main[15], Main[16], Main[17], Main[18]] + BitVec.signExtend 64
        (BitVec.ofNat 12 (Word.toNat_poly #v[Main[21], Main[22], Main[23], Main[24]])))) 2 = true) :
    let op_a := sp1_op_a Main
  ...
  simp [SP1ConstraintList.initialState_poly, StoreHalf.constraints,

...
def sp1_op_a (Main : Vector (Fin BB) 47) : BitVec 5 :=
  BitVec.ofNat 5 Main[6]

def sp1_ob_b (Main : Vector (Fin BB) 47) : BitVec 5 :=
  BitVec.ofNat 5 Main[14]

def sp1_imm_c (Main : Vector (Fin BB) 47) : BitVec 12 :=
  BitVec.ofNat 12 (Word.toNat #v[Main[21], Main[22], Main[23], Main[24]])

def sp1_sb (Main : Vector (Fin BB) 47) : SailM ExecutionResult := do
  let op_a := sp1_op_a Main
  Sail.writeReg Register.nextPC (Word.toBitVec64 #v[Main[3] + 4, Main[4], Main[5], 0])
  let addr : BitVec 64 := Word.toBitVec64 #v[Main[26], Main[27], Main[28], 0]
  Sail.write_ram 64 2 0#64 addr (Word.toBitVec64 #v[Main[7], Main[8], Main[9], Main[10]])
...
theorem correct (Main : Vector (Fin BB) 47)
  ...
  obtain ⟨h_add_addr, h49, h38, h39, h40, h_cpu, h_reader, h_cstrs⟩ := h_cstrs

  -- Extract constraints about the state of operations
  simp [StoreHalf.constraints, AddressOperation.constraints,
  ...
  rw [run_vmem_write_of_width_2' (BitVec.ofNat 5 Main[14])

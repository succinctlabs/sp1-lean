...
def sp1_op_a (Main : Vector (Fin BB) 45) : BitVec 5 :=
  BitVec.ofNat 5 Main[6]

def sp1_ob_b (Main : Vector (Fin BB) 45) : BitVec 5 :=
  BitVec.ofNat 5 Main[14]

def sp1_imm_c (Main : Vector (Fin BB) 45) : BitVec 12 :=
  BitVec.ofNat 12 (Word.toNat #v[Main[21], Main[22], Main[23], Main[24]])

def sp1_sb (Main : Vector (Fin BB) 45) : SailM ExecutionResult := do
...
theorem correct (Main : Vector (Fin BB) 45)

...
def sp1_ld (Main : Vector (ZMod p) 39) : SailM ExecutionResult := do
  ...
  Sail.write_reg op_a (Word.toBitVec64_poly #v[Main[29], Main[30], Main[31], Main[32]])
...
-- Polymorphic counterpart of `byteConcat8_toNat_eq_Word_toNat`. Pure
-- Nat-arithmetic on `.val` projections; works under any `[NeZero p]`.
set_option linter.unusedSectionVars false in
private lemma byteConcat8_toNat_eq_Word_toNat_poly [NeZero p]
...
theorem correct_ld (Main : Vector (ZMod p) 39)
    ...
    (h_cstrs : (LoadDouble.constraints Main).allHold_poly)
    (state_cstrs : (LoadDouble.constraints Main).initialState_poly s)
    (h_is_ld : Main[38] = 1)
    (h_fits_in_mem :
  ...
  rw [SP1ConstraintList.allHold_poly,
    allHold_constraints_iff_of_is_ld_poly Main h_is_ld] at h_cstrs
  ...
  simp [SP1ConstraintList.initialState_poly, LoadDouble.constraints,
    ...
    rw [← heq, Word.toBitVec64_poly, BitVec.toNat_ofNat,
      Nat.mod_eq_of_lt (by
        ...
        have h27m : Main[27].val * 4294967296 ≤ 65535 * 4294967296 := by
          ...
          exact Nat.mul_le_mul_right _ this
        omega)]
  -- haddr_plus_poly: shift k into the Main[25] limb
  have haddr_plus :
      ∀ (k : ℕ), k < 8 →
  ...
  · -- Main goal: result equals SP1 write
    have hconcat := byteConcat8_toNat_eq_Word_toNat_poly Main[29] Main[30] Main[31] Main[32]
    ...
    have heq_toNat : (BitVec.ofNat 8 (Main[32].val >>> 8) ++ BitVec.ofNat 8 Main[32].val ++
        ...
        (Word.toBitVec64_poly #v[Main[29], Main[30], Main[31], Main[32]]).toNat := by
      rw [hconcat, Word.toBitVec64_poly, BitVec.toNat_ofNat, Nat.mod_eq_of_lt]
      rw [Word.toNat_poly_def]
      simp only [Vector.getElem_mk, List.getElem_toArray, List.getElem_cons_zero,
        List.getElem_cons_succ]
      have hpow : (2 ^ 64 : ℕ) = 18446744073709551616 := by decide

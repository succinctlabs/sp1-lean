...
-- Polymorphic counterpart of `divu_remu`. The bv_ctqr arm uses a fresh
-- `ZMod.val_add_of_lt`-distribution + Nat carry-eqs approach (the Fin-KB
-- joins/divs/conv chain doesn't translate). 32M heartbeats + skipKernelTC
-- match the Mul precedent for similar 8-way carry decompositions.
...
-- Polymorphic counterpart of `spec.divu`. **STUB** — mechanical port attempted
-- but fails to unify with `divu_remu_poly`'s signature due to two structural
-- mismatches surfaced 2026-05-14:
--
-- 1. **NatCast normalization**: the chip's constraints have `↑1 - is_word` form
--    (NatCast'd `(1 : ℕ)`), while `divu_remu_poly`'s signature uses literal
--    `1 - is_word`. The constraint compiler emits `↑(1 : ℕ)` because the
--    constraint is parametric over `(F : Type)`. Fix needs `push_cast` /
--    `simp only [Nat.cast_one]` normalization at all `eq_lc_i`/`eq_lb_i`
--    hypotheses before threading into `divu_remu_poly`.
--
-- 2. **U16MSB output form mismatch**: `U16MSBOperation.spec.gen_poly` returns
--    `msb = if a.val ≥ 32768 then 1 else 0` (Nat-level comparison on `a.val`),
--    while `divu_remu_poly`'s signature expects
--    `msb = if (32768 : ZMod p) ≤ a then 1 else 0` (ZMod-level comparison).
--    The two are propositionally equal modulo `val_32768_zmod_p` but
--    syntactically different. Fix needs a bridge per `eq_msb_*` hypothesis
--    OR changing `divu_remu_poly`'s signature to use the Nat form.
--
-- The Fin KB `spec.divu` doesn't hit either issue because (a) Fin KB's `↑1`
...
-- Polymorphic counterpart of `spec.remu`. Twin of `spec.divu_poly`; differs
-- only in: `is_remu_poly` flag, `.2` projection (remainder output), `sop4`
-- mutex implication (instead of `sop2`), and `eq_r_*` writeback equations.
set_option debug.skipKernelTC true in
set_option maxHeartbeats 32000000 in
-- 32M heartbeats: divu_remu_poly's 8-limb carry chain + 13 op-level spec applies.
set_option linter.unusedVariables false in

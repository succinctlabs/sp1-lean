...
lemma spec.sll (h : is_sll Main) :
  ...
  := by sorry
    ...
    -- obtain ⟨ msb_a1, cpu, alu, one_of_ops,
    --          b_sll, b_sllw,
    ...
    -- have eq_m63 : sllw = 0 := by
    --   clear *- b_sll b_sllw one_of_ops eq_sll
    --   simp_all; aesop

    -- simp_all

    -- clear msb_a1 cpu
    -- rw [ALUTypeReader.allHold_constraints_iff_is_real (by rfl)] at alu
    -- simp [Opcode.ofNat, Nat.ble, Nat.beq] at alu
    -- obtain ⟨ h0, h1, h2, h3, h4, h5, h6, h7, h8, h9, h10, h11, h12, h13, h14, is_U64_b, ⟨ h15, h16, is_U64_c ⟩, h17 ⟩ := alu
...
lemma spec.slli (h : is_slli Main) :
  List.Forall SP1Constraint.toProp (constraints Main) →
    Main[25] < 64 ∧ Main[26] = 0 ∧ Main[27] = 0 ∧ Main[28] = 0 ∧
    Word.toBitVec64 #v[Main[32], Main[33], Main[34], Main[35]] = execute_RTYPE_pure_w #v[Main[15], Main[16], Main[17], Main[18]] #v[Main[25], Main[26], Main[27], Main[28]] .SLL
  := by sorry
    -- intro cstrs; rw [allHold_constraints_iff] at cstrs
    -- obtain ⟨ eq_sll, eq_imm ⟩ := h
    ...
    -- rw [ALUTypeReader.allHold_constraints_iff_is_real (by rfl)] at alu
    -- simp [Opcode.ofNat, Nat.ble, Nat.beq] at alu
    -- obtain ⟨ ⟨ h0, hc0, hc1, hc2, hc3 ⟩, h1, h2, h3, h4, h5, h6, h7, h8, h9, h10, h11, h12, h13, h14, is_U64_b, h15, eq_c0, eq_c1, eq_c2, eq_c3 ⟩ := alu
    -- clear h0 h1 h2 h3 h4 h5 h6 h7 h8 h9 h10 h11 h12 h13 h14 h15
    -- rw [← eq_c0] at hc0 eq_c0; simp_all

    -- have ⟨ b0_16, b1_16, b2_16, b3_16 ⟩ := Word.lt_cases_of_isU64 is_U64_b
    -- simp_all

    -- have is_U64_b : Word.isU64 #v[ b0, b1, b2, b3 ] := by apply Word.isU64_of_cases <;> assumption
    -- have is_U64_c : Word.isU64 #v[ c0, 0, 0, 0 ] := by apply Word.isU64_of_cases <;> simp; omega
...
lemma spec.sllw (h : is_sllw Main) :
  ...
  := by sorry
    ...
    -- have eq_m63 : sll = 0 := by
    --   clear *- b_sll b_sllw one_of_ops eq_sllw
    --   simp_all; aesop
    ...
    --   simp_all
    --   rw [← w_04] at *
    --   have h_msb := U16MSBOperation.spec a1 { msb := msb } 1 (by assumption) msb_a1 (by simp)
...
lemma spec.slliw (h : is_slliw Main) :
  List.Forall SP1Constraint.toProp (constraints Main) →
    Main[25] < 32 ∧ Main[26] = 0 ∧ Main[27] = 0 ∧ Main[28] = 0 ∧
    Word.toBitVec64 #v[Main[32], Main[33], Main[34], Main[35]] = execute_RTYPEW_pure_w #v[Main[15], Main[16], Main[17], Main[18]] #v[Main[25], Main[26], Main[27], Main[28]] .SLLW
  := by sorry
    -- intro cstrs; rw [allHold_constraints_iff] at cstrs
    -- obtain ⟨ eq_sllw, eq_imm ⟩ := h

...
private lemma spec.sllw_poly_cb4_zero (Main : Vector (ZMod p) 65)
  ...
  rw [hcb4] at h_su161
  simp only [zero_add, mul_zero, add_zero] at h_su161
  have h_46_eq : Main[46] = 0 := by
    rcases h_su161 with h | h
    · exact h
    · exfalso; exact h_1_ne_0 h.symm
  -- Derive h_45_eq.
...
private lemma spec.sllw_poly_cb4_one (Main : Vector (ZMod p) 65)
  ...
  rw [hcb4] at h_su160
  simp only [one_mul, mul_one, mul_zero, add_zero] at h_su160
...
lemma spec.sllw_poly (Main : Vector (ZMod p) 65) (h : is_sllw_poly Main) :
  ...
  have h_cb_sum_5_lt : (Main[36] + Main[37] * 2 + Main[38] * 4 + Main[39] * 8
    ...
    -- Re-derive 5-bit bound; same chain as h_cb_sum_lt steps 1-4.
    have hcb0 : Main[36].val ≤ 1 := by rcases b_cb0 with h | h <;> rw [h]
                                       · rw [h_v0_val]; omega
                                       · rw [h_v1_val]
    have hcb1 : Main[37].val ≤ 1 := by rcases b_cb1 with h | h <;> rw [h]
  ...
  simp only [mul_zero, zero_mul, add_zero, zero_add] at h_su160 h_su161 h_su162 h_su163
  ...
  · exact spec.sllw_poly_cb4_zero Main eq_sllw h_no_sll
  ...
  · exact spec.sllw_poly_cb4_one Main eq_sllw h_no_sll
      b0_16 b1_16 c0_16 c1_16 b_cb0 b_cb1 b_cb2 b_cb3 hcb4

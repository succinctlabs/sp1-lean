...
-- Pre-regen proof (Main was size 32, is_real at Main[31]):
...
--     exact h
-- The projection chain `.2.2.2.2…1` is pinned to the old conjunction layout and
-- doesn't transfer mechanically. The bound `Main[6] < 32` is still derivable from
-- the `.send (.program … (Opcode.ofNat 46) Main[6] …) Main[30]` constraint
-- (trusted_instr for JAL gives op_a < 32) but needs the cstrs tree re-walked.
...
-- Pre-regen proof (Main was size 32; PC+imm at Main[23..26]; PC+4 at Main[27..30]):
...
-- To port: shift Main[22..30] → Main[21..29] (is_trusted gap at old Main[22],
-- value1 moves 23..26→22..25, value2 moves 27..30→26..29); rewalk the
-- projection chain for h22 against the new cstrs tree; all other steps
-- should transfer as-is once indices are shifted.
theorem SP1JAL_correct
    (cstrs : (constraints Main).allHold)
    (h_is_real : Main[30] = 1)
    (state_cstrs : (constraints Main).initialState s)

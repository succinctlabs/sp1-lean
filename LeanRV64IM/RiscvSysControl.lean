...
/-- Type quantifiers: k_ex65568# : Bool -/
...
def check_Counteren (csr : (BitVec 12)) (p : Privilege) : SailM Bool := do
  if (((zopz0zI_u csr (0xC00 : (BitVec 12))) || (zopz0zI_u (0xC1F : (BitVec 12)) csr)) : Bool)
  then (pure true)
  else
    (do
...
def check_Stimecmp (csr : (BitVec 12)) (p : Privilege) : SailM Bool := do
  if (((csr != (0x14D : (BitVec 12))) && (csr != (0x15D : (BitVec 12)))) : Bool)
  then (pure true)
  else
...
/-- Type quantifiers: k_ex66067# : Bool -/
...
/-- Type quantifiers: k_ex66313# : Bool -/

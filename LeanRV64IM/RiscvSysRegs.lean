                  ...
                  (_update_Misa_F
                    (_update_Misa_D
                      (_update_Misa_C
                        (_update_Misa_B
                          (_update_Misa_A m
                            (if (hartSupports Ext_A)
                            then (_get_Misa_A v)
                            else (0b0 : (BitVec 1))))
                          (if (hartSupports Ext_B)
                          then (_get_Misa_B v)
                          else (0b0 : (BitVec 1))))
                        (if (hartSupports Ext_C)
                        then (_get_Misa_C v)
                        else (0b0 : (BitVec 1))))
                      (if ((hartSupports Ext_D) && ((_get_Misa_F v) == (0b1 : (BitVec 1))))
                      then (_get_Misa_D v)
                      else (0b0 : (BitVec 1))))
                    (if (hartSupports Ext_F)
                    then (_get_Misa_F v)
                    else (0b0 : (BitVec 1))))
                  (if (hartSupports Ext_H)
                  then (_get_Misa_H v)
                  else (0b0 : (BitVec 1)))) (0b1 : (BitVec 1)))
              (if (hartSupports Ext_M)
              then (_get_Misa_M v)
              else (0b0 : (BitVec 1))))
            (if ((hartSupports Ext_S) && ((_get_Misa_U v) == (0b1 : (BitVec 1))))
            then (_get_Misa_S v)
            else (0b0 : (BitVec 1))))
          (if (hartSupports Ext_U)
          then (_get_Misa_U v)
          else (0b0 : (BitVec 1))))
        (if ((hartSupports Ext_V) && (((_get_Misa_F v) == (0b1 : (BitVec 1))) && ((_get_Misa_D v) == (0b1 : (BitVec 1)))))
        then (_get_Misa_V v)
        else (0b0 : (BitVec 1)))))
...
def legalize_mseccfg (o : (BitVec 64)) (v : (BitVec 64)) : SailM (BitVec 64) := do
  ...
  (pure (_update_Seccfg_USEED
      (_update_Seccfg_SSEED o
        (if sseed_read_only_zero
        then (0b0 : (BitVec 1))
        else (_get_Seccfg_SSEED v)))
      (if useed_read_only_zero
      then (0b0 : (BitVec 1))
      ...
      (_update_MEnvcfg_CBIE
        (_update_MEnvcfg_CBCFE
          (_update_MEnvcfg_CBZE
            ...
            (← do
              if (← (currentlyEnabled Ext_Zicboz))
              then (pure (_get_MEnvcfg_CBZE v))
              else (pure (0b0 : (BitVec 1)))))
          (← do
            if (← (currentlyEnabled Ext_Zicbom))
            then (pure (_get_MEnvcfg_CBCFE v))
            else (pure (0b0 : (BitVec 1)))))
        (← do
          if (← (currentlyEnabled Ext_Zicbom))
          then
            (if ((_get_MEnvcfg_CBIE v) != (0b10 : (BitVec 2)))
            then (pure (_get_MEnvcfg_CBIE v))
            else (pure (0b00 : (BitVec 2))))
          else (pure (0b00 : (BitVec 2)))))
      (← do
        if (← (currentlyEnabled Ext_Sstc))
...
def legalize_senvcfg (o : (BitVec 64)) (v : (BitVec 64)) : SailM (BitVec 64) := do
  ...
  (pure (_update_SEnvcfg_CBIE
      ...
      (← do
        if (← (currentlyEnabled Ext_Zicbom))
        then
          (if ((_get_SEnvcfg_CBIE v) != (0b10 : (BitVec 2)))
...
def legalize_mip (o : (BitVec 64)) (v : (BitVec 64)) : SailM (BitVec 64) := do
  ...
  (pure (_update_Minterrupts_STI
      ...
      (← do
        ...
        then
          (do
            if ((← (currentlyEnabled Ext_Sstc)) && ((_get_MEnvcfg_STCE (← readReg menvcfg)) == (0b1 : (BitVec 1))))
...
def tvec_addr (m : (BitVec 64)) (c : (BitVec 64)) : (Option (BitVec 64)) :=
  ...
  | TV_Direct => (some base)
  | TV_Vector =>
    (if ((_get_Mcause_IsInterrupt c) == (0b1 : (BitVec 1)))
...
def lift_sip (o : (BitVec 64)) (d : (BitVec 64)) (s : (BitVec 64)) : (BitVec 64) :=
  let m : Minterrupts := o
  if ((_get_Minterrupts_SSI d) == (0b1 : (BitVec 1)))
  ...
  (_update_Minterrupts_SSI
    (_update_Minterrupts_STI
      (_update_Minterrupts_SEI m
        (if ((_get_Minterrupts_SEI d) == (0b1 : (BitVec 1)))
        then (_get_Sinterrupts_SEI s)
        else (_get_Minterrupts_SEI m)))
      (if ((_get_Minterrupts_STI d) == (0b1 : (BitVec 1)))
...
def legalize_satp (arch : Architecture) (prev_value : (BitVec 64)) (written_value : (BitVec 64)) : SailM (BitVec 64) := do
  ...
  | .some Sv_mode =>
    (do
      ...
      | Bare =>
        (do
          if (← (currentlyEnabled Ext_Svbare))
          then (pure s)
          else (pure prev_value))
      | Sv39 =>
        (do
          if (← (currentlyEnabled Ext_Sv39))
          then (pure s)
          else (pure prev_value))
      | Sv48 =>
        (do
          if (← (currentlyEnabled Ext_Sv48))
          then (pure s)
          else (pure prev_value))
      | Sv57 =>
        (do
          if (← (currentlyEnabled Ext_Sv57))
          then (pure s)
          else (pure prev_value))
      | _ => (pure prev_value))
...
def sew_pow_val_forwards (arg_ : (BitVec 3)) : SailM Nat := do
  let b__0 := arg_
  if (b__0 == (0b000 : (BitVec 3)))
  then (pure 3)
  else
    (do
      if (b__0 == (0b001 : (BitVec 3)))
      then (pure 4)
      else
        (do
          if (b__0 == (0b010 : (BitVec 3)))
          then (pure 5)
          else
            (do
              if (b__0 == (0b011 : (BitVec 3)))
              then (pure 6)
              else
                (do
...
def lmul_pow_val_forwards (arg_ : (BitVec 3)) : SailM Int := do
  let b__0 := arg_
  if (b__0 == (0b101 : (BitVec 3)))
  then (pure (-3))
  else
    (do
      if (b__0 == (0b110 : (BitVec 3)))
      then (pure (-2))
      else
        (do
          if (b__0 == (0b111 : (BitVec 3)))
          then (pure (-1))
          else
            (do
              if (b__0 == (0b000 : (BitVec 3)))

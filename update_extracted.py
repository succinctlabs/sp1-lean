...
#
# The two writers below used to emit a blanket `maxHeartbeats 4000000` + `maxRecDepth 64000` on EVERY
# battery, unmeasured -- 16 of each. The disproof was already sitting in the tree: `BitwiseOperation`
# (396 vectors, 49 KB) carried neither directive and elaborated fine, while `IsZeroOperation`
# (47 vectors, 2.4 KB -- 20x smaller) carried both. The stamp tracked *when a file was last re-dumped*,
# not its cost. Five files generated by an older render path had none at all, so a regeneration would
# have ADDED five more.
#
# Measured 2026-08-02 by stripping all 21 batteries and running `lake test`: exactly two files need
# anything. Note the two directives mask each other -- MulOperation's recursion need only surfaced
# after its heartbeat need was satisfied, so ladder one at a time and re-run.

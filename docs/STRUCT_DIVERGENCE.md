...
9. ~~**`ShiftLeft` decomposition disagrees.**~~ **FULLY CLOSED** (Phase 2.3 + Phase 1 nested-adapter refactor). Both structs now share the same naming + cell-count: `state · adapter: ALUTypeReader T · result · c_bits · v_01/v_012/v_0123 · shift_u16 · lower_limb/higher_limb · limb_result · sllw_msb: U16MSBOperation T · is_sll, is_sllw, is_sllw_imm`. The prior Lean-side `shift_imm_low/shift_imm_high/msb` at Main[29..31] (adapter-flattening padding cells) were removed by switching ShiftLeftCols from inlined adapter fields to `adapter: ALUTypeReader T` (nested), matching the BitwiseChip/LtChip/ShiftRightChip pattern.
...
- Lean (12, post-2026-05-24): CPUState · ITypeReader · `next_pc: Vector T 3` · `is_beq, is_bne, is_blt, is_bge, is_bltu, is_bgeu` · `is_branching` · `compare_operation: LtOperationSigned T` · `adapter_cols`. Matches upstream field count + ordering exactly.
- Status: structural parity reached. The Main[34] field was renamed `lt_is_signed → is_branching` (open-question #6); the Clean-only state-bus carry columns `next_pc_branched_carry`/`next_pc_unbranched_carry` were excised by inlining the carry chain via two `GatedAddOp.assertion` calls (gated by `is_branching` / `1 - is_branching`), mirroring upstream's `when(is_branching) / when(is_real - is_branching)` in `air.rs:185-202`.
...
| `is_branching` | Main[34] | E94 = `Main[34]*(Main[34]-1)` boolean · E96 = `is_real * (Main[34] - E92)` where E92 is the branch-taken predicate (i.e. Main[34] = is_branching under is_real = 1) · `Spec` passes `(cols.is_blt + cols.is_bge)` as the `is_signed` arg to `LtOperationSigned.constraints` (matching upstream's E76) | `is_branching` | raw `builder.assert_bool(local.is_branching)` · constraints forcing `is_branching = (compare result based on opcode flags)` · NOT passed to LtOperationSigned (Rust passes `local.is_blt + local.is_bge` as is_signed there) | EXACT (renamed 2026-05-24; previously `lt_is_signed`. Open-question #6 closed: the field name and the Spec's `is_signed` argument are both correct now.) |
...
> **Branch's DIVERGENT cluster is closed.** All three previously-flagged rows are resolved:
> - The Main[34] slot is now named `is_branching` (rename landed 2026-05-24, commit `82ca65d`); the Clean-only `is_branching_aux` mux selector was removed in the same commit.
> - The Clean-only `next_pc_branched_carry`/`next_pc_unbranched_carry` columns (Lean-only state-bus reify infrastructure) were excised by inlining two `GatedAddOp.assertion` calls targeting the committed `next_pc` directly. This mirrors upstream's `when(is_branching) / when(is_real - is_branching)` carry chains in `control_flow/branch/air.rs:185-202` — no committed carry witnesses.
...
| _none_ | — | — | `adapter_cols` (rest) | `eval_untrusted_program` | — (Rust-only; macro #2) |
|
> **Jal is special in two ways.** (i) Uses two `AddOperation` instances (jump target + return address) where most ALU chips have one. The Lean struct flattens both into Lean-only `next_pc` and `op_a_write_value` `Vector T 4` fields. (ii) Lacks `next_pc_carry_value` — its PC chain is encoded directly in `next_pc` (4 limbs, high forced 0) and the state-bus sends. The `next_pc_carry_value` column was removed across all chips in commit `c87fd21`; the row formerly noting Jal as an exception is moot. The bridge does NOT use `AddOperation` for the second add via the `CS2` style — it uses CS1 (first AddOp), CS3 (second AddOp gated by `is_real - op_a_0`), and embeds state/memory/program bus sends directly in the trailing constraints list.
...
Shared Lean skeleton: CPUState→4 · ITypeReader→9 · `AddressOperation` inlined→4 (`addr_value: Vector T 3` + `addr_top_two_limb_inv: T` ↔ `addr_operation.value: [T;3]` + `top_two_limb_inv: T`) · `MemoryAccessCols` inlined→6 (`load_prev_value: Vector T 4, load_memory_prev_high, load_memory_prev_low, load_memory_flag, load_memory_diff_low, load_memory_diff_high`) · variant fields. (`next_pc_carry_value` was removed across all chips in commit `c87fd21`.)

**Status:** clean flatten. Lean inlines upstream's `MemoryAccessCols<T>` (`prev_value: Word<T>` + `access_timestamp: MemoryAccessTimestamp<T>`) — the 6-field expansion mirrors `prev_value` (4 limbs) + `MemoryAccessTimestamp`'s 4 fields (`prev_high_limb, prev_low_limb, compare_low, diff_low_limb`) packed as scalars. Open-question #7 CLOSED 2026-05-24: `*_memory_flag` is a pure rename of `MemoryAccessTimestamp.compare_low` (NOT a `MemoryAccessColsU8` divergence).
...
| `load_memory_flag` | Main[35] | E30/E31/E32 = `flag * (flag-1) * is_real` (boolean gate under is_real); E34/E35 = `flag * (clk_high - prev_high) * is_real` (gates whether prev access was current-shard or prior-shard); E37/E38 = `(1 - flag) * prev_high` (when flag = 0, prev access is from a prior shard); E40-E43 mixing flag into the diff computation | `memory_access.access_timestamp.compare_low: T` — `MemoryAccessTimestamp<T>` field at `../sp1/.../memory/consistency/columns.rs:18` ("This will be true if the top 24 bits do not match"). Boolean witness selecting the high/low 24-bit clk comparison. | memory bus · `MemoryAccessTimestamp eval` | RENAMED (open-question #7 closed 2026-05-24; not `MemoryAccessColsU8` — pure rename `load_memory_flag` ↔ `compare_low`). |
...
> Rows match LoadByte's `Main[0..37]` exactly except `load_prev_value`/`load_memory_*` rename to `store_prev_value`/`store_memory_*`. Same FLATTENED status; `store_memory_flag` ↔ `MemoryAccessTimestamp.compare_low` (open-question #7 closed 2026-05-24 — see LoadByte's row 8).
...
| `is_mulhu` | Main[79] | as `is_mul` | `is_mulhu` | as `is_mul` | EXACT (slot-order fixed 2026-05-24, commit `664f53a`) |
| `is_mulhsu` | Main[80] | as `is_mul` | `is_mulhsu` | as `is_mul` | EXACT |
| `is_mulw` | Main[81] | as `is_mul` | `is_mulw` | as `is_mul` | EXACT |
| (computed) is_real | (no slot) | trailing `assertZero (sum * (sum - 1))` (E15) gates one-hot | (no analogue) | sum-of-selectors used inline | — |
| _none_ | — | — | `adapter_cols` (rest) | `eval_untrusted_program` | — (Rust-only; macro #2) |
|
> Main[79..81] slot-order CLOSED 2026-05-24 (commit `664f53a`). Lean's `MulCols` field declaration order now matches upstream Rust's `is_mul, is_mulh, is_mulhu, is_mulhsu, is_mulw`; the opcode formula at `SP1Clean/MulChip.lean:110` was updated in lockstep. The Lean-only `next_pc_carry_value` column was removed across all chips in commit `c87fd21`.
...
- Lean (post-Phase-1 nested-adapter refactor): CPUState · `adapter: ALUTypeReader T` (26 cells, Main[6..31]; `op_c: Word T` matches upstream) · `result: Vector T 4` · `c_bits: Vector T 6` · `v_01, v_012, v_0123: T` · `shift_u16: Vector T 4` · `lower_limb, higher_limb: Vector T 4` · `limb_result: Vector T 4` · `sllw_msb: U16MSBOperation T` · `is_sll, is_sllw, is_sllw_imm`.
- Status: **full structural parity reached.** ShiftLeftChip now matches the BitwiseChip/LtChip/ShiftRightChip nested-adapter pattern. The prior 9-field inlining + 3-cell `shift_imm_low/shift_imm_high/msb` padding was a chip-level adapter-flattening artifact; it's been removed by switching to `adapter: ALUTypeReader T`. The Phase 2.3 closure (`shift_pow → v_01/v_012/v_0123`, `limb_shift → lower_limb/higher_limb`, `result_intermediate → limb_result/sllw_msb`, `sign_extend → is_sllw_imm`) stays in place. `next_pc_carry_value` was removed in `c87fd21`.

**Per-column constraint usage** (audit: complete 2026-05-23 — judgement chip, different decomposition)

| Column (Lean) | Slot | Lean constraint refs | Rust column | Rust constraint refs | Status |
|---|---|---|---|---|---|
| `adapter.op_a … adapter.op_c_memory` | Main[6..30] | `ALUTypeReader.constraints` (full 25-cell nested reader) | `adapter` fields | `ALUTypeReader eval` | EXACT (nested adapter; matches BitwiseChip/LtChip/ShiftRightChip pattern after Phase 1) |
| `adapter.imm_c` | Main[31] | `ALUTypeReader.constraints` (R/I selector) · `ProgramTable.assertion` | `adapter.imm_c` | `ALUTypeReader eval` · `eval_untrusted_program` | EXACT (was at Main[28] pre-refactor with 3 dummy padding cells at Main[29..31]; now correctly at Main[31] via the nested adapter) |
...
| `c_bits` (6) | Main[36..41] | 6 boolean asserts in `main` (each bit is 0 or 1) · `sum(c_bits[i] * 2^i)` recovers the bit-shift amount mod 64 · used in `shift_u16` and `v_*` arithmetic | `c_bits` (`[T;6]` — 6-bit decomposition of shift amount mod 64) | `eval` (per-bit booleans + sum reconstruction) | EXACT (was Lean `bit_shift` pre-Phase 2.3; renamed) |
| `v_01, v_012, v_0123` | Main[42..44] | cumulative shift-power products: `v_01 = (b0+1)*(b1*3+1)`, `v_012 = v_01*(b2*15+1)`, `v_0123 = v_012*(b3*255+1)` — the chained `2^k` powers for shift amounts within a byte | `v_01, v_012, v_0123` (3 cumulative bit-power products) | `eval` (cumulative chain) | EXACT (was Lean `shift_pow: Vector T 3` pre-Phase 2.3; split into 3 named scalars) |
| `shift_u16` (4) | Main[45..48] | 4 boolean asserts in `main` (one-hot selector for which byte position the shift moves the bytes to) · sum-one asserts · 4 byte-shift conditional clauses | `shift_u16` (`[T;4]` — one-hot byte-shift selector) | `eval` (per-bit booleans + sum-one + byte-shift conditionals) | EXACT (was Lean `byte_shift` pre-Phase 2.3; renamed) |
| `lower_limb, higher_limb` (4+4) | Main[49..52], Main[53..56] | 8 cells for the byte-shifted limbs (each input limb multiplied by `v_0123` and split across 2 output positions — high half spills into next limb); 4 mux assertions tying these to `limb_result[k]` after `shift_u16` mux | `lower_limb` (`Word<T>` = 4 limbs) + `higher_limb` (`Word<T>` = 4 limbs) | `eval` (byte-shift arithmetic + spill carry) | EXACT (was Lean `limb_shift: Vector T 8` pre-Phase 2.3; split into two `Word<T>`s) |
| `limb_result` (4) | Main[57..60] | 4 cells for the pre-finalization shifted result (before byte_shift mux selects); 4 mux assertions tying `limb_result[k]` to the byte-shifted output | `limb_result` (`Word<T>` = 4 limbs) | `eval` | EXACT (was `result_intermediate[0..3]` pre-Phase 2.3; split out by Phase 2.3) |
| `sllw_msb` (U16MSBOperation T) | Main[61] | bridge's CS0 = `U16MSBOperation.constraints Main[33] { msb := Main[61] } Main[63]` — the SLLW MSB witness driving sign-extension | `sllw_msb: U16MSBOperation<T>` | `U16MSBOperation eval` | EXACT (was `result_intermediate[4]` pre-Phase 2.3; split out by Phase 2.3) |
...
| `is_sllw_imm` | Main[64] | bridge constrains `Main[64] = Main[63] * Main[31] = is_sllw * imm_c = is_slliw` — distinguishes SLLI/SLLIW from SLL/SLLW | `is_sllw_imm` | `eval` · `eval_untrusted_program` | EXACT (renamed Phase 2.3 from `sign_extend`) |
|
> **Open-question #4 FULLY CLOSED.** Phase 2.3 split the shift intermediates and renamed `sign_extend → is_sllw_imm`; the follow-on nested-adapter refactor (Phase 1, see `SP1Clean/ShiftLeftChip.lean:55-67`) removed the `shift_imm_low/shift_imm_high/msb` Main[29..31] padding by switching to `adapter: ALUTypeReader T`. ShiftLeftChip is now structurally identical to upstream's `ShiftLeftCols<T, M>`.
...
| `is_w_imm` | Main[68] | E47 = `Main[68] - (is_srlw + is_sraw) * imm_c` (`is_w_imm = (is_srlw + is_sraw) * imm_c` — boolean witness gating the W-variant immediate adjustment in opcode arithmetic) | `is_w_imm` | `eval` · `eval_untrusted_program` (`builder.assert_eq(local.is_w_imm, (local.is_srlw + local.is_sraw) * local.adapter.imm_c)` at `alu/sr/mod.rs:421`) | EXACT (renamed Phase 2 from `sign_extend`; identical semantics) |
|
> **Open-question #3 CLOSED 2026-05-23.** The 28-cell `intermediates_aux` was decomposed in commit `c3aedab` (Phase 3e) into 11 named sub-fields matching `ShiftRightCols<T, M>` declaration order. Slot pins verified against the bridge's E-expressions in `SP1Chips/ShiftRight/Constraints.lean`. The previously-open `sign_extend` (Lean) ↔ `is_w_imm` (Rust) flag-encoding divergence was closed by renaming `sign_extend → is_w_imm` (the bridge constraint is identical to upstream's `is_w_imm` definition).
...
- **#4. `ShiftLeftCols` decomposition — FULLY CLOSED.** Resolved in two steps: (1) Phase 2.3 (`SP1Clean/ShiftLeftChip.lean:79-102`) split the three Vector blocks into upstream-named sub-fields and renamed `sign_extend → is_sllw_imm` — `shift_pow: Vector T 3 → v_01, v_012, v_0123: T` (Main[42..44]); `limb_shift: Vector T 8 → lower_limb, higher_limb: Vector T 4` (Main[49..56]); `result_intermediate: Vector T 5 → limb_result: Vector T 4 + sllw_msb: U16MSBOperation T` (Main[57..61]). (2) Phase 1 nested-adapter refactor switched ShiftLeftCols from inlined adapter fields + 3 padding cells (`shift_imm_low, shift_imm_high, msb` at Main[29..31]) to `adapter: ALUTypeReader T` (`op_c: Word T`, 26-cell nested struct), matching the BitwiseChip/LtChip/ShiftRightChip pattern and upstream's `ShiftLeftCols<T, M>` declaration order.

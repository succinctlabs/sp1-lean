...
| **Per-column constraint audit** (2026-05-23) | **24 / 24 chips complete** | ~480 column rows total across all chips | 24 DIVERGENT/OPAQUE rows surfaced (top contributors: Branch is_branching/lt_is_signed, Mul slot-order, ShiftLeft/Right decomposition deltas, DivRem aux:209 partial labeling) |
...
**Audit headline findings (2026-05-23).** Out of 24 chips × ~20 rows ≈ 480 column-level comparisons, the audit surfaced these structural concerns that warrant follow-up:
- **Branch — `lt_is_signed` is a misnomer for `is_branching`** (slot Main[34]). The constraint compiler emits Rust's `is_branching` at this slot but the Lean struct calls it `lt_is_signed` and the Lean `Spec` then passes it to `LtOperationSigned.constraints` as the `is_signed` argument. Open-question #6 resolves with a real semantic issue.
- **Mul — Main[79..81] slot-order may differ from upstream** (existing prose says Lean's order is `is_mul, is_mulh, is_mulw, is_mulhsu, is_mulhu`; upstream's struct order is `is_mul, is_mulh, is_mulhu, is_mulhsu, is_mulw`). Worth re-verifying against the constraint compiler's column-name reflection.
- **DivRem — 3-flag `(is_signed, is_w, is_rem)` vs upstream's 8-flag one-hot.** Both encode the same 8 modes (DIV/DIVU/REM/REMU/DIVW/REMW/DIVUW/REMUW), but the encoding is a Boolean-cube projection rather than a direct rename. Constraint-equivalence hinges on the aux:209 block enforcing consistency.
- **9 Load/Store chips — `load_memory_flag`/`store_memory_flag` (Main[35])** is not in upstream's `MemoryAccessCols<T>`, indicating Lean uses (or implicitly models) the `MemoryAccessColsU8` variant for RAM accesses. Open-question #7 partially resolves.
...
5. **`next_pc_carry_value: Vector T 3` is Lean-only at the top of 22/24 chips.** This is almost certainly upstream's `adapter_cols: M::AdapterCols<T>` payload for the user-mode adapter, surfaced explicitly because we have no `M`. The exceptions are `JalCols` and `MulCols` (no `next_pc_carry_value` — they have their own PC handling). **AUDIT UPDATE 2026-05-23:** Confirmed at row level — every chip's audit table has a `next_pc_carry_value (Lean-only)` row with FLATTENED status. The exception list needs revision: Lean's `MulCols` and `JalrCols` actually have `next_pc_carry_value` per commit `6ff1320` ("add next_pc_carry_value column across 24 chips") — the doc text above pre-dates this commit. Of the 24 chips, only `JalCols` truly lacks the field (PC handled via explicit `next_pc: Vector T 4`). See per-chip rows for confirmation.
...
8. **Two opaque `aux` blobs.** `DivRemCols.aux: Vector T 209` and `ShiftRightCols.intermediates_aux: Vector T 28` are unnamed packed-vector witnesses on our side. Upstream gives each constituent a name (DivRem has 40+ named fields including `quotient`, `remainder`, `c_times_quotient_lower: MulOperation<T>`, etc.). These two chips are where the structural picture is least transparent — any deep audit should start by labeling the entries of these `aux` vectors against the upstream field list. **AUDIT UPDATE 2026-05-23:** Partial labeling done. ShiftRight's 28 cells confirmed equal to the sum of upstream's 8 named sub-fields exactly (cell-count match; slot permutation deferred). DivRem's 209 cells reconcile to ~175 named upstream cells + ~34 sub-operation-internal cells (cell-count match within ~16%). Both chips have OPAQUE-tagged rows in the audit; slot-by-slot labeling against the Rust eval impl remains as a follow-up.
...
**Per-column constraint usage** (audit: complete 2026-05-23 — judgement chip, opaque aux:209)

| Column (Lean) | Slot | Lean constraint refs | Rust column | Rust constraint refs | Status |
|---|---|---|---|---|---|
| `Main[0..27]` (clk_high … op_c_memory_diff_low) | Main[0..27] | as Add rows 1-17 (CPUState + RTypeReader on op_a/op_b/op_c register & memory triples) | as Add | as Add | FLATTENED (see Add) |
| `op_a_write_value` (4) | Main[28..31] | `RTypeReader.constraints` (write_value) · referenced by quotient/remainder mux logic in `aux` block | `a` (`Word<T>` — write-back; upstream stores separately from `mul_operation.value`, similar pattern to Mul) | `eval` · `RTypeReader eval` | RENAMED (`op_a_write_value` ↔ `a`) |
| `aux` (209) — **OPAQUE block** | Main[32..240] | The 209-cell aux block encodes the full DivRem arithmetic witness: `c_times_quotient: [T; LONG_WORD_SIZE=16]` (16), `carry: [T; LONG_WORD_SIZE=16]` (16), two `MulOperation<T>` sub-structs (`c_times_quotient_lower` + `c_times_quotient_upper`, each ~34 cells = 68), `IsZeroWordOperation<T>` (`is_c_0`, ~5 cells), `LtOperationUnsigned<T>` (`remainder_lt_operation`, ~10 cells), two `IsEqualWordOperation<T>` (`is_overflow_b` + `is_overflow_c`, ~10 cells), two `AddOperation<T>` (`c_neg_operation` + `rem_neg_operation`, 8 cells), four `U16MSBOperation<T>` (`b_msb, rem_msb, c_msb, quot_msb`, 4 cells), plus the named scalar witnesses (`quotient, quotient_comp, remainder_comp, remainder, abs_remainder, abs_c, max_abs_c_or_1` as 7 `Word<T>` = 28 cells), and the per-mode boolean flags (`is_overflow, b_neg, b_neg_not_overflow, b_not_neg_not_overflow, is_real_not_word, rem_neg, c_neg, abs_c_alu_event, abs_rem_alu_event, remainder_check_multiplicity`, 10 cells). Total estimated: 16 + 16 + 68 + 5 + 10 + 10 + 8 + 4 + 28 + 10 = 175 cells, leaving ~34 unaccounted (likely sign-extension witnesses or per-mode mux cells). | upstream's ~40 named columns (see Per-chip diff prose above) | `MulOperation eval` (×2) · `IsZeroWordOperation eval` · `LtOperationUnsigned eval` · `IsEqualWordOperation eval` (×2) · `AddOperation eval` (×2) · `U16MSBOperation eval` (×4) · raw `builder.assert_*` for the named scalars + flags | **OPAQUE — partial labeling.** The 209-cell budget is consumed by 6 named sub-operations + ~28 named scalar witnesses + ~10 boolean flags ≈ 175 of 209 cells. The remaining ~34 cells are likely sub-operation-internal witnesses (e.g. `MulOperation`'s `b_lower_byte`, `c_lower_byte`, `b_msb`, `c_msb`, `product_msb`, sign-extends) that the bridge expands but the existing prose doesn't enumerate. Resolves open-question #2 by cell-count and named-sub-op decomposition; slot-level labeling deferred to a follow-up reading the bridge's E-expressions against the Rust eval. |
| `is_signed` | Main[241] | `ProgramTable.assertion` (opcode mux contributes `is_signed * 0` — i.e. signed-mode doesn't shift opcode in the simple expression; encoded via the aux selector chain) · trailing `assertZero (is_signed*(is_signed-1))` · gates signed-vs-unsigned arithmetic in aux | (no single upstream column — instead upstream emits 8 separate `is_div, is_divu, is_rem, is_remu, is_divw, is_remw, is_divuw, is_remuw` flags) | `eval` (signed-mode branches in each per-variant flag) · `builder.assert_bool` | **DIVERGENT — encoding scheme differs.** Lean uses a 3-flag encoding `(is_signed, is_w, is_rem)` for 8 modes; upstream uses an 8-flag one-hot encoding. The 3-flag encoding is more compact (3 bits vs 8 cells) but requires careful proof that the same 8 modes are reachable. |
| `is_w` | Main[242] | as `is_signed` (gates 32-bit vs 64-bit arithmetic) | (subset of 8 upstream flags) | `eval` | DIVERGENT (encoding scheme; see `is_signed`) |
| `is_rem` | Main[243] | as `is_signed` (gates remainder-vs-quotient output) | (subset of 8 upstream flags) | `eval` | DIVERGENT (encoding scheme) |
| `is_real` | Main[244] | `CPUState.constraints` · `ProgramTable.assertion` · `is_real_not_word` chain in aux · `assertZero (is_real*(is_real-1))` | `is_real` | `eval` · `CPUState eval` · `builder.assert_bool` | EXACT |
| `msb_aux1` | Main[245] | Lean-specific auxiliary witness (per existing per-chip prose: "`msb_aux1` is Lean") | (no analogue) | — | DIVERGENT (Lean-only) |
...
**Per-column constraint usage** (audit: complete 2026-05-23 — judgement chip, opaque intermediates_aux:28)
...
| `op_a_write_value` (4) | Main[32..35] | `ALUTypeReader.constraints` (write_value) · shift-arithmetic mux clauses in bridge that pick the result from `intermediates_aux` | `a` (`Word<T>` — write-back register output) | `ALUTypeReader eval` · shift-arithmetic eval | RENAMED (`op_a_write_value` ↔ `a`) |
| `intermediates_aux` (28) — **OPAQUE block** | Main[36..63] | Many bridge E-expressions reference these slots for the shift-arithmetic chain; the cell-by-cell semantics depend on reading the SP1 Rust eval impl. The 28 cells correspond bit-for-bit to upstream's 18 named columns spanning `b_msb` (1, `U16MSBOperation`), `srw_msb` (1), `c_bits` (6), `sra_msb_v0123` (1), `v_0123` (1), `v_012` (1), `v_01` (1), `lower_limb` (4), `higher_limb` (4), `limb_result` (4), `shift_u16` (4) — sum = 28. | upstream named cells: `b_msb.msb, srw_msb.msb, c_bits[0..5], sra_msb_v0123, v_0123, v_012, v_01, lower_limb[0..3], higher_limb[0..3], limb_result[0..3], shift_u16[0..3]` (28 cells total) | `eval` (shift-arithmetic chain) · `U16MSBOperation eval` (×2) | **OPAQUE — permutation expected, slot order unverified.** Cell count matches exactly (28 = sum of upstream sub-fields), so the mapping is a permutation. Identifying each slot requires reading the Rust eval impl and comparing against the bridge's E-expressions. Resolves open-question #3 in count only — slot-by-slot labeling deferred. |
...
| `sign_extend` | Main[68] | E47 = `Main[68] - (is_srlw + is_sraw) * imm_c` (sign_extend is 1 for SRA/SRAW when shift amount triggers sign-extension) · sign-extension multiplexing in arithmetic | (no direct analogue — upstream's `is_w_imm` is the closest, but tracks a different flag) | `eval` | DIVERGENT (Lean-only flag; upstream's missing `is_w_imm` is conceptually paired but semantically different) |
...
| `next_pc_carry_value` (3) | (Lean-only) | trace-level | `adapter_cols.next_pc_carry` | `eval_untrusted_program` | FLATTENED (macro #5) |
|
> **Open-question #3 partially resolved.** The 28-cell `intermediates_aux` matches upstream's 28-cell shift-arithmetic witness footprint exactly (8 nested sub-fields summing to 28). The mapping is a permutation; identifying each Lean slot's upstream counterpart requires cross-referencing the bridge's E-expressions with the Rust eval impl line-by-line — a follow-up task. The chip's structural integrity is verified at the cell-count level; the constraint-equivalence claim hinges on the unverified permutation.
...
- **#2. `DivRemCols.aux: Vector T 209` — PARTIAL.** Cell-count breakdown done in the DivRem table (175 named cells + ~34 sub-operation-internal cells = 209). Slot-by-slot labeling against upstream's `quotient, remainder, c_times_quotient_lower, ...` field names requires reading `SP1Chips/DivRem/Constraints.lean` (681 lines) against `alu/divrem/mod.rs` line-by-line. Follow-up task.
- **#3. `ShiftRightCols.intermediates_aux: Vector T 28` — PARTIAL.** Cell-count confirmed equal to sum of 8 upstream sub-fields (28 cells). Slot-by-slot permutation deferred (same follow-up shape as #2).
- **#4. `ShiftLeftCols` decomposition — PARTIAL.** Five role-based mappings established: `bit_shift↔c_bits`, `shift_pow↔(v_01, v_012, v_0123)`, `byte_shift↔shift_u16`, `limb_shift↔(lower_limb, higher_limb)` (8 = 4+4), `result↔a`. Two residual divergences flagged: (i) Lean's `shift_imm_low`/`shift_imm_high` are intermediate columns not present upstream; (ii) Lean's `result_intermediate` has 5 cells vs upstream's `limb_result`'s 4 cells (5th cell role unclear). Lean is also missing `is_sllw_imm` (folded into `imm_c`).
...
- **#6. `BranchCols` `is_branching` and `lt_is_signed` — CLOSED 2026-05-23 (with DIVERGENT finding).** The constraint compiler emits upstream's `is_branching` column at Main[34]. The Lean struct calls it `lt_is_signed`, and the Lean `Spec` then passes it to `LtOperationSigned.constraints` as the `is_signed` argument. This is **semantically wrong** — the LtOp's is_signed input should be `is_blt + is_bge` (which the bridge does pass as E76, separately). Recommended fix: rename Lean's `lt_is_signed` field to `is_branching` to match upstream, and fix the Lean `Spec` to pass the correct value to LtOp. The Lean side's `is_branching, next_pc_branched_carry, next_pc_unbranched_carry` (added in commit 6ff1320 for state-bus reify) are distinct Clean-only columns with no bridge slot.
- **#7. `MemoryAccessCols` granularity — PARTIAL.** Lean's 9 Load/Store chips have a `{load,store}_memory_flag` (Main[35]) that doesn't appear in upstream's standard `MemoryAccessCols<T>`. This suggests Lean uses the `MemoryAccessColsU8` variant (or equivalent) for RAM accesses, which carries an extra is_real-equivalent flag because RAM accesses may be initial (no prior access) where register accesses always have one. Follow-up: read `crates/core/machine/src/memory/consistency/columns.rs` to pin the exact variant Lean is mirroring.
- **#8. `TrustMode` mode parameter — OPEN (out of audit scope).** Decision needed before any system-layer chips are ported: do we mirror the polymorphic `M: TrustMode` design, or commit to user-mode-only and hardcode? Affects every existing chip Cols if we adopt later. Not addressed by the per-column audit since it concerns chips not yet ported.

### Newly-surfaced concerns (not in original open questions)

- **Mul slot-order Main[79..81] flag.** Existing per-chip prose says Lean's selector order is `is_mul, is_mulh, is_mulw, is_mulhsu, is_mulhu`; upstream's struct order is `is_mul, is_mulh, is_mulhu, is_mulhsu, is_mulw`. The constraint compiler should emit the Rust order into the bridge, so Lean's field at Main[79] is actually upstream's `is_mulhu` (and Lean's `is_mulw` would be at Main[81]). The Mul audit table tagged Main[79..81] as RENAMED-slot. Verification needed: read the constraint compiler output's column-name reflection.
- **DivRem 3-flag vs 8-flag encoding.** Lean's `(is_signed, is_w, is_rem)` 3-bit encoding represents 8 modes, where upstream uses 8 separate flags `is_div/divu/rem/remu/divw/remw/divuw/remuw`. These should be related by a Boolean-cube projection: `is_rem = is_rem | is_remu | is_remw | is_remuw`, `is_signed = ¬(is_divu | is_remu | is_divuw | is_remuw)`, `is_w = is_divw | is_remw | is_divuw | is_remuw`. The aux:209 block must enforce these consistency relations for the two encodings to be equivalent.
- **ShiftLeft `result_intermediate[4]` (the 5th cell) role.** Lean has 5 cells; upstream's `limb_result` is 4. The 5th cell may be a sign-extension witness specific to SLLW, but the bridge alone doesn't make this obvious.
...
- Slot-by-slot labeling of DivRem aux:209 and ShiftRight intermediates_aux:28 against upstream named fields.
- Verification of Mul Main[79..81] slot-order claim against the constraint compiler's reflection output.
- Re-verification that the Branch `lt_is_signed`/`is_branching` semantic mismatch is real (not just a name issue) by running the constraint compiler with column-name reflection and confirming slot Main[34] is `is_branching`.

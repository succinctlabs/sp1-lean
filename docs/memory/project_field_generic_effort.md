...
description: ALL 24 chips have `correct_*_poly` companions; constraint extractor now emits field-generic Lean directly (no Python post-process); update_constraints.py is a verbatim writer
type: project
originSessionId: f9ab5b6f-d305-43ed-a962-8c272d98a1d1
---
The repo's field-genericization is **substantively complete**. All 24 chips
have at least one `correct_*_poly` theorem; `grep -rn "Fin KB" SP1Chips
SP1Operations` returns 0 matches; `lake build` is clean (8522 jobs, 0/0).
The only remaining `Fin KB` references in the formalization live in
`SP1Foundations/Field.lean` (the concrete KoalaBear instance block at
lines 93-110) — intentional, anchors the `F := Fin KB` instantiation.

**Update 2026-05-15: constraint extractor emits `F` directly.** The upstream
`sp1-constraint-compiler` (branch `dtumad/field-generic-constraint-extraction`
in `../sp1`) was rewritten to emit field-generic Lean — `{F : Type} [Field F]
[CoeHead F ℕ]? (Main : Vector F N) : SP1ConstraintList F` — instead of
`Fin KB`-bound output that `update_constraints.py` post-processed. Changes in
`crates/hypercube/src/ir/{shape,expr,func,var,ast}.rs` plus the binary at
`crates/core/compiler/src/main.rs` (transitive `CoeHead F ℕ` fixpoint over the
module call graph). `update_constraints.py` is now a verbatim writer: dropped
`PARAMETRIC_OPS`/`PARAMETRIC_CHIPS` dicts + both `apply_parametric_*` post-
process functions (~200 lines deleted). 4 scalar-only ops (`IsZeroOperation`,
`U16CompareOperation`, `U16MSBOperation`, `CPUState`) lost `Type*`
universe-polymorphism — the Rust IR can't see whether a `Shape::Struct`
parameter's hand-written Lean counterpart contains `Word F` internally, so
operations are unconditionally `F : Type`. Zero downstream callers depended
on `Type*`. Follow-up cleanup: `../sp1/crates/core/compiler/src/ir/` (2839
lines, `ast.rs`+`builder.rs`) is dead code — confirmed by moving it aside
and rebuilding successfully — but deletion wasn't authorized in this session.

**Update 2026-05-15: DivRem migration VERIFIED COMPLETE** (commits
`e653978` Word.lean refactor + `091b675` DivRem closure). All 8 variants
(div/divu/divw/divuw/rem/remu/remw/remuw) have `correct_*_poly` companions
that elaborate sorryAx-free. Documented blocker from `a62938b` on the
3-form `w_eq_msb_*` hypothesis variance closed via per-side-goal `first
| … | … | …` derivation chains (3 alternatives covering the 3 known
forms) plus a new `msb_arm_closer_poly` helper in
`SP1Chips/DivRem/Common.lean` for `Word.isU64_poly #v[x, y, m*65535,
m*65535]` shapes. `div_poly` / `rem_poly` use the simpler pre-extract
limb-bounds recipe (no msb derivations needed). **22 of 24 chips done;
ShiftLeft + ShiftRight remain** (both blocked by KB-specific `64⁻¹`
literal — see `feedback_shift_kb_specific_literal.md`).

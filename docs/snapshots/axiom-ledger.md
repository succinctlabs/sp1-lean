...
> Current snapshot: **2026-07-14**. Regenerate with `scripts/run_audit.sh` before relying on it.
> The authoritative per-declaration output is [`axiom-census.txt`](axiom-census.txt).

## Current result

The audit passes with **458 elaborated probes**, no project `axiom` declarations, no
`skipKernelTC`, and no `native_decide` in the main `SP1Clean/` library. Of those probes, **32**
...
The probe generator now includes every chip's bundled `circuit` and DivRem's separately housed
completeness driver. That expansion fixed an audit-harness blind spot: the previous census saw the
textual DivRem `stop`, but did not issue `#print axioms SP1Clean.DivRemChip.completeness`.
...
| `MulChip.completeness` | completeness/liveness | witness normalization regression |
...
## Transitive `sorryAx` carriers (32)

The exact set is machine-gated in `scripts/run_audit.sh`. It consists of:

- 13 chip declarations: the five completeness theorems; the five affected bundled circuits; and
  DivRem's `evidenceSoundness`, `contractSoundness`, and `soundness`;
- 18 registry/coverage/ensemble/capstone declarations that embed or consume those circuits and the two
  machine seams; and
...
The latest full build completed 3537 jobs with no errors, warnings, or stray `info:` output. The probe

...
Snapshot date: 2026-08-24, updated for the shard-model/registry consolidation and awaiting the
post-commit raw-census restamp.
The raw
per-declaration output is split by library — [`axiom-census.txt`](axiom-census.txt) (the main
`SP1Clean` library, 871 declarations) and [`axiom-census-test.txt`](axiom-census-test.txt) (the
...
- 941 released declarations are probed (871 main + 70 test: the 34 real-row satisfiability
  ...
  capstones. The test scope includes four exact boundary-only results that jointly witness the

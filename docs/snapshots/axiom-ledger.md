...
`SP1CleanTest` conformance anchors, 21 declarations) — and is regenerated by
...
- 476 released declarations are probed. The count fell from 478 because `SailDecode.decode_ADD_example`
...
- **The migration changed no declaration's axiom set.** Diffed against the pre-migration census, the
  only difference is those two deleted declarations: all 476 survivors are byte-identical. This is the

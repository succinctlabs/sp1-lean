...
Snapshot date: 2026-08-22, restamped for the exact-AIR/native-artifact campaign following the
PR #110 external-audit response.
The raw
per-declaration output is split by library — [`axiom-census.txt`](axiom-census.txt) (the main
`SP1Clean` library, 808 declarations) and [`axiom-census-test.txt`](axiom-census-test.txt) (the
...
battery, 66 declarations) — and is regenerated by
...
- 874 released declarations are probed (808 main + 66 test: the 34 real-row satisfiability
...
- 120 textual `native_decide` lines are isolated in `SP1CleanTest/` (the witness/trace conformance

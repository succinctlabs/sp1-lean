...
Snapshot date: 2026-08-31, restamped for the halt-table wave (the Exit hand-off, the
committed-fragment Program re-base, and the halted-shard Sail conclusion).
The raw
per-declaration output is split by library — [`axiom-census.txt`](axiom-census.txt) (the main
`SP1Clean` library, 918 declarations) and [`axiom-census-test.txt`](axiom-census-test.txt) (the
...
battery, 77 declarations) — and is regenerated by
...
- 995 released declarations are probed (918 main + 77 test: the 34 real-row satisfiability
  ...
  extracted `Opcode` discriminant table; the 2026-08-31 halt-table wave adds ten main-scope
  probes — the Exit hand-off's forcing theorems (`witness_exitMessages_eq`,
  `witness_exit_code_zero_of_haltFree`, `witness_realHaltRows_eq_of_mem`,
  `witness_haltTable_table_eq_of_mem`), the per-chip fetch discriminant
  (`supportedChip_fetchDiscriminantShape`, `witness_realDecodedInstructionRows_opcodeNeEcall`),
  the halted-shard conclusions (`haltedSail_of_haltGrounding`, `haltedExecution_of_haltGrounding`,
  `supported_core_native_shard_execution_halted`), the verifier's Exit pull
  (`sp1StateVerifierMain_exitInteractions`), and the first whole-execution claim
  `supported_core_boot_to_halt_single_shard`; the 2026-08-21 pair `stateBumpChip_faithful` /
  ...
  typed Syscall/MemoryLocal system-table projections and relation-level row facts. The final
  common-shard pass adds fifteen net main probes: the single production-field facts, shared natural
  ledger bridge, transcript evaluator laws, paired exact-cluster eliminators, direct native shard
  construction/soundness, and conditional correctness/language-equality capstones; it replaces the
  two retired intermediate ordinary-trace probes. The test scope adds two common-witness evaluator
  and round-trip results, for six zero-event results that jointly witness the admissible source and
  invoke both AIR-witness and direct-ensemble conclusions. The active-path audit adds five test
  probes: the official Sail self-jump, its exact deterministic-compiler event, the one-transition
  semantic witness, the nonempty bounded native witness, and soundness back into the shared
  semantic language. The 2026-08-29 halt-table chunk adds the three standalone `HaltChip`
  probes — `soundness`/`completeness`/`circuit`, the native system table witnessing a halting
  shard's `HALT` ECALL (ensemble wiring follows separately) — and the two committed-row-fragment
  probes (`decodedInROM.committed`, `committedInROM.ecall_of_opcode`), which inherit the Sail
  platform hooks through `ConfiguredDecode` like the rest of the decode layer.
...
- The changes above intentionally extend the released census. Surviving declaration axiom sets
  remain stable; the active-path anchors inherit the already disclosed test-library `native_decide`
  quarantine and generated Sail platform hooks, while the field, natural-ledger, and relational
  plumbing proofs add no new trust class. This remains the release gate for a
  toolchain or Sail-model bump: an unexplained changed axiom *set* is a blocker, whereas a moved

...
Snapshot date: 2026-08-21, restamped at the close of the PR #110 external-audit response
(waves W0-W6: bump-chip soundness, machine completeness, and the extracted-to-native transport).
The raw
per-declaration output is split by library — [`axiom-census.txt`](axiom-census.txt) (the main
`SP1Clean` library, 626 declarations) and [`axiom-census-test.txt`](axiom-census-test.txt) (the
`SP1CleanTest` anchors: 34 real-row satisfiability + 3 independent-audit anchors + the joint
non-vacuity anchor + the generated-trace non-vacuity anchor, 40 declarations) — and is regenerated by
...
- 666 released declarations are probed (626 main + 40 test: the 34 real-row satisfiability

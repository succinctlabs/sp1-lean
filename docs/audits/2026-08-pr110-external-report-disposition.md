...
| 2 | HIGH | StateBump omitted: shards capped at ~2²¹ rows / no 64 KiB pc-boundary crossing, and the strict clock-rank argument does not survive adding it | **Closed (W3, 2026-08-22).** The ensemble is now 40 tables: native StateBump and MemoryBump chips are proved sound and complete, and the boundary verifier range-checks the split-limb public values (D5-A, the faithful upstream layout). **State side**: the trail runs on a canonicalized rank, StateBump edges cancel as canon self-loops, and `RankedGrounding`/`TimedGrounding.walk` are byte-identical — so the ~2²¹-row cap and the 64 KiB pc-boundary restriction are **lifted**. **Memory side**: the interim scope restriction `realMemoryBumpRows witness = []` (a conjunct of `SupportedCoreMemoryTimestampRangeRelation`) is **removed** — the capstone now covers shards whose MemoryBump table carries active refresh rows. `Soundness/RefreshWiring.lean` runs `RefreshElimination.eliminate` per location on the widened balance (each active bump row is a genuine `IsRefresh`, its pulled record's two timestamp facts derived from the produced side of the same balance), realizes the rewritten pulls row by row through `AlignedCarrier.exists_rewrittenTouchLists`, and feeds the walk a `ValueAligned` carrier; `memoryFinalizeTruth`'s ∃-witness form absorbs the rewritten finalize records exactly as W2 anticipated. **Timestamp premise (report §8.2)**: also closed. `SupportedCoreMemoryTimestampRangeRelation` is **deleted** — `SupportedCoreNativeRelation` is now exactly two conjuncts (ensemble algebra + semantic boundary binding). The circularity that forced it to be a premise was `ChipGroundingContracts.rowAligned` taking the bound as a *hypothesis*, so the per-row touch lists that the memory balance is built from could not exist until the bound was already known; moving it into the per-touch antecedent of that field's slot conjunct broke the cycle, and the capstone discharges both antecedents from its own `pushGood`/`pullGood` (genesis frontier from the init provider's zeroed clock limbs, instruction-row pushes from `TouchOK.push_hi` under the range-checked `< 2^48` shard-time ceiling, MemoryBump pushes from `memoryBump_pushedMessage_clkFacts`). `TimedGrounding.walk` is unchanged. | **closed** |

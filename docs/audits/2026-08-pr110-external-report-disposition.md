...
| 3 | MED | Joint non-vacuity unwitnessed; the only end-to-end decode witness was deleted by the PR; no `Fact (2^25 < p)` instance exists | Accepted. W1 restores one concrete `ext_decode` reduction per instruction family against the Sail-v5 model (the retained `run_bind_ok_*` helpers are the recipe), adds the missing per-family `decodedInROM_*_hoist` lemmas, a `Fact (2^25 < SP1Prime)` instance, and a joint anchor extending `SP1CleanTest/Audit/OneAddNativePremises.lean` to the full `SemanticBoundaryBinding`. | **open — W1** |
| 4 | MED | The capstone discards `TimedGrounding.walk`'s proved final-State truth and finalize-Memory truth | Accepted. W2 exports both conjuncts (shaped to survive W3's refresh-elimination rework). | **open — W2** |
...
- **Shift/Bitwise Specs omit the reader sub-Spec / is_real binarity clause** — W2 adds the clauses
  for uniformity (or records the cost-based why-not).

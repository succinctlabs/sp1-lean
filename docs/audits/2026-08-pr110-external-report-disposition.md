...
| 15 | LOW | Machine-level completeness absent; the previous partial scaffold was deleted | Substantially closed at the native ensemble boundary. `nativeTrace` is a total proof-independent compiler over the supplied `Machine.EventExecutionTrace`; it covers all 25 instruction families, refresh rows, Memory boundaries, and canonical Byte/Range/Program closure. `supported_core_native_functionalCompleteness` proves all constraints and four-channel balance for that exact map on `SupportedCoreNativeAdmissibleExecutionRelation`, with no existential trace and no `2 * multiplicity ≤ p` restriction. The remaining compiler-domain gap is rich per-chip event validity, Memory genesis, configured-state Program decode stability, and actual footprint on an intended bounded semantic domain. Because exact soundness currently targets an unbounded relation with no row-cap or footprint conclusion, public-language equality also requires a shared capacity-bounded semantic relation. | **partial — full deterministic native compiler image proved; two-sided language alignment open** |
...
| Bump-row generation | W5 | The honest generator was to *derive* StateBump/MemoryBump inputs at clock/pc window crossings — exactly what makes shards above ~2²¹ rows witnessable | Delivered for the deterministic native compiler. `AccessSchedule.lean` inserts MemoryBump events before register touches which cross the 24-bit window; `stateBumpEvents` derives StateBump events; `Table.build` constructs both physical tables; and the State/Memory histories expose their hand-off ledgers. The remaining admissibility work concerns semantic row agreement/validity, not placement. |

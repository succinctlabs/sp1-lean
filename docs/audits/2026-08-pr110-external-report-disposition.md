...
| 2 | HIGH | StateBump omitted: shards capped at ~2²¹ rows / no 64 KiB pc-boundary crossing, and the strict clock-rank argument does not survive adding it | **Largely closed (W3, 2026-08-22) — with one stated residual.** The ensemble is now 40 tables: native StateBump and MemoryBump chips are proved sound and complete, and the boundary verifier range-checks the split-limb public values (D5-A, the faithful upstream layout). **The state side is fully closed**: the trail runs on a canonicalized rank, StateBump edges cancel as canon self-loops, and `RankedGrounding`/`TimedGrounding.walk` are byte-identical — so the ~2²¹-row cap and the 64 KiB pc-boundary restriction are **lifted**. **The memory side is scope-restricted**: `SupportedCoreMemoryTimestampRangeRelation` gained the conjunct `realMemoryBumpRows witness = []`, so the capstone covers only shards whose MemoryBump table carries no *active* refresh row. That is a genuine restriction, not a vacuous hypothesis (an all-padding bump table satisfies it, and the joint anchor exhibits one), and it is strictly weaker than the pre-W3 position, where no bump table existed at all. Remaining to lift it: the list-level redistribution of `RefreshElimination.eliminate`'s rewritten pull multiset back onto individual rows' `memPulls`; everything downstream of that step is already proved (`ValueAligned` carrier transports, `memoryBump_isRefresh`, the per-table message enumerations). The timestamp-premise derivation (Finding 8.2) is likewise still open and is tracked as W3's follow-up. | **largely closed — memory-refresh residual + timestamp premise open** |

...
- [capstone-seam-plan.md](capstone-seam-plan.md) — dependency-ordered plan to close the `supported_core_native_sound` seam and the 6 chip admissions: the closure set, what is already proved, the `rowAligned`-upfront refactor (in progress), the `walk`-assembly steps, the 24-chip rollout, and the parallel admission tracks.

        ...
        (do
          ...
          for j in [loop_j_lower:loop_j_upper:1]i do
            ...
            loop_vars_1 ← do
              ...
              | .Ok true => (pure ())
              | .Ok false =>
                (internal_error "riscv_insts_vext_mem.sail" 202 "store got false from vmem_write")
              | .Err e => SailME.throw (e : ExecutionResult)
          (pure loop_vars_1))
      else (pure ())
  (pure loop_vars)
        ...
        (do
          ...
          for j in [loop_j_lower:loop_j_upper:1]i do
            ...
            loop_vars_1 ← do
              ...
              | .Ok true => (pure ())
              | .Ok false =>
                (internal_error "riscv_insts_vext_mem.sail" 323 "store got false from vmem_write")
              | .Err e => SailME.throw (e : ExecutionResult)
          (pure loop_vars_1))
      else (pure ())
  (pure loop_vars)
            ...
            let cur_elem ← (( do
              ...
              for i in [loop_i_lower:loop_i_upper:1]i do
                ...
                loop_vars ← do
                  ...
                  | .Ok true => (pure ())
                  | .Ok false =>
                    (internal_error "riscv_insts_vext_mem.sail" 619
          ...
          loop_vars_1 ← do
            ...
            for i in [loop_i_lower:loop_i_upper:1]i do
              ...
              loop_vars_2 ← do
                ...
                | .Ok true => (pure ())
                | .Ok false =>
                  (internal_error "riscv_insts_vext_mem.sail" 634 "store got false from vmem_write")
                | .Err e => SailME.throw (e : ExecutionResult)
                (pure (cur_elem +i 1))
            (pure loop_vars_2)
        (pure loop_vars_1) ) : SailME ExecutionResult Int )
            ...
            (do
              ...
              then
                (do
                  ...
                  | .Ok true => (pure ())
                  | .Ok false =>
                    (internal_error "riscv_insts_vext_mem.sail" 690
                      "store got false from vmem_write")
                  | .Err e => SailME.throw (e : ExecutionResult))
              else (pure ())))
      else

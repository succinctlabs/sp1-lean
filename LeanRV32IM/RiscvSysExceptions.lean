...
def prepare_trap_vector (p : Privilege) (cause : (BitVec 32)) : SailM (BitVec 32) := do
  let tvec ← (( do
    ...
    | User => (internal_error "model/riscv_sys_exceptions.sail" 25 "Invalid privilege level") ) :
    SailM Mtvec )
  match (tvec_addr tvec cause) with
  | .some epc => (pure epc)
  | none => (internal_error "model/riscv_sys_exceptions.sail" 29 "Invalid tvec mode")

def get_xepc (p : Privilege) : SailM (BitVec 32) := do
  match p with
  | Machine => (align_pc (← readReg mepc))
  | Supervisor => (align_pc (← readReg sepc))
  | User => (internal_error "model/riscv_sys_exceptions.sail" 45 "Invalid privilege level")
...
def set_xepc (p : Privilege) (value : (BitVec 32)) : SailM (BitVec 32) := do
  ...
  | User => (internal_error "model/riscv_sys_exceptions.sail" 54 "Invalid privilege level")
  (pure target)

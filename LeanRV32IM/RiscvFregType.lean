...
def FRegStr (r : (BitVec (bif false then 8 else 4 * 8))) : String :=
  (BitVec.toFormatted r)

def fregval_from_freg (r : (BitVec (bif false then 8 else 4 * 8))) : (BitVec (bif false then 8 else 4 * 8)) :=
  r

def fregval_into_freg (v : (BitVec (bif false then 8 else 4 * 8))) : (BitVec (bif false then 8 else 4 * 8)) :=
  v

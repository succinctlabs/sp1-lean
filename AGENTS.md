...
- **`Operations/`** — the native gadgets: `<Op>/{Populate,RawSpec,Formal}.lean` (the auto-gen circuit form
  moved to `Extracted/Circuit/`) plus flat ops (`BitwiseU16Operation.lean`, `AddressOperation.lean`, …).
- **`Chips/`** — per-chip directories `<Op>Chip/`: `Defs.lean` (`main` + `ElaboratedCircuit`),
  `Formal.lean` (soundness/completeness/`circuit`; the semantic `Spec` lives in
  `FormalModel/Contracts/Chips.lean`, and the ALU chips' `Assumptions`/`ProverAssumptions` in
  `FormalModel/Contracts/ChipAssumptions.lean`), and (where present) `Bridge.lean` (the Sail bridge).
  `ByteChip.lean`/`ProgramChip.lean`/`MemoryProvider.lean` are flat pure-lemma "receiver" infra files.
- **`Faithful/`** — the per-operation constraint anchors (`<Op>.lean`).
- **`WitnessTests/`** — the hand-written `<Op>Witness.lean` `native_decide` conformance anchors plus the
  shared `WitnessConformance.lean` scaffold (the auto-generated vectors they consume now live under
  `Extracted/WitnessVectors/<Op>.lean`). All in namespace `SP1Clean.WitnessTests`; vectors regenerated by
  `update_extracted.py`.
...
`SP1Extracted` / `SP1FormalModel` (selected by submodule globs, e.g. `"SP1Clean.Math.+"`). `lake build
SP1Extracted` builds just that layer. Isolation is **by convention** — Lake does not forbid cross-layer
...
the ALU chips' `Assumptions`/`ProverAssumptions`), and the layer libraries. Planned (approved plan, not yet
done): the `Native/`+`Proofs/` pillar re-bucket of `Chips`/`Operations`/`Readers`; lifting the remaining
chips' `Assumptions` (helper-dependent chips Jal/Jalr/Branch/DivRem/ShiftLeft/ShiftRight + split-`Spec`
Lt/Bitwise keep theirs in their proof files); and surfacing the trace statements in `FormalModel/Trace/`.

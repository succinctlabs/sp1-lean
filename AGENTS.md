...
- **`TraceGenTests/`** — the **full-trace** chip testing layer (self-contained; no `WitnessTests/`
  ...
  `generate_trace` (see `docs/architecture.md` §"Witness generation, trace generation"). 7 chips:
  Add/Sub/Subw/Addw unmasked; Mul/Bitwise/Lt masked-flags (constant-zero flag witnesses). Namespace
  `SP1Clean.TraceGenTests`; vectors regenerated by `update_extracted.py` (`EXTRACT_ONLY=<Chip>Trace`).

...
- **`WitnessTests/`** — the **per-operation** witness conformance bridge (temporary, op-level):
  auto-generated `<Op>WitnessVectors.lean` vectors (dumped from SP1's real `populate`) paired with
  hand-written `<Op>Witness.lean` `native_decide` anchors, plus the shared `WitnessConformance.lean`
  scaffold. Namespace `SP1Clean.WitnessTests`.
- **`TraceGenTests/`** — the **full-trace** chip testing layer (self-contained; no `WitnessTests/`
  dependency): the **circuit-as-trace-generator** (`TraceGenerator.lean` + `EventPopulate.lean` +
  `Conformance.lean` + `<Chip>TraceWitness(+Vectors).lean`) — whole chip traces derived from the
  chips' own `main` witness closures + output struct, `native_decide`-checked against SP1's real
  `generate_trace` (see `docs/architecture.md` §"Witness generation, trace generation"). 9 chips,
  **all unmasked**: Add/Sub/Subw/Addw fixed-witness; Mul/Bitwise/Lt/ShiftLeft/ShiftRight
  hint-driven flags (per-event `ProverHint` built from the dumped executor opcode; full-variant
  batteries). Namespace `SP1Clean.TraceGenTests`; vectors regenerated by `update_extracted.py`
  (`EXTRACT_ONLY=<Chip>Trace`).
